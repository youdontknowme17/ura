...
BEGIN_MESSAGE_MAP(CQuestEditDlg, CDialog)
	...
	//}}AFX_MSG_MAP
	ON_WM_CLOSE()
	ON_BN_CLICKED(IDC_BUTTON_NEW,OnMenuitemNew)
	ON_BN_CLICKED(IDC_BUTTON_LOAD,  OnMenuitemLoad)
	ON_BN_CLICKED(IDC_BUTTON_SAVE2, OnMenuitemSave)
	ON_BN_CLICKED(IDC_BUTTON_CSVLOAD, OnMenuitemCsvLO)
	ON_BN_CLICKED(IDC_BUTTON_CSVSAVE, OnMenuitemCsvSA)
	ON_BN_CLICKED(IDC_BUTTON_TEXTIMPORT, OnMenuitemTextImport)
	ON_BN_CLICKED(IDC_BUTTON_TEXTEXPORT, OnMenuitemTextExport)
	ON_BN_CLICKED(IDC_BUTTON_CNV_ALL, OnBnClickedButtonCnvAll)
END_MESSAGE_MAP()
...
void CQuestEditDlg::OnBnClickedButtonCnvAll()
{
	if ( MessageBox ( "Convert All Quest to ALL Class?", "Question", MB_YESNO ) == IDYES )
	{
		if ( m_pSheetTab->SetAll() )
		{
			MessageBox ("Success Converting Quests to ALL type",":P",MB_OK );
		}
	}
}
