...
BEGIN_MESSAGE_MAP(CSkillEditorDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_BUTTON_LOADSKILL2, OnBnClickedButtonLoadskill2)
END_MESSAGE_MAP()
...
void CSkillEditorDlg::OnBnClickedButtonLoadskill()
{
	...
	if ( dlg.DoModal() == IDOK )
	{
		...
		HRESULT hr;
		hr = GLSkillMan::GetInstance().LoadFile ( dlg.GetFileName().GetString(), TRUE, FALSE );
...
void CSkillEditorDlg::OnBnClickedButtonLoadskill2()
{
	CString szFilter = "Skill Setting File (*.ssf)|*.ssf|";
	...
	if ( dlg.DoModal() == IDOK )
	{
		...
		HRESULT hr;
		hr = GLSkillMan::GetInstance().LoadFile ( dlg.GetFileName().GetString(), TRUE, TRUE );
