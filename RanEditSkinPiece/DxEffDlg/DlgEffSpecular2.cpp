// DlgEffSpecular2.cpp : implementation file
//

#include "stdafx.h"
#include "../RanEditSkinPiece.h"
#include "DlgEffSpecular2.h"
...
IMPLEMENT_DYNAMIC(CDlgEffSpecular2, CPropertyPage)
CDlgEffSpecular2::CDlgEffSpecular2( LOGFONT logfont )
	: CPropertyPage(CDlgEffSpecular2::IDD)
	, m_pFont( NULL )
	, m_pPiece( NULL )
	, bNEWEFF( FALSE )
	, m_nMatEdit( -1 )
...
CDlgEffSpecular2::~CDlgEffSpecular2()
{
}

void CDlgEffSpecular2::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CDlgEffSpecular2, CPropertyPage)
...
// CDlgEffSpecular2 message handlers
BOOL CDlgEffSpecular2::OnInitDialog()
{
	...
	SetWin_Combo_Init( this, IDC_COMBO_COLOR_OP2, strCOLOROP, 3 );

	return TRUE;
}

void CDlgEffSpecular2::SetData( DxSkinPiece* pData, DxEffCharSpecular2* pEff, BOOL bNEW )
{
	if ( pData ) 
	{
		m_pPiece = NULL;
...
void CDlgEffSpecular2::ShowData()
{
	if ( m_pPiece && m_pEff )
	{
		EFFCHAR_PROPERTY_SPECULAR2* pProperty = (EFFCHAR_PROPERTY_SPECULAR2*) m_pEff->GetProperty();
		if ( pProperty )
		{
			...
			SetWin_Combo_Sel( this, IDC_COMBO_COLOR_OP2, pProperty->m_dwColorOP - 4 );
			SetWin_Check( this, IDC_CHECK_CUBEMAP, pProperty->m_dwFlag&EFFCHARSPEC2_CUBE );
...
void CDlgEffSpecular2::ShowMaterialList()
{
	if ( m_pPiece && m_pEff )
	{
		EFFCHAR_PROPERTY_SPECULAR2* pProperty = (EFFCHAR_PROPERTY_SPECULAR2*) m_pEff->GetProperty();
		...
		if ( pProperty&&pMAT )
		{
			...
			SetWin_Enable( this, IDC_EDIT_SPECULAR_POWER, FALSE );
...
BOOL CDlgEffSpecular2::SaveData()
{
	if ( m_pPiece && m_pEff )
	{
		EFFCHAR_PROPERTY_SPECULAR2 Property;
		...
		Property.m_dwColorOP = GetWin_Combo_Sel( this, IDC_COMBO_COLOR_OP2) +4 ;
		SetCheck_Flags ( GetWin_Check ( this, IDC_CHECK_CUBEMAP), Property.m_dwFlag, EFFCHARSPEC2_CUBE );
...
void CDlgEffSpecular2::OnBnClickedButtonDlgeffApply()
{
	if ( m_pPiece && m_pEff )
	{
		if ( SaveData() )
...
void CDlgEffSpecular2::OnBnClickedButtonDlgeffSave()
{
	if ( SaveData() )
	{
		m_pPiece = NULL;
...
void CDlgEffSpecular2::OnBnClickedButtonDlgeffCancel()
{
	if ( m_pToolTab )
	{
		m_pPiece = NULL;
...
void CDlgEffSpecular2::OnNMDblclkListSpecular(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	*pResult = 0;
	...
	if ( m_pPiece && m_pEff )
	{
		...
		if ( pMAT )
		{
			...
			SetWin_Enable( this, IDC_EDIT_SPECULAR_POWER, TRUE );
			...
			SetWin_Num_float( this, IDC_EDIT_SPECULAR_POWER, pMAT[nSelect].d3dMaterial.Power );
		}
	}
}

void CDlgEffSpecular2::OnBnClickedButtonSpecular()
{
...
void CDlgEffSpecular2::OnBnClickedButtonSpecularOk()
{
	...
	if ( m_pPiece && m_pEff )
	{
		...
		if ( pMAT )
		{
			...
			pMAT[m_nMatEdit].d3dMaterial.Power = GetWin_Num_float( this, IDC_EDIT_SPECULAR_POWER );
...
void CDlgEffSpecular2::OnBnClickedButtonColor()
{
