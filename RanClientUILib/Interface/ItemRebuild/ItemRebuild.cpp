			...
			for(int i = 0; i < 4; i++)
			{
				...
				if( (EMR_OPT_NULL < nRandOptType) && (nRandOptType < EMR_OPT_SIZE) )
				{
					...
					if( fVal[i] != 0.0f )
					{
						...
						m_pOption[i]->ClearText();
						m_pOption[i]->SetTextTitle( strDescText, NS_UITEXTCOLOR::LITEBLUE );
		...
		if( CHECK_MOUSE_IN( dwMsg ) )
			{
				...
				//this part will show the information of the item..
				SITEMCUSTOM sItemCustom = GLGaeaClient::GetInstance().GetCharacter()->GET_REBUILD_ITEM();
				if( sItemCustom.sNativeID != NATIVEID_NULL() )
	...
	case ITEM_REBUILD_ITEM_OPTION03:
		{
			if ( CHECK_MOUSEIN_LBUPLIKE( dwMsg ) )
			{
				...
				if ( wType == 1 ) {
					...
					if ( m_nSelOption[0] != -1 && m_nSelOption[1] != -1 && m_nSelOption[2] != -1 )
						bOk = true;
				}
				...
				if ( wType == 3 )
				{
					...
					m_SealData03.emOpt = emOpt[m_nSelOption[2]];
					m_SealData03.fValue = fVal[m_nSelOption[2]];
					m_SealData03.nOption = 2;
				}
