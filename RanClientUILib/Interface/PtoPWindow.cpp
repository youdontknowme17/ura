...
//	CreateCloseButton ( "PTOPMENU_WINDOW_CLOSE", "PTOPMENU_WINDOW_CLOSE_F", "PTOPMENU_WINDOW_CLOSE_OVER" );
	...
	CreateFlipButton ( "PTOPMENU_VIEW_PROPERTIES", "PTOPMENU_VIEW_PROPERTIES_F", "PTOPMENU_VIEW_PROPERTIES_OVER",	PTOPMENU_VIEW_PROPERTIES );
	...
	case PTOPMENU_VIEW_PROPERTIES:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )
			{
				...
				if ( dwMsg & UIMSG_LB_UP )
				{
					...
					if ( !pCharClient )	return;
					{
						GLMSG::SNET_VIEW_PROPERTIES NetMsg;
						NetMsg.dwTargetID = m_dwGaeaID;
						NETSEND ( (NET_MSG_GENERIC*) &NetMsg );
					}
				}
