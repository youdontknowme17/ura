...
void CSummonWindow::SetVisibleSingle ( BOOL bVisible )
{
	...
	if( bVisible )
	{
		...
		if( pSummonData )
		{
			...
			strTemp.Format( "%d - %d", pSummonData->m_sCrowAttack[0].sDamage.dwLow, pSummonData->m_sCrowAttack[0].sDamage.dwHigh );
			...
			GLPWDATA dwHP = GLGaeaClient::GetInstance().GetSummonClient()->GetHp(); //original gldwdata
