...
CConftModalWindow::CConftModalWindow () :
	...
	m_pYES ( NULL ),
	m_pNO ( NULL ),
	m_pEditVal ( NULL ),//add duel bet
	m_pTextBox ( NULL ),
	m_pBetValue ( NULL ),//add duel bet
	m_RollOverID ( NO_ID ),
	m_bReqModal ( FALSE ),
	bNowEdit(false)
{
...
void CConftModalWindow::CreateSubControl ()
{
	...
	m_pEditVal = CreateTextButton ( "CONFT_MODAL_VAL_EDIT", MODAL_CONFT_BET_VAL_EDIT, (char*)ID2GAMEWORD ( "MODAL_BUTTON", 4 ) );
	...
	pTextBox = CreateStaticControl ( "CONFT_MODAL_BET_INFO_STATIC", pFont8, NS_UITEXTCOLOR::DEFAULT, TEXT_ALIGN_LEFT );
	pTextBox->AddText ( (char*)ID2GAMEWORD ( "CONFT_MODAL_CONDITION", 3 ) );	
	...
	m_pComboBoxBetType = new CBasicComboBox;
	m_pComboBoxBetType->CreateSub ( this, "BASIC_COMBOBOX", UI_FLAG_XSIZE, CONFT_MODAL_BET_COMBO_OPEN );
	m_pComboBoxBetType->CreateBaseComboBox ( "CONFT_MODAL_BET_COMBO_OPEN" );			
	RegisterControl ( m_pComboBoxBetType );
	...
	m_pComboBoxBetTypeRollOver = new CBasicComboBoxRollOver;
	m_pComboBoxBetTypeRollOver->CreateSub ( this, "BASIC_COMBOBOX_ROLLOVER", UI_FLAG_XSIZE | UI_FLAG_YSIZE, CONFT_MODAL_BET_COMBO_ROLLOVER );
	m_pComboBoxBetTypeRollOver->CreateBaseComboBoxRollOver ( "CONFT_MODAL_BET_COMBO_ROLLOVER" );
	m_pComboBoxBetTypeRollOver->SetVisibleSingle ( FALSE );
	RegisterControl ( m_pComboBoxBetTypeRollOver );

	 //add duel bet
	CBasicLineBox* pBasicLineBox = new CBasicLineBox;
	pBasicLineBox->CreateSub( this, "BASIC_LINE_BOX_EDIT", UI_FLAG_XSIZE | UI_FLAG_YSIZE );
	pBasicLineBox->CreateBaseBoxEditBox( "CONFT_BET_EDIT_BACK" );
	RegisterControl( pBasicLineBox );

	CUIEditBox* pEditBox = new CUIEditBox;
	pEditBox->CreateSub ( this, "CONFT_BET_EDIT", UI_FLAG_XSIZE | UI_FLAG_RIGHT, CONFT_MODAL_BET_EDIT );
	pEditBox->CreateCarrat ( "MODAL_CARRAT", TRUE, UINT_MAX );
	pEditBox->SetLimitInput ( 12 );
	pEditBox->SetFont ( pFont8 );
...
void CConftModalWindow::DoModal ( CString strText, UIGUID CallerID, CString strMyName, CString strHisName )
{
	...
	else
	{
		...
		m_pEditVal->SetVisibleSingle ( FALSE );	
	}
...
void	CConftModalWindow::LoadComboData ()
{
	...
	{
		m_pComboBoxBetTypeRollOver->ClearText ();
		for ( int i = 0; i < EMCONFT_BET_SIZE; i++ )
		{
			strTemp.Format ( "%s",  COMMENT::CONFT_BET_TYPE[i].c_str() );
			m_pComboBoxBetTypeRollOver->AddText ( strTemp );
		}
	}
}
...
void	CConftModalWindow::LoadDefaultCondition ()
{
	...
	LoadHPStateHis ();
	LoadRangeLimit ();
	LoadBetValue ();//add duel bet
	LoadBetType (); //add duel bet
}
...
void	CConftModalWindow::LoadBetType ()
{
	CString strTemp;

	if ( m_bReqModal )
	{			
		strTemp.Format ( "%s", COMMENT::CONFT_BET_TYPE[ RANPARAM::wBetType ].c_str() );
	}
	else
	{		
		SCONFT_OPTION* pConftOption = CInnerInterface::GetInstance().GetConftOption ();
		strTemp.Format ( "%s", COMMENT::CONFT_BET_TYPE[ pConftOption->emBet ].c_str() );
	}

	m_pComboBoxBetType->SetText ( strTemp );
}

void	CConftModalWindow::LoadBetValue ()
{
	CString strCombine;

	if ( m_bReqModal )
	{	
		ULONGLONG	lldefault = 0;
		strCombine.Format ( "%I64u", lldefault );
		m_pBetValue->SetEditString(strCombine);
	}
	else
	{		
		SCONFT_OPTION* pConftOption = CInnerInterface::GetInstance().GetConftOption ();	
		strCombine.Format ( "%I64u", pConftOption->llBet );
	}
...
const ULONGLONG CConftModalWindow::GetDuelBet()//add duel bet
{	
	const CString& strTemp = m_pBetValue->GetEditString();
