...
private: //	등록되는 모든 컨트롤
	...
	CAutoPilotWindow*			m_pAutoPilotWindow;				//  AutoPilotWindow
	CTopUpWindow*				m_pTopUpWindow;				
	CGmcMainWindow*				m_pGmcMainWindow;
	CGmcVocWnd*					m_pGmcVocWnd;				
	...
	CBasicVarTextBoxGreen*			m_pInfoDisplayEx;	
	...
	CPointShopWindow*			m_pPointShopWindow;
...
public:
	...
	CTopUpWindow*			GetTopUpWindow()				{ return m_pTopUpWindow; }			
	CGmcMainWindow*			GetGmcMainWindow()				{ return m_pGmcMainWindow; }
	CGmcVocWnd*				GetGmcVocWnd()					{ return m_pGmcVocWnd; }		
	CAutoPilotWindow*		GetAutoPilotWindow()			{ return m_pAutoPilotWindow; }			//  AutoPilotWindow
	...
	CItemRebuild* 		GetItemRebuild() 	{ return m_pItemRebuildWindow; 	} //sealed card 
public:
	void	CloseAllWindow ();
...
public:	
	void	SetItemMallTopUpWindowClose();
	void	SetItemMallTopUpWindowOpen();
	void	SetGmcMainWindowClose();
	void	SetGmcMainWindowOpen();
	void	SetGmcVocWndClose();		
	void	SetGmcVocWndOpen();			
	/////////////////////////////////////////////////
	//  AutoPilotWindow
	void	SetAutoPilotWindowOpen();
	void	SetAutoPilotWindowClose();
	BOOL	m_bAutoSkill;
	BOOL	IsUsingAutoSkillOn()		{ return m_bAutoSkill; }
	void	SetIsUsingAutoSkillOn()		{ m_bAutoSkill = TRUE; }
	void	SetIsUsingAutoSkillOff()	{ m_bAutoSkill = FALSE; }
	/////////////////////////////////////////////////
	...
	void	OpenPointShopWindow( DWORD dwNpcID );		// 아이템 조합
	void	ClosePointShopWindow();
	void	SetBuyItem( CString strMsg, bool bSuccess = false, bool bFail = false );
...
public:
	...
	//add sw buff
	void    SetSwBonus ( bool bBuff, bool bBonus );

public:
	bool	SET_QUESTION_ITEM_ID ( int nID );
	bool	SET_PLAYERKILL_STREAK_ID ( int nID );	///add pk streak
	bool    SET_TYRANNY_NOTICE_ID ( int nID ); // add tyranny notice
...
public:
	...
	void	TyrannyWarWinnerRefresh();
	BOOL	TyrannyWinnerRankOpen();
	
	void	UpdateTyrannyWarNextBattle( float fProgress, float fNextBattleStart, float fNextBattleEnd, float fPreviousBattleStart, float fPreviousBattleEnd );
	void ADDITEM_RENDER ( SNATIVEID sICONINDEX, const char* szTexture ); 
	void RESETITEM_RENDER (); 
	void	SetItemImage ( SNATIVEID sICONINDEX, const char* szTexture, SITEMCUSTOM sCustom );
	void	ReSetItemImage ();
	void	SetItemName ( const CString& strText, const D3DCOLOR& dwColor );
