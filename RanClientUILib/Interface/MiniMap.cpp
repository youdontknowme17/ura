...
CMiniMap::CMiniMap()
	...
	, m_pMapWindowButton ( NULL )
	, m_pAmmoText(NULL)
	, m_pCenterPoint(NULL)
	, m_pClubTimeText(NULL)
	, m_pRoyalRumbleTimeText(NULL)
	, m_pMiniMapBackground(NULL)
{
}
...
/*	CUIControl* pBack = new CUIControl;
	...
	CUIControl* pMiniMapBackground = new CUIControl;
	pMiniMapBackground->CreateSub ( this, "CNDEV_MINIMAP_BACKGROUND", UI_FLAG_RIGHT );
	pMiniMapBackground->SetVisibleSingle ( TRUE );
	RegisterControl ( pMiniMapBackground );
	m_pMiniMapBackground = pMiniMapBackground;

	CUIControl * pMapWindow = new CUIControl;
	pMapWindow->CreateSub ( this, "CNDEV_MINIMAP_MAP", UI_FLAG_RIGHT, CNDEV_MAPWINDOW );
	pMapWindow->SetVisibleSingle ( TRUE );
	RegisterControl ( pMapWindow );
	m_pMapWindowButton = pMapWindow;
...
void CMiniMap::TranslateUIMessage ( UIGUID ControlID, DWORD dwMsg )
{
	...
	case MINIMAP_FULLSCREEN:
		...
		break;
	case CNDEV_MAPWINDOW:
		{
			if ( CHECK_MOUSEIN_LBUPLIKE ( dwMsg ) )
			{
			//if ( CInnerInterface::GetInstance().IsVisibleGroup( MINIMAP_WINDOW ) )		CInnerInterface::GetInstance().HideGroup ( MINIMAP_WINDOW );
			//else	CInnerInterface::GetInstance().ShowGroupFocus( MINIMAP_WINDOW );
			}
		}break;
	}
}
...
void CMiniMap::UpdateClubTime2( float fCLUBBATTLETime2 )
{
	CString strCombine;
	strCombine.Format ( "%s %02d:%02d", ID2GAMEWORD( "CLUB_BATTLE_TIME", 1 ), (DWORD)fCLUBBATTLETime2/60, (DWORD)fCLUBBATTLETime2%60 );
	m_pClubTimeText->SetOneLineText( strCombine, NS_UITEXTCOLOR::RED );
}
...
//	RotateMap ();
	...
	if ( GLGaeaClient::GetInstance().GetCharacter()->VALID_SLOT_ITEM(emLHand) )
	{
		...
		{
			int nType = (emItemType == ITEM_ARROW)?0:2;
			...
			if ( sItemCustom.wTurnNum != m_wAmmoCount )
			{
				...
				strCombine.Format ( "%s : %d", ID2GAMEWORD("ARROW_TYPE",0), sItemCustom.wTurnNum );
				if ( emItemType == ITEM_CHARM )
				strCombine.Format ( "%s : %d", ID2GAMEWORD("ARROW_TYPE",1), sItemCustom.wTurnNum );
				if ( emItemType == ITEM_BULLET )
				strCombine.Format ( "%s : %d", ID2GAMEWORD("ARROW_TYPE",2), sItemCustom.wTurnNum );



                DWORD ArrowColor = NS_UITEXTCOLOR::WHITE;
				if ( sItemCustom.wTurnNum < 100 ) ArrowColor = NS_UITEXTCOLOR::RED;
				else if( sItemCustom.wTurnNum < 300 ) ArrowColor = NS_UITEXTCOLOR::YELLOW;
