...
void CBasicVideoOption::CreateSubControl ()
{
	...
	{	//	기본 텍스트		
		...
		pTextBox = CreateStaticControl ( "HWOPTION_VIDEO_ITEMDROP_EFFECT_STATIC",m_pFont, dwFontColor, nAlign );
		pTextBox->SetOneLineText ( ID2GAMEWORD ( "HWOPTION_VIDEO_OPTION", 14 ) );

		pTextBox = CreateStaticControl ( "HWOPTION_VIDEO_MAX_GAMEZOOM_STATIC",m_pFont, dwFontColor, nAlign );
	...
	{	//	기능
		...
		//	콤보박스
		{
			...
			pComboBox = new CBasicComboBox;
			pComboBox->CreateSub ( this, "BASIC_COMBOBOX", UI_FLAG_XSIZE, HWOPTION_VIDEO_MAX_GAMEZOOM_COMBO_OPEN );
			pComboBox->CreateBaseComboBox ( "HWOPTION_VIDEO_MAX_GAMEZOOM_COMBO_OPEN" );			
            RegisterControl ( pComboBox );
			m_pComboBoxGameZoomOpen = pComboBox;
		}
		...
		//	버튼
		{
			...
			m_pTargetEffectButton = CreateFlipButton ( "HWOPTION_VIDEO_TARGET_EFEFCT_BUTTON", "HWOPTION_VIDEO_TARGET_EFEFCT_BUTTON_F", HWOPTION_VIDEO_TARGET_EFEFCT_BUTTON );
			m_pItemDropEffectButton = CreateFlipButton ( "HWOPTION_VIDEO_ITEMDROP_EFFECT_BUTTON", "HWOPTION_VIDEO_ITEMDROP_EFFECT_BUTTON_F", HWOPTION_VIDEO_ITEMDROP_EFFECT_BUTTON );
		}
		...
		//	롤 오버 ( 언제나 가장 마지막에 뿌려져야합니다. )
		{
			...
			pComboBoxRollOver = new CBasicComboBoxRollOver;
			pComboBoxRollOver->CreateSub ( this, "BASIC_COMBOBOX_ROLLOVER", UI_FLAG_XSIZE | UI_FLAG_YSIZE, HWOPTION_VIDEO_MAX_GAMEZOOM_COMBO_ROLLOVER );
			pComboBoxRollOver->CreateBaseComboBoxRollOver ( "HWOPTION_VIDEO_MAX_GAMEZOOM_COMBO_ROLLOVER" );
			pComboBoxRollOver->SetVisibleSingle ( FALSE );
			RegisterControl ( pComboBoxRollOver );
			m_pComboBoxGameZoomRollOver = pComboBoxRollOver;
		}
	}
}
...
void CBasicVideoOption::LoadGameZoom()
{
	m_dwScreenZoom = DXPARAMSET::GetInstance().m_dwScreenZoom;
...
void CBasicVideoOption::LoadItemDropEffectButton()
{
...
{	
	...
	{	//	시야
		m_pComboBoxGameZoomRollOver->ClearText ();
		for ( int i = 0; i < DEFAULT_ZOOM_SIZE; i++ )
		{
			strTemp = ID2GAMEWORD ( "HWOPTION_VIDEO_GAMEZOOM", i );
			m_pComboBoxGameZoomRollOver->AddText ( strTemp );
		}
	}
...
void CBasicVideoOption::SetTextGameZoomCombo( DWORD dwIndex )
{
	if ( 0 <= dwIndex && dwIndex < DEFAULT_ZOOM_SIZE )
	{
		CString strTemp( ID2GAMEWORD ( "HWOPTION_VIDEO_GAMEZOOM", dwIndex ) );
		m_pComboBoxGameZoomOpen->SetText ( strTemp );
	}
	else
	{
		GASSERT ( 0 && "스킨 디테일 범위를 넘어섭니다." );
