...
CCtfRankingPage* CCtfRankingDisplay::CreateRankPage( UIGUID ControlID, bool bMyRank )
{
	CCtfRankingPage* pRankPage = new CCtfRankingPage;
	if(!bMyRank) 
	{
		pRankPage->CreateSub ( this, "CTF_RESULT_RANKING", UI_FLAG_DEFAULT, ControlID );
		pRankPage->CreateSubControl ( 0, true );
	}
	else
	{
		pRankPage->CreateSub ( this, "CTF_RESULT_MYRANKING", UI_FLAG_DEFAULT, ControlID );
...
void CCtfRankingDisplay::IsOpenWindowDefault()
{
	m_pCtfRankingPage[3]->RefreshRankSlot( false,3 );
	m_pCtfRankingPage[3]->SetVisibleSingle(TRUE);
	SetVisiblePage(3);
}
