...
char * szHAIRSTYLE_NAME_M  = "2012_HAIR_NAME";
char * szHAIRSTYLE_NAME_FM = "2012_HAIR_NAME";
char * szFACESTYLE_NAME_M  = "2012_FACE_NAME";
char * szFACESTYLE_NAME_FM = "2012_FACE_NAME";
...
CBasicTextButton*	 CGenderChangeWindow::CreateTextButton ( char* szButton, UIGUID ControlID, char* szText )
{
...
void CGenderChangeWindow::CreateSubControl ()
{
	...
	{ // Note : 버튼
		CreateTextButton ( "GENDER_CHANGE_OK", GENDER_CHANGE_OK, (char*)ID2GAMEWORD( "MODAL_BUTTON", 0 ) );
		CreateTextButton ( "GENDER_CHANGE_CANCEL", GENDER_CHANGE_CANCEL, (char*)ID2GAMEWORD( "MODAL_BUTTON", 1 ) );
	...
	case FACESTYLE_LEFT:
		{
			if( CHECK_MOUSEIN_LBUPLIKE( dwMsg ) )
			{
				...
				m_pFaceStyleName->SetText( ID2GAMEWORD( m_pFaceId2GameText, m_nFace ) );
			}
		}
		break;
	...
	case HAIRSTYLE_LEFT:
		{
			if( CHECK_MOUSEIN_LBUPLIKE( dwMsg ) )
			{
				...
				m_pHairStyleName->SetText( ID2GAMEWORD( m_pHairId2GameText, m_nHair ) );
...
void CGenderChangeWindow::SetVisibleSingle ( BOOL bVisible )
{
	...
	if( bVisible )
	{
		...
		m_pFaceStyleName->AddText( ID2GAMEWORD( m_pFaceId2GameText, m_nFace ) );
		m_pFaceStyle->SetVisibleSingle( TRUE );

		m_pHairStyleName->AddText( ID2GAMEWORD( m_pHairId2GameText, m_nHair ) );
		m_pHairStyle->SetVisibleSingle( TRUE );
...
void CGenderChangeWindow::CalcTextureFace()
{
	...
	rcTexPos.top = rcTexPos.sizeY * ( m_nSex * 4 );
	rcTexPos.top += rcTexPos.sizeY * ( m_nFace / 16 );
	rcTexPos.bottom = rcTexPos.top + rcTexPos.sizeY;	
	rcTexPos.left =  rcTexPos.sizeX * ( m_nFace % 16 );
...
void CGenderChangeWindow::CalcTextureHair()
{
	...
	rcTexPos.top = rcTexPos.sizeY * ( nSex * 4 );
	rcTexPos.top += rcTexPos.sizeY * ( nHair / 16 );
	rcTexPos.bottom = rcTexPos.top + rcTexPos.sizeY;	
	rcTexPos.left =  rcTexPos.sizeX * ( nHair % 16 );
	rcTexPos.right = rcTexPos.left + rcTexPos.sizeX;	
