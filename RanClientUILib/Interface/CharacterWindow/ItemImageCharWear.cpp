...
{	
    ...
    //m_pItemImage = ref_sItemCustom;    
 // Add Colorazation Per Ups  Kun :D 
	if ( m_pTextBox && nGrind >= 1 && nGrind <= 4 ){
		CString strGrind;
		strGrind.Format( "+%d", nGrind );
		m_pTextBox->SetOneLineText( strGrind.GetString(),NS_UITEXTCOLOR::GOLD );
	}else if ( m_pTextBox && nGrind >= 5 && nGrind <= 9 ){
		CString strGrind;
		strGrind.Format( "+%d", nGrind );
		m_pTextBox->SetOneLineText( strGrind.GetString(),NS_UITEXTCOLOR::PALEGOLDENROD );
	}else if ( m_pTextBox && nGrind >= 10 && nGrind <= 11 ){
		CString strGrind;
		strGrind.Format( "+%d", nGrind );
		m_pTextBox->SetOneLineText( strGrind.GetString(),NS_UITEXTCOLOR::JERMTHESLAYER );
