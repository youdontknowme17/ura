...
void CCharacterWindowChar::SetData ( const GLCHARLOGIC& sCharData )
{
	...
	strCombine.Format ( "%d", sCharData.m_dwPkWin );
	m_pKillP->SetOneLineText ( strCombine, NS_UITEXTCOLOR::WHITE );
	strCombine.Format ( "%d", sCharData.m_dwPkLoss );
	m_pDeathP->SetOneLineText ( strCombine, NS_UITEXTCOLOR::WHITE );
