...
HRESULT CCharacterWindowCharRender::FrameMove( LPDIRECT3DDEVICEQ pd3dDevice, float fTime, float fElapsedTime )
{
	...
	if ( !bPeaceZone && GLGaeaClient::GetInstance().GetCharacter()->IsSTATE(EM_ACT_PEACEMODE) )		bPeaceZone = TRUE;

	if ( m_pSkinChar )
	{
		m_pSkinChar->FrameMove ( fTime, fElapsedTime, TRUE, FALSE, !bPeaceZone );
		...
		case ITEMATT_WHIP:
			m_pSkinChar->SELECTANI( AN_GUARD_N, AN_SUB_WHIP );
			break;

		case ITEMATT_CUBE:
			m_pSkinChar->SELECTANI( AN_GUARD_N, AN_SUB_BOX );
			break;
		};
	}
		...
		else
		{
			...
			if ( pItem ) 
			{
				...
				{//add itemcolor
					...
					{
						m_pSkinChar->SetColor( PIECE_RHAND , 1 );
						...
						if ( pRHAND )
						{
							...
							{
								m_pSkinChar->SetColor( PIECE_LHAND , 1 );
							}
						}	
					}
				}
