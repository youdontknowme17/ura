		...
		if ( CHECK_MOUSE_IN ( dwMsg ) )
		{
			...
			if ( dwMsg & UIMSG_LB_UP || dwMsg & UIMSG_RB_UP )
			{
				...
				if ( sNativeID != NATIVEID_NULL () )
				{
					...
					if ( pItemData )
					{
						if ( pItemData->sBasicOp.IsPointRecover() )
						{
							if ( pItemData->sBasicOp.emLevel == LEVEL_VOTE_ITEM )
							{
								...
								CString strCombine = CInnerInterface::GetInstance().MakeString ( ID2GAMEINTEXT("SELL_ITEM_CONFIRM_VOTE"), strItemName.c_str() );
								DoModal ( strCombine, MODAL_QUESTION, YESNO, MODAL_SELL_ITEM );
							}
							else if ( pItemData->sBasicOp.emLevel == LEVEL_PREMIUM_ITEM )
							{
								const std::string& strItemName = pItemData->GetName();
								CString strCombine = CInnerInterface::GetInstance().MakeString ( ID2GAMEINTEXT("SELL_ITEM_CONFIRM_PREMIUM"), strItemName.c_str() );
								DoModal ( strCombine, MODAL_QUESTION, YESNO, MODAL_SELL_ITEM );
							}
						}
						else
					...
					{
						GASSERT ( 0 && "Item to sell is not AN ITEM!." );
					}
...
//			if ( CUIMan::GetFocusControl () == this )
			...
			if ( DxInputDevice::GetInstance().GetKeyState ( DIK_LCONTROL ) & DXKEY_PRESSED )
			{
				...
				//add money 2
				//add confirmation if item is premium to avoid wrong purchase
				...
				if ( dwMsg & UIMSG_LB_UP )
				{
					...
					if ( pITEM )
					{
						if( pITEM->sBasicOp.emLevel == LEVEL_VOTE_ITEM)
						{
							...
							CString strCombine = CInnerInterface::GetInstance().MakeString ( ID2GAMEINTEXT("BUY_ITEM_CONFIRM_VOTE"), strItemName.c_str() );
						...
						{
							const std::string& strItemName = pITEM->GetName();
							CString strCombine = CInnerInterface::GetInstance().MakeString ( ID2GAMEINTEXT("BUY_ITEM_CONFIRM_PREMIUM"), strItemName.c_str() );
							DoModal ( strCombine, MODAL_QUESTION, YESNO, MODAL_BUY_ITEM );							
							return ;
						}
						...
						{
							const std::string& strItemName = pITEM->GetName();
							CString strCombine = CInnerInterface::GetInstance().MakeString ( ID2GAMEINTEXT("BUY_ITEM_CONFIRM_CONTRIBUTION"), strItemName.c_str() );
							DoModal ( strCombine, MODAL_QUESTION, YESNO, MODAL_BUY_ITEM );
							return;
						}
						else
			...
			if ( dwMsg & UIMSG_LB_UP )
			{
				...
				if ( pITEM )
				{
					...
					if( pITEM->ISPILE() && (pITEM->GETAPPLYNUM()==1) )
					{
						...
						return ;*/
						//add money 2
						//warming for purchase
						if( pITEM->sBasicOp.emLevel == LEVEL_VOTE_ITEM)
						{
							DoModal ( ID2GAMEINTEXT ( "BUY_PILE_ITEM_VOTE" ), MODAL_INPUT, EDITBOX_NUMBER, MODAL_PILE_ITEM );
						return ;
						}
						//add money 3
						else if( pITEM->sBasicOp.emLevel == LEVEL_PREMIUM_ITEM)
						{
							DoModal ( ID2GAMEINTEXT ( "BUY_PILE_ITEM_PREMIUM" ), MODAL_INPUT, EDITBOX_NUMBER, MODAL_PILE_ITEM );
						return ;
						}
						//add contributionpoint by CNDev
						else if( pITEM->sBasicOp.emLevel == LEVEL_CONTRIBUTION_ITEM)
						{
							DoModal ( ID2GAMEINTEXT ( "BUY_PILE_ITEM_CONTRIBUTION" ), MODAL_INPUT, EDITBOX_NUMBER, MODAL_PILE_ITEM );
							return ;
						}
						else
						{
