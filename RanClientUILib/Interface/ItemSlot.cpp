		...
		for ( int i = 0; i < m_nMaxColumn; i++ )
		{
			...
			{	
				...
				if ( pItemData )
				{
					...
					const WORD wAttackNum = m_InvenItem[i].sItemCustom.cDAMAGE;
					const WORD wDefenseNum = m_InvenItem[i].sItemCustom.cDEFENSE;
					...
					if ( wAttackNum>1 )
					{
						SetNumber2 ( i,  wAttackNum );
					}
					if ( wDefenseNum>1 )
					{
						SetNumber3 ( i, wDefenseNum );
					}
