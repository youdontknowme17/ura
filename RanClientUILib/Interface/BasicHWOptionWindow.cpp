	...
	case HWOPTION_VIDEO:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )
			{
				...
				CInnerInterface::GetInstance().SHOW_COMMON_LINEINFO (  ID2GAMEWORD ( "HWOPTION_AV_NAME_DESC", 0 ), NS_UITEXTCOLOR::DEFAULT );
			}
		}
		break;
	...
	case HWOPTION_AUDIO:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )
			{
				...
				CInnerInterface::GetInstance().SHOW_COMMON_LINEINFO (  ID2GAMEWORD ( "HWOPTION_AV_NAME_DESC", 1 ), NS_UITEXTCOLOR::DEFAULT );
			}
		}
		break;
	...
	case HWOPTION_GAMEPLAY:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )
			{
				...
				CInnerInterface::GetInstance().SHOW_COMMON_LINEINFO (  ID2GAMEWORD ( "HWOPTION_AV_NAME_DESC", 2 ), NS_UITEXTCOLOR::DEFAULT );
			}
		}
		break;
...
void CBasicHWOptionWindow::SetVisibleSingle ( BOOL bVisible )
{
	...
	if( bVisible )
	{
		...
		if( m_bFirstSetting )
		{
			GLGaeaClient::GetInstance().GetCharacter()->ReqRankName( RANPARAM::bShowRankName ); //add pkrank
			GLGaeaClient::GetInstance().GetCharacter()->ReqRankMark( RANPARAM::bShowRankMark); //add pkrank
...
void CBasicHWOptionWindow::GamePlayOption_OK()
{
	...
	RANPARAM::bShowRankName		= m_pGamePlayOption->m_bRankName; //add pkrank
	RANPARAM::bShowRankMark		= m_pGamePlayOption->m_bRankMark; //add pkrank
...
void CBasicHWOptionWindow::VideoOption_OK()
{
	...
	RANPARAM::bClickEffect		= m_pVideoOption->m_bClickEffect;
	RANPARAM::bTargetEffect		= m_pVideoOption->m_bTargetEffect;
	RANPARAM::bMineEffect		= m_pVideoOption->m_bMineEffect;
	DXPARAMSET::GetInstance().m_dwScreenZoom		= m_pVideoOption->m_dwScreenZoom;
