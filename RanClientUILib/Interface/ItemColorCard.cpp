...
const int CItemColorCard::nLIMIT_NUMBER_CHAR = 2;
const int CItemColorCard::nMAX_COLOR = 31;
const float CItemColorCard::fRGB_RATIO = 8.225806f;

CItemColorCard::CItemColorCard ()
...
CBasicTextBox* CItemColorCard::CreateStaticControl( char* szControlKeyword, CD3DFontPar* pFont, int nAlign, const UIGUID& cID )
{
...
void CItemColorCard::CreateSubControl ()
{
	...
	{	// Note : 프로그래스바
		...
		pDummyBar->CreateSub ( this, "HAIRCOLORCARD_R_PROGRESS_DUMMY", UI_FLAG_DEFAULT, BIKECOLORCARD_R_PROGRESS );
		...
		pDummyBar = new CUIControl;
		pDummyBar->CreateSub ( this, "HAIRCOLORCARD_G_PROGRESS_DUMMY", UI_FLAG_DEFAULT, BIKECOLORCARD_G_PROGRESS );
		RegisterControl ( pDummyBar );
		...
		pDummyBar = new CUIControl;
		pDummyBar->CreateSub ( this, "HAIRCOLORCARD_B_PROGRESS_DUMMY", UI_FLAG_DEFAULT, BIKECOLORCARD_B_PROGRESS );
		RegisterControl ( pDummyBar );
	}
	...
	{ // Note : 입력 에디터 박스
		...
		pEditBox->CreateSub( this, "HAIRCOLORCARD_R_EDITBOX", UI_FLAG_XSIZE | UI_FLAG_RIGHT, BIKECOLORCARD_R_EDITBOX );
		...
		pEditBox->CreateSub( this, "HAIRCOLORCARD_G_EDITBOX", UI_FLAG_XSIZE | UI_FLAG_RIGHT, BIKECOLORCARD_G_EDITBOX );
		pEditBox->CreateCarrat( "MODAL_CARRAT", TRUE, UINT_MAX );
		pEditBox->SetLimitInput( nLIMIT_NUMBER_CHAR );
		pEditBox->SetFont( pFont );	
		pEditBox->DoUSE_NUMBER( true );
		...
		pEditBox = new CUIEditBox;
		pEditBox->CreateSub( this, "HAIRCOLORCARD_B_EDITBOX", UI_FLAG_XSIZE | UI_FLAG_RIGHT, BIKECOLORCARD_B_EDITBOX );
		pEditBox->CreateCarrat( "MODAL_CARRAT", TRUE, UINT_MAX );
		pEditBox->SetLimitInput( nLIMIT_NUMBER_CHAR );
		pEditBox->SetFont( pFont );
		pEditBox->DoUSE_NUMBER( true );
	...
	{ // Note : 버튼
		CreateTextButton ( "HAIRCOLORCARD_OK", BIKECOLORCARD_OK, (char*)ID2GAMEWORD( "MODAL_BUTTON", 0 ) );
		CreateTextButton ( "HAIRCOLORCARD_CANCEL", BIKECOLORCARD_CANCEL, (char*)ID2GAMEWORD( "MODAL_BUTTON", 1 ) );
...
CBasicTextButton*	 CItemColorCard::CreateTextButton ( char* szButton, UIGUID ControlID, char* szText )
{
...
BOOL CItemColorCard::UpdateProgressBar( CBasicProgressBar * pBar, int x )
{
...
void CItemColorCard::UpdateProgressBarToEditBox( CBasicProgressBar * pBar, CUIEditBox * pEditBox, INT & nColorVal )
{
	int nVal(0);
...
BOOL CItemColorCard::UpdateEditBoxToProgressBar( CUIEditBox * pEditBox, CBasicProgressBar * pBar, INT & nColorVal )
{
...
void CItemColorCard::Update( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{
	...
	if( bFirstControl )
	{
		...
		if( bChange )
		{
			...
			GLGaeaClient::GetInstance().GetCharacter()->ItemColorChange( wTemp );
		}
	}
}

void CItemColorCard::TranslateProgressBarMsg( CBasicProgressBar * pBar, DWORD dwMsg )
{
	if( !pBar ) return;
...
void CItemColorCard::TranslateEditBoxMsg( CUIEditBox * pEditBox, DWORD dwMsg )
{
	if( !pEditBox ) return;
...
void CItemColorCard::TranslateUIMessage( UIGUID ControlID, DWORD dwMsg )
{
	switch ( ControlID )
	{
...
void CItemColorCard::SetVisibleSingle ( BOOL bVisible )
{
	...
	if( bVisible )
	{
		...
		GLGaeaClient::GetInstance().GetCharacter()->ItemColorInitData();
	}
	else
	{
		if( m_bOKButton )
		{
			WORD wTemp = m_nR << 10	| m_nG << 5 | m_nB;
			GLGaeaClient::GetInstance().GetCharacter()->ReqInvenItemColorChange( wTemp );
...
void CItemColorCard::InitUIControl()
{
	EMSLOT emSlot = GLGaeaClient::GetInstance().GetCharacter()->m_ColorSlot;
	if ( emSlot < SLOT_TSIZE )
	{
		SITEMCUSTOM sItemCustom = GLGaeaClient::GetInstance().GetCharacter()->GET_SLOT_ITEM(emSlot);;
		if ( sItemCustom.sNativeID != NATIVEID_NULL () )
		{
			const WORD & wITEMCOLOR = sItemCustom.wColor;

			m_nRBack = m_nR = (wITEMCOLOR & 0x7c00) >> 10;
			m_nGBack = m_nG = (wITEMCOLOR & 0x3e0) >> 5;
			m_nBBack = m_nB = (wITEMCOLOR & 0x1f);
...
void CItemColorCard::RegulateRGB()
{
...
void CItemColorCard::ClearEditBox()
{
