...
{
	pTextBoxPage->ClearText();
	pTextBoxPage->SetText ( ID2GAMEEXTEXT("2012_NEW_CHAR_TEXT_PAGE"), NS_UITEXTCOLOR::DEFAULT );
	bool bMakeMsg = true;

	if( NEW_LOGIN_PAGE == cID || SECPASS_SETPAGE == cID || SECPASS_CHECKPAGE == cID )
	{
		HideGroup( PASS_KEYBOARD, bMakeMsg );
		HideGroup(SELECT_MENU_PAGE, bMakeMsg);
		HideGroup(NEW_LOGIN_PAGE, bMakeMsg);
		HideGroup(PASS_KEYBOARD, bMakeMsg);
		HideGroup(SELECT_USER_PAGE, bMakeMsg);
		HideGroup(REGISTER_PAGE, bMakeMsg);
		HideGroup(CHANGEPASSWORD_PAGE, bMakeMsg);
		HideGroup(CHANGEPINCODE_PAGE, bMakeMsg);
		HideGroup(FORGOTACCOUNT_PAGE, bMakeMsg);
		HideGroup(FIXUSER_PAGE, bMakeMsg);
	}
...
{
	pTextBoxPage->ClearText();
	pTextBoxPage->SetText ( ID2GAMEEXTEXT("2012_NEW_CHAR_TEXT_PAGE2"), NS_UITEXTCOLOR::DEFAULT );
	bool bMakeMsg = true;
...
{
	pTextBoxPage->ClearText();
	pTextBoxPage->SetText ( ID2GAMEEXTEXT("2012_NEW_CHAR_TEXT_PAGE3"), NS_UITEXTCOLOR::DEFAULT );
	bool bMakeMsg = true;
...
{ 
	//return m_pNewCreateCharPage->m_pBackImage->GetGlobalPos(); //newcharcreate
	return m_pCreateCharacterNew->m_pBackImage->GetGlobalPos();
}

void COuterInterface::ToSelectMenuPage(UIGUID cID)
{
	bool bMakeMsg = true;

	if (m_pSelectMenuPage)
	{
		ShowGroupFocus(SELECT_MENU_PAGE, bMakeMsg);
		HideGroup(SELECT_SERVER_PAGE, bMakeMsg);
		HideGroup(NEW_LOGIN_PAGE, bMakeMsg);
		HideGroup(PASS_KEYBOARD, bMakeMsg);
		HideGroup(SELECT_USER_PAGE, bMakeMsg);
		HideGroup(REGISTER_PAGE, bMakeMsg);
		...
		HideGroup(FIXUSER_PAGE, bMakeMsg);
		HideGroup(SELECT_CHARACTER_PAGE, bMakeMsg);
		HideGroup(CREATE_CHARACTER_PAGE, bMakeMsg);
	}
}

void COuterInterface::ToNewLoginPage(UIGUID cID)
{
	bool bMakeMsg = true;
	...
	if (m_pNewLoginPage)
	{
		...
		ShowGroupFocus(NEW_LOGIN_PAGE, bMakeMsg);
		HideGroup(SELECT_SERVER_PAGE, bMakeMsg);
		HideGroup(SELECT_MENU_PAGE, bMakeMsg);
		HideGroup(SELECT_USER_PAGE, bMakeMsg);
		HideGroup(REGISTER_PAGE, bMakeMsg);
...
void COuterInterface::ToSelectUserPage(UIGUID cID)
{
	bool bMakeMsg = true;

	if (m_pSelectUserPage)
	{
		ShowGroupFocus(SELECT_USER_PAGE, bMakeMsg);
		HideGroup(SELECT_SERVER_PAGE, bMakeMsg);
		HideGroup(SELECT_MENU_PAGE, bMakeMsg);
		HideGroup(NEW_LOGIN_PAGE, bMakeMsg);
		HideGroup(PASS_KEYBOARD, bMakeMsg);		
...
void COuterInterface::ToChangePasswordPage(UIGUID cID)
{
	bool bMakeMsg = true;

	if (m_pChangePasswordPage)
	{
		ShowGroupFocus(CHANGEPASSWORD_PAGE, bMakeMsg);
		HideGroup(SELECT_SERVER_PAGE, bMakeMsg);
		HideGroup(SELECT_MENU_PAGE, bMakeMsg);
		HideGroup(NEW_LOGIN_PAGE, bMakeMsg);
		HideGroup(PASS_KEYBOARD, bMakeMsg);
...
void COuterInterface::ToChangePinCodePage(UIGUID cID)
{
	bool bMakeMsg = true;

	if (m_pChangePinCodePage)
	{
		ShowGroupFocus(CHANGEPINCODE_PAGE, bMakeMsg);
		HideGroup(SELECT_SERVER_PAGE, bMakeMsg);
		HideGroup(SELECT_MENU_PAGE, bMakeMsg);
		HideGroup(NEW_LOGIN_PAGE, bMakeMsg);
		HideGroup(PASS_KEYBOARD, bMakeMsg);
...
void COuterInterface::ToForgotAccountPage(UIGUID cID)
{
	bool bMakeMsg = true;

	if (m_pForgotAccountPage)
	{
		ShowGroupFocus(FORGOTACCOUNT_PAGE, bMakeMsg);
		HideGroup(SELECT_SERVER_PAGE, bMakeMsg);
		HideGroup(SELECT_MENU_PAGE, bMakeMsg);
		HideGroup(NEW_LOGIN_PAGE, bMakeMsg);
		HideGroup(PASS_KEYBOARD, bMakeMsg);
...
void COuterInterface::ToFixUserPage(UIGUID cID)
{
	bool bMakeMsg = true;

	if (m_pFixUserPage)
	{
		ShowGroupFocus(FIXUSER_PAGE, bMakeMsg);
		HideGroup(SELECT_SERVER_PAGE, bMakeMsg);
		HideGroup(SELECT_MENU_PAGE, bMakeMsg);
		HideGroup(NEW_LOGIN_PAGE, bMakeMsg);
		HideGroup(PASS_KEYBOARD, bMakeMsg);
