...
#include "../RanClientLib/G-Logic/GLGaeaServer.h"
...
void CNameDisplayMan::ADD_DISP_NAME ( CROWREN &sDISP_NAME, const TCHAR * szOwnerName )
{
	...
	if ( found!=m_listDISP_NAME.end() )
	{
		...
		if ( pDISP->DIFFERENT(sDISP_NAME) )
		{
			...
			//	Note : 새로 생성.
			{
				...
				if ( sDISP_NAME.m_emCROW == CROW_PC ) //ad pkrank
				{
					pDISP_NEW->SetName ( sDISP_NAME.m_szNAME,sDISP_NAME.m_dwPkWin,true,sDISP_NAME.m_bRankName,sDISP_NAME.m_bRankMark, sDISP_NAME.m_dwCOLOR, sDISP_NAME.m_wSCHOOL, sDISP_NAME.m_emPARTY );
				}
				else
				{
					pDISP_NEW->SetName ( sDISP_NAME.m_szNAME,sDISP_NAME.m_dwPkWin,false,sDISP_NAME.m_bRankName,sDISP_NAME.m_bRankMark, sDISP_NAME.m_dwCOLOR, sDISP_NAME.m_wSCHOOL, sDISP_NAME.m_emPARTY );
				}
				...
				if ( sDISP_NAME.m_emCROW == CROW_MOB )
				{
					PGLCROWCLIENT pCROW = GLGaeaClient::GetInstance().GetActiveMap()->GetCrow ( sDISP_NAME.m_dwID );
					if ( pCROW )
					{
						float fPercent = float(pCROW->GetHp().dwNow) / float(pCROW->GetHp().dwMax);
						DWORD dwHP = float(pCROW->GetHp().dwMax);
					   if (!( dwHP <=3) )
					   {
						if( pDISP_NEW )
							pDISP_NEW->SetHP 
										(	fPercent, 
											pCROW->GetTarID() == GLGaeaClient::GetInstance().GetCharacter()->GetCtrlID()
										);
					   }				
					}
				}
		...
		else
		{
			...
			}else if ( sDISP_NAME.m_emCROW == CROW_ITEM )
				{
					if ( pDISP )
					{
						pDISP->SetHP(0.0f, FALSE);
						pDISP->SetItemLineBox();
					}
				}
			else
			{
	...
	else
	{
		...
		if( sDISP_NAME.m_emCROW == CROW_PC ) //add pkrank
		{
			pDISP_NEW->SetName ( sDISP_NAME.m_szNAME,sDISP_NAME.m_dwPkWin,true,sDISP_NAME.m_bRankName,sDISP_NAME.m_bRankMark, sDISP_NAME.m_dwCOLOR, sDISP_NAME.m_wSCHOOL, sDISP_NAME.m_emPARTY );
		}
		else
...
DWORD CNameDisplayMan::spouseColor( WORD characterSex )
{
	const WORD spouseSex = !characterSex;
	const WORD female = 0;
	return spouseSex == female ? NS_UITEXTCOLOR::YELLOW : NS_UITEXTCOLOR::DODGERBLUE;
}