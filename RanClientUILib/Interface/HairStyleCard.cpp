...
char * szHAIRSTYLECARD_NAME_M = "2012_HAIR_NAME";
char * szHAIRSTYLECARD_NAME_FM = "2012_HAIR_NAME";
...
CBasicTextButton*	 CHairStyleCard::CreateTextButton ( char* szButton, UIGUID ControlID, char* szText )
{
...
void CHairStyleCard::CreateSubControl ()
{
	...
	{ // Note : 버튼
		CreateTextButton ( "HAIRSTYLECARD_OK", HAIRSTYLECARD_OK, (char*)ID2GAMEWORD( "MODAL_BUTTON", 0 ) );
		CreateTextButton ( "HAIRSTYLECARD_CANCEL", HAIRSTYLECARD_CANCEL, (char*)ID2GAMEWORD( "MODAL_BUTTON", 1 ) );
	...
	case HAIRSTYLECARD_LEFT:
		{
			if( CHECK_MOUSEIN_LBUPLIKE( dwMsg ) )
			{
				...
				m_pStyleName->SetText( ID2GAMEWORD( m_pId2GameText, m_nHair ) );				
...
void CHairStyleCard::SetVisibleSingle ( BOOL bVisible )
{
	...
	if( bVisible )
	{
		...
		CalcTextureHair();
		m_pStyleName->AddText( ID2GAMEWORD( m_pId2GameText, m_nHair ) );
		m_pHairStyle->SetVisibleSingle( TRUE );
