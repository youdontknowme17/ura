...
void CPetTimeUnit::CreateSubControl ()
{
...
void CPetTimeUnit::Update ( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{
	if ( !IsVisible () ) return ;

	m_fLIFE_TIME = 1000;
	m_fLEFT_TIME = float( GLGaeaClient::GetInstance().GetPetClient()->m_nFull );
