...
char * szFACESTYLECARD_NAME_M = "2012_FACE_NAME";
char * szFACESTYLECARD_NAME_FM = "2012_FACE_NAME";
...
CBasicTextButton*	 CFaceStyleCard::CreateTextButton ( char* szButton, UIGUID ControlID, char* szText )
{
...
void CFaceStyleCard::CreateSubControl ()
{
	...
	{ // Note : 버튼
		CreateTextButton ( "FACESTYLECARD_OK", FACESTYLECARD_OK, (char*)ID2GAMEWORD( "MODAL_BUTTON", 0 ) );
		CreateTextButton ( "FACESTYLECARD_CANCEL", FACESTYLECARD_CANCEL, (char*)ID2GAMEWORD( "MODAL_BUTTON", 1 ) );
	...
	case FACESTYLECARD_LEFT:
		{
			if( CHECK_MOUSEIN_LBUPLIKE( dwMsg ) )
			{
				...
				m_pStyleName->SetText( ID2GAMEWORD( m_pId2GameText, m_nFace ) );
...
void CFaceStyleCard::SetVisibleSingle ( BOOL bVisible )
{
	...
	if( bVisible )
	{
		...
		m_nFaceBack = m_nFace;
		m_pStyleName->AddText( ID2GAMEWORD( m_pId2GameText, m_nFace ) );
