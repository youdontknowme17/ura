...
CMiniMapInfo::CMiniMapInfo()
	...
	, m_pMinimapPosBox(NULL)
	, m_pClubMark(NULL)
	, m_pMinimapTimeBox(NULL)
{
}
...
void CMiniMapInfo::UpdatePostion ()
{
	...
	if ( CInnerInterface::GetInstance().IsCHANNEL () )
	{
		...
		strText.Format ( "%s", m_strMapName );
		m_pMinimapTextBox->SetOneLineText ( strText, NS_UITEXTCOLOR::ORANGE );

		strText.Format ( "%d/%d" , m_nMiniPosX, m_nMiniPosY );
		m_pMinimapPosBox->SetOneLineText ( strText, NS_UITEXTCOLOR::WHITE );

		if ( m_pMinimapTimeBox ) // Monster7j
		{
			CString strCombine;
			...
			if ( nServerMinute != nServerMinuteBACK )
			{
				...
				m_pMinimapTimeBox->SetOneLineText ( strCombine, NS_UITEXTCOLOR::WHITE );
			}
		}
	}
	else
...
void CMiniMapInfo::CreateSubControl ()
{
	...
	pMinimapTextBox->CreateSub ( this, "CNDEV_MINIMAP_LOCATION" );
	...
	CBasicTextBox* pMiniMapPosBox = new CBasicTextBox;
	pMiniMapPosBox->CreateSub ( this, "CNDEV_MINIMAP_POSITION");
	pMiniMapPosBox->SetFont ( pFont );
	pMiniMapPosBox->SetTextAlign ( TEXT_ALIGN_RIGHT );
	RegisterControl ( pMiniMapPosBox );
	m_pMinimapPosBox = pMiniMapPosBox;

	CBasicTextBox* pMiniMapTimeBox = new CBasicTextBox;
	pMiniMapTimeBox->CreateSub ( this, "CNDEV_MINIMAP_TIME");
	pMiniMapTimeBox->SetFont ( pFont );
	pMiniMapTimeBox->SetTextAlign ( TEXT_ALIGN_RIGHT );
	RegisterControl ( pMiniMapTimeBox );
	m_pMinimapTimeBox = pMiniMapTimeBox;
}