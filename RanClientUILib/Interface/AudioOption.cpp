	...
	case HWOPTION_AUDIO_SFX_PROGRESS:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )		//	컨트롤 내부에 마우스가 위치하고,
			{
				...
				CInnerInterface::GetInstance().SHOW_COMMON_LINEINFO (  ID2GAMEWORD ( "HWOPTION_AUDIO_DESC", 0 ), NS_UITEXTCOLOR::DEFAULT );
			}
	...
	case HWOPTION_AUDIO_ENVIR_PROGRESS:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )		//	컨트롤 내부에 마우스가 위치하고,
			{
				...
				CInnerInterface::GetInstance().SHOW_COMMON_LINEINFO (  ID2GAMEWORD ( "HWOPTION_AUDIO_DESC", 1 ), NS_UITEXTCOLOR::DEFAULT );
			}
			else if ( CHECK_LB_UP_LIKE ( dwMsg ) )		//	컨트롤 영역밖에서 버튼을 떼는 경우가
			{											//	발생하더라도
				m_pEnvirProgressBar->ResetExclusiveControl();		//	단독 컨트롤을 해제하고,
	...
	case HWOPTION_AUDIO_BGM_PROGRESS:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )		//	컨트롤 내부에 마우스가 위치하고,
			{
				...
				CInnerInterface::GetInstance().SHOW_COMMON_LINEINFO (  ID2GAMEWORD ( "HWOPTION_AUDIO_DESC", 2 ), NS_UITEXTCOLOR::DEFAULT );
			}
			else if ( CHECK_LB_UP_LIKE ( dwMsg ) )		//	컨트롤 영역밖에서 버튼을 떼는 경우가
			{											//	발생하더라도
				m_pBgmProgressBar->ResetExclusiveControl();		//	단독 컨트롤을 해제하고,
	...
	case HWOPTION_SFX_BUTTON:
		{
			...
			{
				CInnerInterface::GetInstance().SHOW_COMMON_LINEINFO (  ID2GAMEWORD ( "HWOPTION_AUDIO_DESC", 3 ), NS_UITEXTCOLOR::DEFAULT );
			}
		}
		break;
	...
	case HWOPTION_ENVIR_BUTTON:
		{
			...
			{
				CInnerInterface::GetInstance().SHOW_COMMON_LINEINFO (  ID2GAMEWORD ( "HWOPTION_AUDIO_DESC", 4 ), NS_UITEXTCOLOR::DEFAULT );
			}
		}
		break;
	...
	case HWOPTION_BGM_BUTTON:
		{
			...
			{
				CInnerInterface::GetInstance().SHOW_COMMON_LINEINFO (  ID2GAMEWORD ( "HWOPTION_AUDIO_DESC", 5 ), NS_UITEXTCOLOR::DEFAULT );
			}
		}
		break;
	}
