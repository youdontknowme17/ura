...
CPlayerKillStreakType::CPlayerKillStreakType () :
    m_bPLAY ( false )
{
}
...
void CPlayerKillStreakType::CreateSubControl ( CString strKeyword )
{
...
void	CPlayerKillStreakType::START()
{
	RESET ();
	m_bPLAY = true;
}

void	CPlayerKillStreakType::RESET()
{
	m_fPlayedTime = 0.0f;
...
void CPlayerKillStreakType::Update ( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{
