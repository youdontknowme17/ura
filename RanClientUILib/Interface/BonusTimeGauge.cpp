...
void CBonusTimeGauge::Update ( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{
	...
	if ( m_bCharging )	SetTimeCharge( sCharData.m_EventStartTime - sCharData.m_RemainEventTime, sCharData.m_EventStartTime );
	else				SetTimeConsume( sCharData.m_RemainBusterTime, sCharData.m_EventBusterTime );
	...
	if ( CHECK_MOUSE_IN ( GetMessageEx () ) )
	{
		if ( CInnerInterface::GetInstance().BEGIN_COMMON_LINEINFO_MULTI() )
		{
			...
			int nEXP = int(sEventState.fExpGainRate);
			int nITEM = int(sEventState.fItemGainRate);
			int nREMAIN = sCharData.m_RemainBusterTime / 60;
			CString strEXP, strITEM;
			strEXP.Format ( "EXP: %3d", nEXP );
			strEXP += "%";
			strITEM.Format("ITEM: %3d", nITEM );
			strITEM += "%";
			CString strCOMBINE;
			strCOMBINE.Format("%s %s Remaining Time: %d minutes.", strEXP, strITEM, nREMAIN);
			CInnerInterface::GetInstance().ADD_COMMON_LINEINFO_MULTI( strCOMBINE.GetString(), NS_UITEXTCOLOR::WHITE  );
