...
const	float	CDialogueWindow::fANSWER_PART_INTERVAL = 12.0f;
...
void CDialogueWindow::CreateSubControl ()
{
...

	{
		...
		m_pMoney->SetTextAlign ( TEXT_ALIGN_RIGHT );
		m_pMoney->SetVisibleSingle( FALSE );
		RegisterControl ( m_pMoney );
	}
/*	{
		m_pMoney1 = new CBasicTextBox;
		m_pMoney1->CreateSub ( this, "NEW_MONEY_TEXT1" , UI_FLAG_XSIZE | UI_FLAG_YSIZE );
		m_pMoney1->SetFont ( pFont9 );
		m_pMoney1->SetTextAlign ( TEXT_ALIGN_RIGHT );
		m_pMoney1->SetVisibleSingle( FALSE );
		RegisterControl ( m_pMoney1 );
	}
	{
		m_pMoney2 = new CBasicTextBox;
		m_pMoney2->CreateSub ( this, "NEW_MONEY_TEXT2" , UI_FLAG_XSIZE | UI_FLAG_YSIZE );
		m_pMoney2->SetFont ( pFont9 );
		m_pMoney2->SetTextAlign ( TEXT_ALIGN_RIGHT );
		m_pMoney2->SetVisibleSingle( FALSE );
		RegisterControl ( m_pMoney2 );
	}*/

	{
		...
		m_pBuyButton->SetAlignText( TEXT_ALIGN_RIGHT );
		m_pBuyButton->SetShortcutKey ( DIK_ESCAPE );
		...
		m_pMoneyItem->SetTextAlign ( TEXT_ALIGN_LEFT );
...
CBasicTextButton*  CDialogueWindow::CreateTextButton ( const char* szButton, UIGUID ControlID, const char* szText )
{
	...
	pTextButton->CreateBaseButton ( szButton, nBUTTONSIZE, CBasicButton::RADIO_FLIP, szText, _DEFAULT_FONT_SHADOW_EX_FLAG );
...
//	LONGLONG lnVoteP = GLGaeaClient::GetInstance().GetCharacter ()->m_lnVoteP;
//	LONGLONG lnPremP = GLGaeaClient::GetInstance().GetCharacter ()->m_lnPremP;
	CString strTemp = NS_UITEXTCONTROL::MAKE_MONEY_FORMAT ( lnMoney, 3, "," );
//	CString strTemp1 = NS_UITEXTCONTROL::MAKE_MONEY_FORMAT ( lnVoteP, 3, "," );
//	CString strTemp2 = NS_UITEXTCONTROL::MAKE_MONEY_FORMAT ( lnPremP, 3, "," );
	strGold.Format ( "Gold: %s", strTemp );
//	strVoteP.Format ( "Vote Points: %s", strTemp1 );
//	strPremP.Format ( "Premium Points: %s", strTemp2 );
	m_pMoney->SetOneLineText( strGold , NS_UITEXTCOLOR::SILVER);
//	m_pMoney1->SetOneLineText( strVoteP, NS_UITEXTCOLOR::GREEN);
//	m_pMoney2->SetOneLineText( strPremP, NS_UITEXTCOLOR::GOLD);
	...
	if ( m_pHaveItem )
	{
		if ( (DWORD)lnMoney < MoneyTotal )
		{
			...
			m_pMoneyItem->AddText( strTemp , NS_UITEXTCOLOR::RED);
		}
		else
		{
			CString strTemp = NS_UITEXTCONTROL::MAKE_MONEY_FORMAT ( (LONGLONG)MoneyTotal, 3, "," );
			m_pMoneyItem->ClearText();
			m_pMoneyItem->AddText( strTemp , NS_UITEXTCOLOR::SILVER);
		}
	...
	{
		m_pText[0]->AddText( ID2GAMEWORD ( "NEW_TEXT_DIALOGUE_WINDOW_PAGE", 0 ) , NS_UITEXTCOLOR::SILVER);
		m_pText[1]->AddText( ID2GAMEWORD ( "NEW_TEXT_DIALOGUE_WINDOW_PAGE", 1 ) , NS_UITEXTCOLOR::SILVER);
		m_pText[2]->AddText( ID2GAMEWORD ( "NEW_TEXT_DIALOGUE_WINDOW_PAGE", 2 ) , NS_UITEXTCOLOR::SILVER);


		if ( IsUse && IsBuy ) // 〈롤蓼 プ于芽紆털丞藕닻
	...
	case DIALOGUE_ANSWER_TEXTBOX:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )
			{
				...
				m_pAnswerBox->SetOverColor ( nIndex, NS_UITEXTCOLOR::WHITE);
...
//		m_pMoney1->SetVisibleSingle( TRUE );
//		m_pMoney2->SetVisibleSingle( TRUE );
...
void CDialogueWindow::TranslateBasicMessage ( SNpcTalk* pNpcTalk )
{
	...
	case SNpcTalk::EM_ITEM_MIX:
		...
		break;
	case SNpcTalk::EM_POINT_SHOP:
		{
			CInnerInterface::GetInstance().HideGroup ( GetWndID () );
			CInnerInterface::GetInstance().OpenPointShopWindow( m_sCrowID.dwID );
		}
		break;
	};
}
...
void CDialogueWindow::LoadBasicTalk ()
{
	...
	m_pQuestionBox->SetText ( str.c_str(), NS_UITEXTCOLOR::WHITE );
