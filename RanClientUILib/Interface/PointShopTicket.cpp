...
CPointShopTicket::CPointShopTicket () 
	: m_pPointShopTicket ( NULL )
	, m_pd3dDevice ( NULL )
...
HRESULT CPointShopTicket::InitDeviceObjects ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	HRESULT hr = S_OK;
...
void	CPointShopTicket::CreateSubControl ()
{
	m_pPointShopTicket = new CUIControl;
	m_pPointShopTicket->CreateSub ( this, "ITEM_IMAGE" );	
	m_pPointShopTicket->SetUseRender ( TRUE );
	m_pPointShopTicket->SetVisibleSingle ( FALSE );
	RegisterControl ( m_pPointShopTicket );
...
void CPointShopTicket::Update( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{
...
void CPointShopTicket::SetItem ( SITEMCUSTOM sItemCustom )
{	
	SITEM* pItemSet = GLItemMan::GetInstance().GetItem( sItemCustom.sNativeID );
	if ( !pItemSet )	return;

	if ( !m_pPointShopTicket )	return;

	if( m_sNativeID == pItemSet->sBasicOp.sNativeID && pItemSet->sBasicOp.sNativeID != NATIVEID_NULL() )	return;

	if ( ( pItemSet->sBasicOp.sICONID == m_sICONINDEX) && ( pItemSet->GetInventoryFile() == m_pPointShopTicket->GetTextureName ()) )	return ;


	ResetItem ();
	SetItemProcess( pItemSet->sBasicOp.sNativeID );

	if ( m_pPointShopTicket )
	{
		m_pPointShopTicket->SetTexturePos ( NS_SKILL::GetIconTexurePos ( pItemSet->sBasicOp.sICONID ) );
		m_pPointShopTicket->SetTextureName ( pItemSet->GetInventoryFile() );	

		if ( m_pd3dDevice )
		{	
			m_pPointShopTicket->InitDeviceObjects ( m_pd3dDevice );
			m_pPointShopTicket->SetVisibleSingle ( TRUE );
...
SITEMCUSTOM&	 CPointShopTicket::GetItem ()
{
	return sItem;
}

void CPointShopTicket::ResetItem ()
{
	if ( m_pPointShopTicket )
	{
		if ( m_sICONINDEX != NATIVEID_NULL () )
		{
			m_pPointShopTicket->DeleteDeviceObjects ();
			m_pPointShopTicket->SetTextureName ( NULL );
			m_pPointShopTicket->SetVisibleSingle ( FALSE );
			m_sICONINDEX = NATIVEID_NULL ();
			ReSetItemProcess();
...
const CString&	CPointShopTicket::GetItemTextureName () const
{
	return m_strTextureName;
}

void CPointShopTicket::SetItemProcess( SNATIVEID sNativeID )
{
	m_bItemUsed = false;	
...
void CPointShopTicket::ReSetItemProcess()
{
	m_bItemUsed = false;
...
void CPointShopTicket::SetDiffuse(D3DCOLOR _diffuse)
{
...
void CPointShopTicket::VisibleFlipImage( bool bVisible )
{
	if ( m_pItemImageFlip )
	{
