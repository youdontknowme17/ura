			...
			if ( pItem->sBasicOp.emItemType != ITEM_VEHICLE )
			{
...

				{
					if ( pItem->sBasicOp.emItemType != ITEM_VEHICLE )
					{
						...
						{
						DxSkinPieceRootData* pPieceDataRoot;
						pPieceDataRoot = DxSkinPieceRootDataContainer::GetInstance().LoadData( pItem->GetWearingFile2(emIndex) , pd3dDevice , TRUE );
						if ( pPieceDataRoot )
						{
							if ( pPieceDataRoot->GetUseCPS() )
							{
							...
							m_pSkinChar->SetPiece ( pItem->GetWearingFile2(emIndex), pd3dDevice, NULL, ItemCustom.GETGRADE_EFFECT(), TRUE );
						}
					}
				}
			}
