...
BOOL CRanManagerCharEdit::OnInitDialog()
{
	...
	SetWin_Combo_Init ( this, IDC_COMBO_EMCLASS, COMMENT::CHARCLASS, GLCI_NUM_2015 );
...
void CRanManagerCharEdit::LoadChar(int nChar)
{
	...
	_snprintf( szTemp, 1024, "SELECT UserNum,SGNum,ChaName,ChaTribe,ChaClass,"
							...
							"ChaReborn, ChaPkWin, ChaPkLoss, ChaVotePoint, ChaPremiumPoint, ChaCP,"
	...
	while (true)
	{
		...
		{		
			...
			::SQLGetData(hStmt,52, SQL_C_LONG, &nChaPkWin, 0, &cbChaPkWin);	
			::SQLGetData(hStmt,53, SQL_C_LONG, &nChaPkLoss, 0, &cbChaPkLoss);
			::SQLGetData(hStmt,54, SQL_C_SBIGINT, &llChaMoney2,  0, &cbChaMoney2);
			::SQLGetData(hStmt,55, SQL_C_SBIGINT, &llChaMoney3,  0, &cbChaMoney3);
			::SQLGetData(hStmt,56, SQL_C_LONG, &nChaCP,			0, &cbChaCP);
			...
			strMoney.Format("%I64d",llChaMoney2);
			SetWin_Text ( this, IDC_EDIT_CHAVOTEP,strMoney);
			strMoney.Format("%I64d",llChaMoney3);
			SetWin_Text ( this, IDC_EDIT_CHAPREMP,strMoney);
...
void CRanManagerCharEdit::OnCbnSelchangeComboEmclass()
{
	...
	if ( nClass == GLCI_FIGHTER_M || nClass == GLCI_ARMS_M || nClass == GLCI_EXTREME_M || nClass == GLCI_ARCHER_M ||
		nClass == GLCI_SPIRIT_M || nClass == GLCI_SCIENCE_M || nClass == GLCI_ASSASIN_M || nClass == GLCI_TESTING_M ||
		nClass == GLCI_SHAPER_M )
	{
		SetWin_Text(this,IDC_EDIT_SEX,"Male");
			...
			nClass == GLCI_SPIRIT_W || nClass == GLCI_SCIENCE_W || nClass == GLCI_ASSASIN_W || nClass == GLCI_TESTING_W ||
			nClass == GLCI_SHAPER_W )
	{
...
void CRanManagerCharEdit::OnBnClickedButtonChareditSave()
{
	...
	LONGLONG	llVote = _strtoi64 ( GetWin_Text(this,IDC_EDIT_CHAVOTEP),NULL,10  );
	LONGLONG	llPremium = _strtoi64 ( GetWin_Text(this,IDC_EDIT_CHAPREMP),NULL,10  );
	...
	if ( emIndex == GLCI_FIGHTER_M || emIndex == GLCI_ARMS_M || emIndex == GLCI_EXTREME_M || emIndex == GLCI_ARCHER_M ||
		emIndex == GLCI_SPIRIT_M || emIndex == GLCI_SCIENCE_M || emIndex == GLCI_ASSASIN_M || emIndex == GLCI_TESTING_M ||
		emIndex == GLCI_SHAPER_M )
	{
		nSex =1;
	}
	else if ( emIndex == GLCI_FIGHTER_W || emIndex == GLCI_ARMS_W || emIndex == GLCI_EXTREME_W || emIndex == GLCI_ARCHER_W ||
			emIndex == GLCI_SPIRIT_W || emIndex == GLCI_SCIENCE_W || emIndex == GLCI_ASSASIN_W || emIndex == GLCI_TESTING_W ||
			emIndex == GLCI_SHAPER_W )
	{
		nSex =0;
	}
	...
	_snprintf( szTemp, 2048, "UPDATE %s.dbo.ChaInfo SET ChaBright=%d, ChaLevel=%u, ChaMoney=%I64d, "
				...
				"ChaVotePoint=%I64d, ChaPremiumPoint=%I64d, ChaCP=%u ,ChaName='%s' ,ChaTribe=%d, "
				"ChaClass=%d, SGNum=%u, ChaInvenLine=%u, GuNum=%u, ChaSex=%u, ChaHairColor=%u, "
				"ChaDeleted=%d, GuPosition=%u, ChaCreateDate='%s', ChaDeletedDate='%s', ChaGuSecede='%s'  "
				"WHERE ChaNum=%u", 
	...
	if (sReturn == SQL_SUCCESS || sReturn == SQL_SUCCESS_WITH_INFO)	MessageBox("Success! Saving Char Data :-)","Ran Manager",MB_OK);
