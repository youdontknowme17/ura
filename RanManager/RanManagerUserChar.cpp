...
void CRanManagerUserChar::LoadChar(int nUser)
{
	...
	while (true)
	{
		...
		if (sReturn == SQL_SUCCESS || sReturn == SQL_SUCCESS_WITH_INFO)
		{
			...
			CString strTemp;
			if (nChaOnline == 0 )	strTemp.Format("Offline(%d)",nChaOnline);
			else if (nChaOnline > 0 )	strTemp.Format("Online(%d)",nChaOnline);
			m_list_Char.SetItemText(nNum,3,strTemp.GetString());

			if (nChaDeleted == 0 )	strTemp.Format("Not Deleted(%d)",nChaDeleted);
			else if (nChaDeleted> 0 )	strTemp.Format("Deleted(%d)",nChaDeleted);
