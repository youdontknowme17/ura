...
void CRanManagerUser::OnBnClickedButtonSearch()
{
	...
	while (true)
	{
		...
		if (sReturn == SQL_SUCCESS || sReturn == SQL_SUCCESS_WITH_INFO)
		{
			...
			CString strTemp;
			if (nUserType < 10 )	strTemp.Format("Normal(%d)",nUserType);
			else if (nUserType == 10 )	strTemp.Format("Staff(%d)",nUserType);
			else if (nUserType > 10 )	strTemp.Format("GM(%d)",nUserType);
			m_list_User.SetItemText(nNum,3,strTemp.GetString());

			if (nUserOnline == 0)strTemp.Format("Offline(%d)",nUserOnline);
			else if (nUserOnline > 0)strTemp.Format("Online(%d)",nUserOnline);
