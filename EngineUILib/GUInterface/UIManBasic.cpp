...
BOOL CUIMan::UpdateList ( CUIFocusContainer* pList, BOOL bFocusList, LPDIRECT3DDEVICEQ pd3dDevice, float fTime, float fElapsedTime )
{
	GASSERT( pList );
	...
	for ( ; riter != riter_end; ++riter )
	{
		...
		pControl->FrameMove( pd3dDevice, fTime, fElapsedTime );
...
HRESULT CUIMan::FrameMove ( LPDIRECT3DDEVICEQ pd3dDevice, float fTime, float fElapsedTime )
{
	GASSERT( pd3dDevice );
	...
	if( !UpdateList( &m_FocusList, TRUE, pd3dDevice, fTime, fElapsedTime ) )	return E_FAIL;
	if( !UpdateList( &m_BottomList, FALSE, pd3dDevice, fTime, fElapsedTime ) )	return E_FAIL;
	if( !UpdateList( &m_TopList, FALSE, pd3dDevice, fTime , fElapsedTime ) )	return E_FAIL;
