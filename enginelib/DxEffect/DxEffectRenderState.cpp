...
#include "../DxOctree/DxOctreeMesh.h"
...
const DWORD	DxEffectRenderState::VERSION_SAVE = 0x00000104;
const char DxEffectRenderState::NAME[] = "DxEffectRenderState";
...
void DxEffectRenderState::SetProperty ( PBYTE &pProp, DWORD &dwSize, DWORD dwVer )
{
	...
	{
		CString strTemp = "";
		strTemp.Format( "DxEffectRenderState::SetProperty Error! Version:%d Size:%d Current Version:%d" 		,dwVer
		,dwSize
		,VERSION
		);
...
void DxEffectRenderState::LoadBufferSet ( CSerialFile &SFile, DWORD dwVer, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	{
		CString strTemp = "";
		strTemp.Format( "DxEffectRenderState::LoadBufferSet Error! Name:%s Version:%d Current Version:%d" ,SFile.GetFileName(),dwVer,VERSION);
		if ( CCrypt::bEngineDebug )  MessageBox (NULL,strTemp,"Need New Version",MB_OK);
		if ( CCrypt::bEngineDebugLog )  CDebugSet::ToFile ( "EngineLoadError.txt",strTemp );
		strTemp.Empty();
...
void DxEffectRenderState::LoadBuffer ( CSerialFile &SFile, DWORD dwVer, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	{
		CString strTemp = "";
		strTemp.Format( "DxEffectRenderState::LoadBuffer Error! Name:%s Version:%d Current Version:%d" ,SFile.GetFileName(),dwVer,VERSION);
		if ( CCrypt::bEngineDebug )  MessageBox (NULL,strTemp,"Need New Version",MB_OK);
		if ( CCrypt::bEngineDebugLog )  CDebugSet::ToFile ( "EngineLoadError.txt",strTemp );
		strTemp.Empty();
