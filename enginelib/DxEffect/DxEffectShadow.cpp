...
const DWORD	DxEffectShadow::VERSION_SAVE = 0x101;
const char	DxEffectShadow::NAME[] = "DxEffectShadow";
...
void DxEffectShadow::SetProperty ( PBYTE &pProp, DWORD &dwSize, DWORD dwVer )
{
	...
	{
		CString strTemp = "";
		strTemp.Format( "DxEffectShadow::SetProperty Error! Version:%d Size:%d Current Version:%d" 		,dwVer
		,dwSize
		,VERSION
		);
...
void DxEffectShadow::LoadBuffer ( CSerialFile &SFile, DWORD dwVer, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	else
	{
		...
		CString strTemp = "";
		strTemp.Format( "DxEffectShadow::LoadBuffer Error! Name:%s Version:%d Size:%d Current Version:%d" ,SFile.GetFileName(),dwVer,dwBuffSize,VERSION);
		if ( CCrypt::bEngineDebug )  MessageBox (NULL,strTemp,"Need New Version",MB_OK);
		if ( CCrypt::bEngineDebugLog )  CDebugSet::ToFile ( "EngineLoadError.txt",strTemp );
		strTemp.Empty();
