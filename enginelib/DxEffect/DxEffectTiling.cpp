...
#include "../Meshs/DxMaterial.h"
#include "../Meshs/DxFrameMesh.h"
#include "../DxEffect/DxEffectDefine.h"
...
const DWORD	DxEffectTiling::VERSION_SAVE = 0x00000102;
const char DxEffectTiling::NAME[] = "DxEffectTiling";
...
{
	if ( ( dwVer==0x00000105 || dwVer==0x00000103 ) && dwSize==sizeof(TILING_PROPERTY_103) )
	{
		TILING_PROPERTY_103 sProp;
		memcpy ( &sProp, pProp, dwSize );

		m_Property.m_dwFlag		= sProp.m_dwFlag;
		m_Property.m_vMax		= sProp.m_vMax;
		m_Property.m_vMin		= sProp.m_vMin;
		m_Property.m_dwVertices	= sProp.m_dwVertices;
		m_Property.m_dwFaces	= sProp.m_dwFaces;
		m_Property.m_dwUNIQUE_NUMBER	= sProp.m_dwUNIQUE_NUMBER;
		m_Property.m_dwMaterials_B		= sProp.m_dwMaterials_B;
		m_Property.m_matFrameComb		= sProp.m_matFrameComb;
	}
	else if ( dwVer==VERSION && dwSize==sizeof(TILING_PROPERTY) )
	{
	...
	{
		CString strTemp = "";
		strTemp.Format( "DxEffectTiling::SetProperty Error! Version:%d Size:%d Current Version:%d" 
		,dwVer
		,dwSize
		,VERSION
		);
