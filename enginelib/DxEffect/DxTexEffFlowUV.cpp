...
const DWORD	DxTexEffFlowUV::VERSION_SAVE = 0x00000100;
const char DxTexEffFlowUV::NAME[] = "DxTexEffFlowUV";
...
void DxTexEffFlowUV::LoadPSF( LPDIRECT3DDEVICEQ pd3dDevice, CSerialFile& SFile )
{
	...
	if( dwVer==0x00000101 )
	{
		...
		SFile >> m_sProp.dwFlag;
		SFile >> m_sProp.strAlphaTex;
		SFile >> m_sProp.strFlowTex;
		SFile >> m_sProp.fScale;
		SFile >> m_sProp.vSpeed.x;
		SFile >> m_sProp.vSpeed.y;
		SFile >> m_sProp.fAlpha;
		SFile >> m_sProp.fSpeed;
		SFile >> m_sProp.vDirect.x;
		SFile >> m_sProp.vDirect.y;
	...
	{
		CString strTemp = "";
		strTemp.Format( "DxTexEffFlowUV::LoadPSF Error! Name:%s Version:%d Size:%d Current Version:%d" ,SFile.GetFileName(),dwVer,dwSize,VERSION);
