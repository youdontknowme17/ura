...
const DWORD	DxEffectCloth::VERSION_SAVE = 0x00000104;
const char DxEffectCloth::NAME[] = "DxEffectCloth";
...
void DxEffectCloth::SetProperty ( PBYTE &pProp, DWORD &dwSize, DWORD dwVer )
{
	...
	{
		CString strTemp = "";
		strTemp.Format( "DxEffectCloth::SetProperty Error! Version:%d Size:%d Current Version:%d" ,dwVer,dwSize,VERSION);
...
{	
	...
	//----------- 인덱스 버퍼에서 3개의 노드로 분리 
	int k = 0;
	for( int i = 0 ; i < (int)dwNumIndices ; i+=3 )
	{
		for( int j = 0 ; j < 3 ; j++, k++ )
		{
