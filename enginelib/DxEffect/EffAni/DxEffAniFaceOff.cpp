...
DWORD		DxEffAniData_FaceOff::VERSION_SAVE	= 0x0100;
TCHAR		DxEffAniData_FaceOff::NAME[64]		= _T("EffAniData_FaceOff");
...
HRESULT	DxEffAniData_FaceOff::LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice ,bool bTool)
{
	HRESULT hr;
	...
	{
		CString strTemp = "";
		strTemp.Format( "DxEffAniData_FaceOff::LoadFile Error! Name:%s Version:%d Size:%d Current Version:%d" ,SFile.GetFileName(),dwVer,dwSize,VERSION);
...
//add abl
HRESULT	DxEffAniFaceOff::Render ( const LPDIRECT3DDEVICEQ pd3dDevice, DxSkinCharPiece* pSkinCharPiece, const D3DXMATRIX& matCurPos )
{
	if( !m_bFrameMove )	return S_OK;	// 애니메이션이 같지가 않다.

	if ( !pSkinCharPiece->m_PartArray[PIECE_HEAD].m_pSkinMesh )	return S_OK;
	if ( !pSkinCharPiece->m_PartArray[PIECE_HEAD].m_pmcMesh )	return S_OK;

	pSkinCharPiece->m_sSkinEffData.m_rFaceTex = m_pTexture;
	pd3dDevice->SetTexture( 0, m_pTexture );
...
//add vcf
HRESULT	DxEffAniFaceOff::Render ( const LPDIRECT3DDEVICEQ pd3dDevice, DxSkinVehicle* pSkinVehicle, const D3DXMATRIX& matCurPos )
{
	if( !m_bFrameMove )	return S_OK;	// 애니메이션이 같지가 않다.

	if ( !pSkinVehicle->m_PartArray[PIECE_HEAD].m_pSkinMesh )	return S_OK;
	if ( !pSkinVehicle->m_PartArray[PIECE_HEAD].m_pmcMesh )	return S_OK;

	pSkinVehicle->m_sSkinEffData.m_rFaceTex = m_pTexture;
	pd3dDevice->SetTexture( 0, m_pTexture );
