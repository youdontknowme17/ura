...
DWORD		DxEffAniData_Single::VERSION_SAVE	= 0x0101;
char		DxEffAniData_Single::NAME[64]		= "EffAniData_Single";
...
HRESULT	DxEffAniData_Single::LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice ,bool bTool)
{
	HRESULT hr;
	...
	{
		CString strTemp = "";
		strTemp.Format( "DxEffAniData_Single::LoadFile Error! Name:%s Version:%d Size:%d Current Version:%d" ,SFile.GetFileName(),dwVer,dwSize,VERSION);
...
//add abl
HRESULT	DxEffAniSingle::Render ( const LPDIRECT3DDEVICEQ pd3dDevice, DxSkinCharPiece* pSkinCharPiece, const D3DXMATRIX& matCurPos )
{
	if ( !m_pSingleGroup)	return S_OK;
	...
	{
		LPD3DXMATRIX pBoneMatrix(NULL);
		if ( !SetPosition ( pSkinCharPiece, pBoneMatrix ) )	return S_OK;
...
//add vcf
HRESULT	DxEffAniSingle::Render ( const LPDIRECT3DDEVICEQ pd3dDevice, DxSkinVehicle* pSkinVehicle, const D3DXMATRIX& matCurPos )
{
	if ( !m_pSingleGroup)	return S_OK;
	...
	{
		LPD3DXMATRIX pBoneMatrix(NULL);
		if ( !SetPosition ( pSkinVehicle, pBoneMatrix ) )	return S_OK;
...
//add abl
BOOL	DxEffAniSingle::SetPosition ( DxSkinCharPiece* pSkinCharPiece, LPD3DXMATRIX& pMatrix )
{
	DxSkeleton::BONETRANSMAP* BoneMap = &pSkinCharPiece->GetSkeleton()->BoneMap;
...
//add vcf
BOOL	DxEffAniSingle::SetPosition ( DxSkinVehicle* pSkinVehicle, LPD3DXMATRIX& pMatrix )
{
	DxSkeleton::BONETRANSMAP* BoneMap = &pSkinVehicle->GetSkeleton()->BoneMap;
