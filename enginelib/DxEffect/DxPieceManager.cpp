...
void DxPieceAABB::Load_EP9( LPDIRECT3DDEVICEQ pd3dDevice, CSerialFile& SFile, const char* pPieceName )
{
	DWORD dwVER;
	DWORD dwSIZE;
	...
	if ( dwVER == 0x0100 )
	{
		...
		NSPIECETREE::Load_EP9( pd3dDevice, m_pPieceAABBTree, SFile, pPieceEdit );
	}
	else
	{
...
const DWORD DxPieceManager::VERSION_PSF_SAVE = 0x101;
const DWORD DxPieceManager::VERSION_WLD_SAVE = 0x100;
...
void DxPieceManager::LoadWLD_EP9( LPDIRECT3DDEVICEQ pd3dDevice, CSerialFile& SFile )
{
	CleanUp();
	...
	if( dwVER == 0x0100 )
	{
		...
		for( DWORD i=0; i<dwNUM; ++i )
		{
			SFile >> szPieceName;
			DxPieceAABB* pNew = new DxPieceAABB;
			pNew->Load_EP9( pd3dDevice, SFile, szPieceName.c_str() );
