...
DWORD		DxCharAroundEff::VERSION			= 0x0150;
DWORD		DxCharAroundEff::VERSION_SAVE			= 0x0001;
...
HRESULT	DxCharAroundEff::LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice ,bool bTool,bool bRan2 )
{
	HRESULT hr;
	DWORD dwVer, dwSize;
	...
	{
		CString strTemp = "";
		strTemp.Format( "DxCharAroundEff::LoadFile Error! Name:%s Version:%d Size:%d Current Version:%d" ,SFile.GetFileName(),dwVer,dwSize,VERSION);
...
HRESULT	DxCharAroundEff::SaveFile ( basestream &SFile ,bool bTool)
{
	SFile << TYPEID;
	SFile << VERSION_SAVE;
...
void DxCharAroundEff::InsertAroundEffectTool( SAROUNDEFF_DATA addData )
{
	m_vecAroundEffList.push_back(addData);
}
