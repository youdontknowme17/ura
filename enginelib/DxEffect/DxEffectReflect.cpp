...
const DWORD	DxEffectReflect::VERSION_SAVE = 0x0100;
const char DxEffectReflect::NAME[] = "DxEffectReflect";
...
void DxEffectReflect::SetProperty ( PBYTE &pProp, DWORD &dwSize, DWORD dwVer )
{
	...
	{
		CString strTemp = "";
		strTemp.Format( "DxEffectReflect::SetProperty Error! Version:%d Size:%d Current Version:%d" 		,dwVer
		,dwSize
		,VERSION
		);
...
void DxEffectReflect::LoadBuffer ( CSerialFile &SFile, DWORD dwVer, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	{
		CString strTemp = "";
		strTemp.Format( "DxEffectReflect::LoadBuffer Error! Name:%s Version:%d Size:%d Current Version:%d" ,SFile.GetFileName(),dwVer,dwBuffSize,VERSION);
		if ( CCrypt::bEngineDebug )  MessageBox (NULL,strTemp,"Need New Version",MB_OK);
		if ( CCrypt::bEngineDebugLog )  CDebugSet::ToFile ( "EngineLoadError.txt",strTemp );
		strTemp.Empty();
