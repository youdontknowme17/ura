...
void DxEffectTiling::LoadBufferSet ( CSerialFile &SFile, DWORD dwVer, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	else
	{
		...
		CString strTemp = "";
		strTemp.Format( "DxEffectTiling::LoadBufferSet Error! Name:%s Version:%d Size:%d Current Version:%d" 
		,SFile.GetFileName(),dwVer,dwBuffSize_Base,VERSION);
...
void DxEffectTiling::LoadBuffer ( CSerialFile &SFile, DWORD dwVer, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	else if ( VERSION==dwVer || 0x00000103==dwVer )
	{
		SFile >> dwBuffSize_Blend; //	읽지 않고 스킵용으로 사용되므로 실제 로드에서는 무의미.
	...
	else
	{
		...
		CString strTemp = "";
		strTemp.Format( "DxEffectTiling::LoadBuffer Error! Name:%s Version:%d Size:%d Current Version:%d" ,SFile.GetFileName(),dwVer,dwBuffSize_Base,VERSION);
		if ( CCrypt::bEngineDebug )  MessageBox (NULL,strTemp,"Need New Version",MB_OK);
		if ( CCrypt::bEngineDebugLog )  CDebugSet::ToFile ( "EngineLoadError.txt",strTemp );
		strTemp.Empty();
