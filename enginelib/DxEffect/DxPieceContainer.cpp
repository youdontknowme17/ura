...
const DWORD DxPieceEdit::VERSION_SAVE	= 0x0102;
...
HRESULT DxPieceEdit::LoadPEF( LPDIRECT3DDEVICEQ pd3dDevice, const char* szName )
{
	...
	{
		CString strTemp = "";
		strTemp.Format( "DxPieceEdit::LoadPEF Error! Name:%s Version:%d Size:%d Current Version:%d" ,SFile.GetFileName(),dwVer,dwBufferSize,VERSION);
...
HRESULT DxPieceEdit::LoadPIE( LPDIRECT3DDEVICEQ pd3dDevice, const char* szName )
{
	...
	if( dwVer==0x0103 ) //official dummy
	{
		SFile >> m_vMin;
		SFile >> m_vMax;
		SFile >> m_vStaticMin;
	...
	{
		CString strTemp = "";
		strTemp.Format( "DxPieceEdit::LoadPIE Error! Name:%s Version:%d Size:%d Current Version:%d" ,SFile.GetFileName(),dwVer,dwBufferSize,VERSION);
		if ( CCrypt::bEngineDebug )  MessageBox (NULL,strTemp,"Need New Version",MB_OK);
		if ( CCrypt::bEngineDebugLog )  CDebugSet::ToFile ( "EngineLoadError.txt",strTemp );
		strTemp.Empty();
