...
const DWORD	EFF_PROPGROUP::FILEVERSION_SAVE = 0x00000001;
const DWORD	EFF_PROPGROUP::VERSION			= 0x0106;
const DWORD	EFF_PROPGROUP::VERSION_SAVE		= 0x0105;
...
EFF_PROPERTY* EFF_PROPERTY::NEW_PROP ( DWORD TypeID )
{
	...
	{
		CString strTemp = "";
		strTemp.Format( "EFF_PROPERTY::NEW_PROP New Effect Type ID :%d" ,TypeID);
...
HRESULT EFF_PROPGROUP::LoadFile ( const char* szFileName, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	{
		CString strTemp = "";
		strTemp.Format( "EFF_PROPGROUP::LoadFile Error! Name:%s FileVersion:%d Current FileVersion:%d" ,SFile.GetFileName(),dwFileVer,EFF_PROPGROUP::FILEVERSION);
		if ( CCrypt::bEngineDebug )  MessageBox (NULL,strTemp,"Need New Version",MB_OK);
		if ( CCrypt::bEngineDebugLog )  CDebugSet::ToFile ( "EngineLoadError.txt",strTemp );
		strTemp.Empty();
	...
	{
		CString strTemp = "";
		strTemp.Format( "EFF_PROPGROUP::LoadFile Error! Name:%s Version:%d Current Version:%d" ,SFile.GetFileName(),dwVer,VERSION);
		if ( CCrypt::bEngineDebug )  MessageBox (NULL,strTemp,"Need New Version",MB_OK);
		if ( CCrypt::bEngineDebugLog )  CDebugSet::ToFile ( "EngineLoadError.txt",strTemp );
		strTemp.Empty();
	}
...
HRESULT EFF_PROPGROUP::SaveFile ( const char* szFileName )
{
	...
	CSerialFile SFile;
	SFile.SetFileType ( "EFF_PROPGROUP", EFF_PROPGROUP::FILEVERSION_SAVE );

	if ( SFile.OpenFile(FOT_WRITE,m_strFileName.c_str()) )
	{
		//	Note : EFF_PROPGROUP
		//
		SFile << EFF_PROPGROUP::VERSION_SAVE;
		SFile << m_dwFlag;
		SFile << m_vMax;
		SFile << m_vMin;
		SFile << m_vLocal;
