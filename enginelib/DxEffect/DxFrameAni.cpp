...
const DWORD DxFrameAniVBTEX::VERSION_SAVE	= 0x0100;
...
void DxFrameAniVBTEX::Load( LPDIRECT3DDEVICEQ pd3dDevice, CSerialFile& SFile, DxAnimationMan* pAniManHead, const char* pName )
{
	...
	{
		CString strTemp = "";
		strTemp.Format( "DxFrameAniVBTEX::Load Error! Name:%s Version:%d Size:%d Current Version:%d" ,SFile.GetFileName(),dwVer,dwSize,VERSION);
...
const DWORD DxFrameAni::VERSION_SAVE	= 0x0100;
...
void DxFrameAni::Load( LPDIRECT3DDEVICEQ pd3dDevice, CSerialFile& SFile, DxAnimationMan* pAniManHead )
{
	...
	{
		CString strTemp = "";
		strTemp.Format( "DxFrameAni::Load Error! Name:%s Version:%d Size:%d Current Version:%d" ,SFile.GetFileName(),dwVer,dwSize,VERSION);
		if ( CCrypt::bEngineDebug )  MessageBox (NULL,strTemp,"Need New Version",MB_OK);
		if ( CCrypt::bEngineDebugLog )  CDebugSet::ToFile ( "EngineLoadError.txt",strTemp );
		strTemp.Empty();
