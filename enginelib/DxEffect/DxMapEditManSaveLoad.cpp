...
void DxMapEditMan::LoadSet ( char *pFileName, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	{
		CString strTemp = "";
		strTemp.Format( "DxMapEditMan::LoadSet Error! Name:%s Version:%d Current Version:%d" ,SFile.GetFileName(),dwVer,VERSION);
...
void DxMapEditMan::LoadTileData ( const char* pName )
{
	...
	{
		CString strTemp = "";
		strTemp.Format( "DxMapEditMan::LoadTileData Error! Name:%s Version:%d Current Version:%d" ,SFile.GetFileName(),dwVer,VERSION_DATA);
		if ( CCrypt::bEngineDebug )  MessageBox (NULL,strTemp,"Need New Version",MB_OK);
		if ( CCrypt::bEngineDebugLog )  CDebugSet::ToFile ( "EngineLoadError.txt",strTemp );
		strTemp.Empty();
	}
