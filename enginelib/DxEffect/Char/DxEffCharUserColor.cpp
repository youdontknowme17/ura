...
extern BOOL			g_bCHAR_VIEW_RUN;//add huiyi

DWORD		DxEffCharUserColor::TYPEID			= EMEFFCHAR_USERCOLOR;
DWORD		DxEffCharUserColor::VERSION			= 0x0102;
DWORD		DxEffCharUserColor::VERSION_SAVE	= 0x0101;
char		DxEffCharUserColor::NAME[MAX_PATH]	= "EffCharUserColor";
...
HRESULT	DxEffCharUserColor::LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice ,bool bTool,bool bRan2 )
{
	HRESULT hr;
	DWORD dwVer, dwSize;
	...
	if ( dwVer == VERSION )
	{
		...
		{
			SFile >> m_dwFlag;
			SFile.ReadBuffer ( &m_cColor, sizeof(m_cColor) );
			SFile >> m_dwMaterials;
			DWORD	dwUnknown = 0;
			SFile >> dwUnknown;
		}
...
HRESULT	DxEffCharUserColor::SaveFile ( basestream &SFile ,bool bTool)
{
	SFile << TYPEID;
	SFile << VERSION_SAVE;
