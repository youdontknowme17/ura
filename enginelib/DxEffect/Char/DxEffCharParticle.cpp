...
DWORD		DxEffCharParticle::VERSION			= 0x0102;
DWORD		DxEffCharParticle::VERSION_SAVE		= 0x0101;
char		DxEffCharParticle::NAME[MAX_PATH]	= "EffCharParticle"; 
...
D3DXVECTOR3 DxEffCharParticle::GetSelectedVec( const DWORD dwIndex)
{
	DWORD dwCount(0L);
	LISTPOSITION_ITER iter = m_listPosition.begin();
	for( ; iter!=m_listPosition.end(); ++iter, ++dwCount )
	{
		if( dwIndex == dwCount )	return (*iter);
	}
...
//----------------------------------------------------------------------------------------------------------------------------------------
HRESULT	DxEffCharParticle::LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice ,bool bTool,bool bRan2 )
{
	HRESULT hr;
	DWORD dwVer, dwSize;
	...
	{
		CString strTemp = "";
		strTemp.Format( "DxEffCharParticle::LoadFile Error! Name:%s Version:%d Size:%d Current Version:%d" ,SFile.GetFileName(),dwVer,dwSize,VERSION);
...
HRESULT	DxEffCharParticle::SaveFile ( basestream &SFile ,bool bTool)
{
	SFile << TYPEID;
	SFile << VERSION_SAVE;
