...
DWORD		DxEffCharNeon::VERSION			= 0x0106;
DWORD		DxEffCharNeon::VERSION_SAVE		= 0x0105;
char		DxEffCharNeon::NAME[MAX_PATH]	= "EffCharNeon";
...
void	DxEffCharNeon::Render( LPDIRECT3DDEVICEQ pd3dDevice, BOOL bPieceRender )
{
	...
	if ( !DxGlowMan::GetInstance().IsOptionEnable() )	return;	// ???? ?? ??? ???? ???.
...
HRESULT	DxEffCharNeon::LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice ,bool bTool,bool bRan2 )
{
	HRESULT hr;
	DWORD dwVer, dwSize;
	...
	else if ( dwVer == 0x0103 )
	{
		...
		if ( !( m_dwFlag&(USE_ALL_TEX|USE_SELECT_TEX) ) )		m_dwFlag |= USE_SELECT_TEX;	// ??? ??? ??
		if ( !( m_dwFlag&(USE_GLOW_SRCTEX|USE_GLOW_USER) ) )	m_dwFlag |= USE_GLOW_USER;	// ??? ??? ??
		if ( !( m_dwFlag&(USE_GENERAL|USE_BURN) ) )				m_dwFlag |= USE_BURN;		// ??? ??? ??
	...
	else if ( dwVer == 0x0102 )
	{
		...
		m_dwFlag	|= USE_SELECT_TEX;	// ??? ??? ??
		m_dwFlag	|= USE_GLOW_USER;	// ??? ??? ??
		m_dwFlag	|= USE_BURN;		// ??? ??? ??
...
HRESULT	DxEffCharNeon::SaveFile ( basestream &SFile ,bool bTool)
{
	SFile << TYPEID;
	SFile << VERSION_SAVE;
