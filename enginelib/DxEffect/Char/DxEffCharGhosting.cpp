...
DWORD		DxEffCharGhosting::VERSION_SAVE		= 0x0101;
char		DxEffCharGhosting::NAME[MAX_PATH]	= "EffCharGhosting";
...
HRESULT	DxEffCharGhosting::LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice ,bool bTool,bool bRan2 )
{
	HRESULT hr;
	DWORD dwVer, dwSize;
	...
	{
		CString strTemp = "";
		strTemp.Format( "DxEffCharGhosting::LoadFile Error! Name:%s Version:%d Size:%d Current Version:%d" ,SFile.GetFileName(),dwVer,dwSize,VERSION);
...
HRESULT	DxEffCharGhosting::SaveFile ( basestream &SFile ,bool bTool)
{
	SFile << TYPEID;
	SFile << VERSION_SAVE;
