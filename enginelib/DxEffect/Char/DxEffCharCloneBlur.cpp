...
DWORD		DxEffCharCloneBlur::VERSION_SAVE	= 0x0100;
char		DxEffCharCloneBlur::NAME[MAX_PATH]	= "EffCharCloneBlur";
...
HRESULT	DxEffCharCloneBlur::LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice ,bool bTool,bool bRan2 )
{
	HRESULT hr;
	DWORD dwVer, dwSize;
	...
	{
		CString strTemp = "";
		strTemp.Format( "DxEffCharCloneBlur::LoadFile Error! Name:%s Version:%d Size:%d Current Version:%d" ,SFile.GetFileName(),dwVer,dwSize,VERSION);
...
HRESULT	DxEffCharCloneBlur::SaveFile ( basestream &SFile ,bool bTool)
{
	SFile << TYPEID;
	SFile << VERSION_SAVE;
