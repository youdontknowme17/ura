...
DWORD		DxEffCharBonePosEff::VERSION_SAVE	= 0x0100;
char		DxEffCharBonePosEff::NAME[MAX_PATH]	= "EffCharBonePossEff"; 
...
HRESULT	DxEffCharBonePosEff::LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice ,bool bTool,bool bRan2 )
{
	HRESULT hr;
	DWORD dwVer, dwSize;
	...
	{
		CString strTemp = "";
		strTemp.Format( "DxEffCharBonePosEff::LoadFile Error! Name:%s Version:%d Size:%d Current Version:%d" ,SFile.GetFileName(),dwVer,dwSize,VERSION);
...
HRESULT	DxEffCharBonePosEff::SaveFile ( basestream &SFile ,bool bTool)
{
	SFile << TYPEID;
	SFile << VERSION_SAVE;
