...
DWORD		DxEffCharLine2BoneEff::VERSION			= 0x0103;
DWORD		DxEffCharLine2BoneEff::VERSION_SAVE		= 0x0101;
char		DxEffCharLine2BoneEff::NAME[MAX_PATH]	= "EffCharLine2BoneEff"; 
...
void EFFCHAR_PROPERTY_LINE2BONEEFF::Assign( EFFCHAR_PROPERTY_LINE2BONEEFF_0100 &sOldProperty )
{
	...
	m_bWithCamMove	= TRUE;
	strcpy( m_szTexture, sOldProperty.m_szTexture );
	strcpy( m_szTexture2, sOldProperty.m_szTexture2 );
...
//----------------------------------------------------------------------------------------------------------------------------------------
HRESULT	DxEffCharLine2BoneEff::LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice ,bool bTool,bool bRan2 )
{
	HRESULT hr;
	DWORD dwVer, dwSize;
	...
	if ( dwVer == VERSION )
	{
		...
		SFile >> m_dwFlag;
		SFile >> m_dwOtherColor;
		SFile >> m_dwParticlePec;
		SFile >> m_dwColor;
		SFile >> m_dwColor2;
		SFile >> m_dwSequenceCol;
		SFile >> m_dwSequenceRow;
		SFile >> m_fCreateLength;
		...
		SFile.ReadBuffer ( &m_vLife, sizeof(m_vLife) );
		SFile.ReadBuffer ( &m_vSize, sizeof(m_vSize) );
		SFile.ReadBuffer ( &m_vScaleX, sizeof(m_vScaleX) );
		SFile.ReadBuffer ( &m_vScaleY, sizeof(m_vScaleY) );

		SFile >> m_fShake;

		float m_fShake2 = 0;
	
		SFile >> m_fMinMaxDist.min;
		SFile >> m_fMinMaxDist.max;
		SFile >> m_fShake2;
		SFile >> m_fMinMaxTexRotate.min;
		SFile >> m_fMinMaxTexRotate.max;
		m_fShake += m_fShake2;


		SFile.ReadBuffer ( m_szTexture, sizeof(char)*BONELISTEFF_TEXNAME_SIZE );
		SFile.ReadBuffer ( m_szTexture2, sizeof(char)*BONELISTEFF_TEXNAME_SIZE );
	...
	{
		CString strTemp = "";
		strTemp.Format( "DxEffCharLine2BoneEff::LoadFile Error! Name:%s Version:%d Size:%d Current Version:%d" ,SFile.GetFileName(),dwVer,dwSize,VERSION);
...
HRESULT	DxEffCharLine2BoneEff::SaveFile ( basestream &SFile ,bool bTool)
{
	SFile << TYPEID;
	SFile << VERSION_SAVE;
