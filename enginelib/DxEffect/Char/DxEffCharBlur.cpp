...
DWORD		DxEffCharBlur::VERSION			= 0x0106;
DWORD		DxEffCharBlur::VERSION_SAVE		= 0x0105;
char		DxEffCharBlur::NAME[MAX_PATH]	= "EffCharBlur";
...
//												L	O	A	D		&&		 S	A	V	E
//------------------------------------------------------------------------------------------------------------------------------------------------
HRESULT	DxEffCharBlur::LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice , bool bTool,bool bRan2 )
{
	HRESULT hr;
	DWORD dwVer, dwSize;		
	...
	if ( dwVer == VERSION )
	{
		if (bRan2)
		{
			...
			SFile >> m_fFullLifeTemp;

			SFile.ReadBuffer ( m_szTraceUP, sizeof(TCHAR)*STRACE_NSIZE );
			SFile.ReadBuffer ( m_szTraceDOWN, sizeof(TCHAR)*STRACE_NSIZE );

			SFile >> (DWORD) m_vColorUPTemp;
			SFile >> (DWORD) m_vColorDOWNTemp;

			SFile >> dwSize;
			m_SetAniMType = (EMANI_MAINTYPE) dwSize;
			SFile >> dwSize;
			m_SetAniSType = (EMANI_SUBTYPE) dwSize;

			SFile.ReadBuffer ( m_szTexture, sizeof(TCHAR)*TEXTURE_LENGTH );
		...
		else
		{
			...
			if ( m_dwFlag == 16 )
			{
				m_dwFlag = 0; //fix when blur is not properly created in version 6 //jdev
			}
			...
			SFile >> m_fFullLifeTemp;
			SFile >> m_vColorUPTemp;
			SFile >> m_vColorDOWNTemp;
	...
	{
		CString strTemp = "";
		strTemp.Format( "DxEffCharBlur::LoadFile Error! Name:%s Version:%d Size:%d Current Version:%d" ,SFile.GetFileName(),dwVer,dwSize,VERSION);
...
HRESULT	DxEffCharBlur::SaveFile ( basestream &SFile ,bool bTool)
{
	SFile << TYPEID;
	SFile << VERSION_SAVE;
