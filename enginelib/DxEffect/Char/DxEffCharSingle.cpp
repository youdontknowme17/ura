...
DWORD		DxEffCharSingle::VERSION			= 0x0108;
DWORD		DxEffCharSingle::VERSION_SAVE		= 0x0105;
char		DxEffCharSingle::NAME[MAX_PATH]		= "EffCharSingle";
...
HRESULT	DxEffCharSingle::LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice ,bool bTool,bool bRan2 )
{
	HRESULT hr;
	DWORD dwVer, dwSize;
	...
	{
		SFile >> m_dwFlag;
		SFile >> m_dwPlaySlngle;

		DWORD dwSize;

		SFile >> dwSize;
		m_nWeather = (int)dwSize;
		SFile >> dwSize;
		m_nAniMType = (int)dwSize;
		
		SFile >> m_fCoolTime;
		SFile >> m_fScale;
		SFile >> m_fDepthUp;
		SFile >> m_fDepthDown;
		SFile >> m_fRepeatSpeed;

		SFile.ReadBuffer ( m_szFileName, sizeof(char)*MAX_PATH );
		SFile.ReadBuffer ( m_szTrace, sizeof(char)*STRACE_NSIZE );
		SFile.ReadBuffer ( m_szTraceBack, sizeof(char)*STRACE_NSIZE );

		float	m_f_Mincoms[4];
		DWORD	m_dw_is[4];
		DWORD	m_dw_Fucking[4];
		DWORD	m_dw_stupid[4];
	...
	{
		CString strTemp = "";
		strTemp.Format( "DxEffCharSingle::LoadFile Error! Name:%s Version:%d Size:%d Current Version:%d" ,SFile.GetFileName(),dwVer,dwSize,VERSION);
...
HRESULT	DxEffCharSingle::SaveFile ( basestream &SFile ,bool bTool)
{
	SFile << TYPEID;
	SFile << VERSION_SAVE;
