...
DWORD		DxEffCharMultiTex::VERSION			= 0x0106;
DWORD		DxEffCharMultiTex::VERSION_SAVE		= 0x0105;
char		DxEffCharMultiTex::NAME[MAX_PATH]	= "EffCharMultiTex";
...
HRESULT DxEffCharMultiTex::InitDeviceObjects ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	if ( m_pTempMaterials )
	{
		...
		{
			if( i >= m_dwTempMaterials )						continue;	// ? ? ???? ??? ???. Loading? m_dwTempMaterials = 3 ?? ?? ?? ? ??? ??.
...
void	DxEffCharMultiTex::Render( LPDIRECT3DDEVICEQ pd3dDevice, BOOL bPieceRender )
{
	...
	//	??
	vMove.x = m_vSumTex01.x * m_vTexUV01.x;	// m_vTexUV01 ??? UV ??? ???.
	vMove.y = m_vSumTex01.y * m_vTexUV01.y;
	DxImageMove ( pd3dDevice, m_pTex1, DxSurfaceTex::GetInstance().m_pTempSuf128,
				vMove, TRUE, D3DFVF_DIFFUSE, m_cDiffuse1, m_vTexUV01, 128 );			// Alpha ? ?? ??? m_cDiffuse? ???? ? ?.
	//	?? ( ?? ?? )
...
HRESULT	DxEffCharMultiTex::LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice ,bool bTool,bool bRan2 )
{
	HRESULT hr;
	DWORD dwVer, dwSize;
	...
	if ( dwVer == VERSION )
	{
		...
		else
		{
			...
			SFile.ReadBuffer ( m_szTex1, sizeof(char)*MAX_PATH );
			SFile.ReadBuffer ( m_szTex2, sizeof(char)*MAX_PATH );
			...
			m_nBlend = (int)dwSize;
			SFile >> dwSize;
			m_nTexNum = (int)dwSize;

			SFile.ReadBuffer ( &m_cDiffuse1, sizeof(m_cDiffuse1) );
			SFile.ReadBuffer ( &m_cDiffuse2, sizeof(m_cDiffuse2) );
			SFile.ReadBuffer ( &m_vTex01, sizeof(m_vTex01) );
			SFile.ReadBuffer ( &m_vTex02, sizeof(m_vTex02) );
			SFile.ReadBuffer ( &m_vTexUV01, sizeof(m_vTexUV01) );
			SFile.ReadBuffer ( &m_vTexUV02, sizeof(m_vTexUV02) );
		}
	...
	else if ( dwVer == 0x0104 )
	{
		...
		{
			m_dwFlag	|= USE_SELECT_TEX;	// Ver.105 ?? ??? ??? ??
		}
...
HRESULT	DxEffCharMultiTex::SaveFile ( basestream &SFile ,bool bTool)
{
	SFile << TYPEID;
	SFile << VERSION_SAVE;
