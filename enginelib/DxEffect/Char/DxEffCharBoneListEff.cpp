...
DWORD		DxEffCharBoneListEff::VERSION			= 0x0101;
DWORD		DxEffCharBoneListEff::VERSION_SAVE		= 0x0100;
char		DxEffCharBoneListEff::NAME[MAX_PATH]	= "EffCharBoneListEff"; 
...
void DxEffCharBoneListEff::InsertBoneTool(  DXCUSTOMBONE	sCBoneNew )
{
	m_listCustomBone.push_back( sCBoneNew );
}
...
DXCUSTOMBONE DxEffCharBoneListEff::GetSelectedBoneData( const DWORD dwIndex )
{
	int nCount = 0;
	LISTCUSTOMBONE_ITER iter = m_listCustomBone.begin();
	for( ; iter!=m_listCustomBone.end(); ++iter, ++nCount )
...
//----------------------------------------------------------------------------------------------------------------------------------------
HRESULT	DxEffCharBoneListEff::LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice ,bool bTool,bool bRan2 )
{
	HRESULT hr;
	DWORD dwVer, dwSize;
	...
	if ( dwVer == VERSION )
	{
		...
		SFile.ReadBuffer( m_vRotate, sizeof(m_vRotate) );
		SFile.ReadBuffer( m_vVelocity, sizeof(m_vVelocity) );
	...
	{
		CString strTemp = "";
		strTemp.Format( "DxEffCharBoneListEff::LoadFile Error! Name:%s Version:%d Size:%d Current Version:%d" ,SFile.GetFileName(),dwVer,dwSize,VERSION);
...
HRESULT	DxEffCharBoneListEff::SaveFile ( basestream &SFile ,bool bTool)
{
	SFile << TYPEID;
	SFile << VERSION_SAVE;
