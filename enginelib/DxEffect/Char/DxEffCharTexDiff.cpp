...
DWORD		DxEffCharTexDiff::VERSION			= 0x0106;
DWORD		DxEffCharTexDiff::VERSION_SAVE		= 0x0101;
char		DxEffCharTexDiff::NAME[MAX_PATH]	= "EffCharTexDiff";
...
HRESULT	DxEffCharTexDiff::LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice ,bool bTool,bool bRan2 )
{
	HRESULT hr;
	DWORD dwVer, dwSize;
	...
	if ( dwVer == VERSION )
	{
		...
		{
			SFile >> m_dwFlag;
			SFile.ReadBuffer ( &m_strTex, sizeof(m_strTex) );
			SFile >> m_fSpeed;
			SFile >> m_dwMaterials;
			SFile >> m_dwIntensity;
		}
...
HRESULT	DxEffCharTexDiff::SaveFile ( basestream &SFile ,bool bTool)
{
	SFile << TYPEID;
	SFile << VERSION_SAVE;
