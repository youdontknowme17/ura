...
const DWORD	DxEffectRainPoint::VERSION_SAVE = 0x00000106;
const char DxEffectRainPoint::NAME[] = "DxEffectRainPoint";	// 벚꽃
...
void DxEffectRainPoint::SetProperty ( PBYTE &pProp, DWORD &dwSize, DWORD dwVer )
{
	...
	{
		CString strTemp = "";
		strTemp.Format( "DxEffectRainPoint::SetProperty Error! Version:%d Size:%d Current Version:%d" 		,dwVer
		,dwSize
		,VERSION
		);
...
void DxEffectRainPoint::LoadBufferSet ( CSerialFile &SFile, DWORD dwVer, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	{
		CString strTemp = "";
		strTemp.Format( "DxEffectRainPoint::LoadBufferSet Error! Name:%s Version:%d Size:%d Current Version:%d" ,SFile.GetFileName(),dwVer,dwBuffSize,VERSION);
		if ( CCrypt::bEngineDebug )  MessageBox (NULL,strTemp,"Need New Version",MB_OK);
		if ( CCrypt::bEngineDebugLog )  CDebugSet::ToFile ( "EngineLoadError.txt",strTemp );
		strTemp.Empty();
...
//	GASSERT(pd3dDevice);
	...
	{
		CString strTemp = "";
		strTemp.Format( "DxEffectRainPoint::LoadBuffer Error! Name:%s Version:%d Size:%d Current Version:%d" ,SFile.GetFileName(),dwVer,dwBuffSize,VERSION);
		if ( CCrypt::bEngineDebug )  MessageBox (NULL,strTemp,"Need New Version",MB_OK);
		if ( CCrypt::bEngineDebugLog )  CDebugSet::ToFile ( "EngineLoadError.txt",strTemp );
		strTemp.Empty();
