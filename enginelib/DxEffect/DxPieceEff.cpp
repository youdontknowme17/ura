...
const DWORD DxPieceEff::VERSION_SAVE	= 0x0102;
...
void DxPieceEff::LoadEdt( LPDIRECT3DDEVICEQ pd3dDevice, CSerialFile& SFile, DxFrameMesh* pFrameMesh )
{
	...
	{
		CString strTemp = "";
		strTemp.Format( "DxPieceEff::LoadEdt Error! Name:%s Version:%d Size:%d Current Version:%d" ,SFile.GetFileName(),dwVer,dwBufferSize,VERSION);
...
void DxPieceEff::Load( LPDIRECT3DDEVICEQ pd3dDevice, CSerialFile& SFile, DxAnimationMan* pAnimation )
{
	...
	{
		CString strTemp = "";
		strTemp.Format( "DxPieceEff::Load Error! Name:%s Version:%d Size:%d Current Version:%d" ,SFile.GetFileName(),dwVer,dwBufferSize,VERSION);
		if ( CCrypt::bEngineDebug )  MessageBox (NULL,strTemp,"Need New Version",MB_OK);
		if ( CCrypt::bEngineDebugLog )  CDebugSet::ToFile ( "EngineLoadError.txt",strTemp );
		strTemp.Empty();
