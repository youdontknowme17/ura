...
const DWORD	DxTexEffDiffuse::VERSION_SAVE = 0x00000101;
const char DxTexEffDiffuse::NAME[] = "DxTexEffDiffuse";
...
void DxTexEffDiffuse::LoadPSF( LPDIRECT3DDEVICEQ pd3dDevice, CSerialFile& SFile )
{
	...
	if( dwVer==0x00000103 )//official dummy
	{
		TextureManager::ReleaseTexture( m_sProp.strTex.c_str(), m_sProp.pTex );
	...
	{
		CString strTemp = "";
		strTemp.Format( "DxTexEffDiffuse::LoadPSF Error! Name:%s Version:%d Size:%d Current Version:%d" ,SFile.GetFileName(),dwVer,dwSize,VERSION);
