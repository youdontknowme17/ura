...
//add huiyi
void SMATERIAL_PIECE::SaveFileTool ( CSerialFile &SFile )
{
	SFile << VERSION_SAVE;
	SFile.BeginBlock();
	{
...
void SMATERIAL_PIECE::LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	else
	{
		...
		CString strTemp = "";
		strTemp.Format( "SMATERIAL_PIECE::LoadFile Error! Name:%s Version:%d Size:%d Current Version:%d" ,SFile.GetFileName(),dwVERSION,dwBLOCKSIZE,VERSION);
...
void SMATERIAL_PIECE::LoadFileTool ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
...
const DWORD	DxSkinPiece::VERSION		 = 0x00000117;
const DWORD	DxSkinPiece::VERSION_SAVE	 = 0x00000117;
...
//add huiyi
HRESULT DxSkinPiece::SetSkinMeshTool ( const char* szXFileName, const char* szSkeleton, LPDIRECT3DDEVICEQ pd3dDevice, const BOOL bThread )
{
	GASSERT(szXFileName);
	GASSERT(szSkeleton);
	//GASSERT(pd3dDevice);
...
HRESULT DxSkinPiece::LoadPiece( const char* szFile, LPDIRECT3DDEVICEQ pd3dDevice, const BOOL bThread )
{
		...
		if ( m_dwMaterialNum!=m_pmcMesh->GetNumMaterials() )
	{
		...
		for ( DWORD i=0; i<dwSize; i++ )
		{
			...
			hr = pEffChar->LoadFile ( SFile, pd3dDevice ,false,false);
			if (FAILED(hr))
			{
	...
	else
	{
		...
		CString strTemp = "";
		strTemp.Format( "DxSkinPiece::LoadPiece Error! Name:%s Version:%d Current Version:%d" ,SFile.GetFileName(),dwVer,VERSION);
		CDebugSet::ToFile ( "EngineLoadError.txt",strTemp );
		strTemp.Empty();
...
HRESULT DxSkinPiece::LoadPieceTool( const char* szFile, LPDIRECT3DDEVICEQ pd3dDevice, const BOOL bThread )
{
	HRESULT hr;
	...
	if ( dwVer == VERSION )
	{
		...
		hr = SetSkinMeshTool ( szXFileName, szSkeleton, pd3dDevice, bThread );
		SAFE_DELETE_ARRAY(szXFileName);
		SAFE_DELETE_ARRAY(szSkeleton);
		if ( FAILED(hr) )	return hr;
		...
		if ( m_dwMaterialNum!=0 )
		{
			...
			for ( DWORD i=0; i<m_dwMaterialNum; ++i )
			{
				m_pMaterialPiece[i].LoadFileTool ( SFile, pd3dDevice );
			}
		}
		...
		for ( DWORD i=0; i<dwSize; i++ )
		{
			...
			pEffChar->SetLinkObj ( NULL, this );
			hr = pEffChar->LoadFile ( SFile, pd3dDevice ,true,false);
			if (FAILED(hr))
			{
				SAFE_DELETE(pEffChar);
				continue;
...
HRESULT DxSkinPiece::LoadPieceToolRan2( const char* szFile, LPDIRECT3DDEVICEQ pd3dDevice, const BOOL bThread )
{
	HRESULT hr;
	...
	if ( dwVer == VERSION )
	{
		...
		for ( DWORD i=0; i<dwSize; i++ )
		{
			...
			pEffChar->SetLinkObj ( NULL, this );
			hr = pEffChar->LoadFile ( SFile, pd3dDevice ,true,true);
			if (FAILED(hr))
			{
				SAFE_DELETE(pEffChar);
				continue;
...
//	GASSERT(szFile);
	...
	for ( DWORD i=0; i<m_dwMaterialNum; ++i )
	{
		m_pMaterialPiece[i].SaveFileTool ( SFile );
	}
...
HRESULT DxSkinPiece::SavePieceTool ( const char* szFile )
{
//	GASSERT(szFile);
	DWORD	dwDummy;
	string strDummy;
