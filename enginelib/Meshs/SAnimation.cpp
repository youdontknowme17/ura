	...
	m_pAniScale(NULL)
{
	...
	memset( m_szBin, 0, sizeof(TCHAR)*ACF_SZNAME );
...
SANIMCONINFO& SANIMCONINFO::operator = ( SANIMCONINFO &value )
{
	...
	StringCchCopy( m_szBin, ACF_SZNAME, value.m_szBin );
...
HRESULT SAnimation::LoadFromFile ( CSerialFile &SFile, const DxSkeleton* pSkeleton )
{
	...
	else if ( 0x102==dwVER )	Load_0102( SFile, pSkeleton );
	else if ( 0x101==dwVER )	Load_0101( SFile, pSkeleton );
	else if ( 0x100==dwVER )	Load_0100( SFile, pSkeleton );
	else
	{
		CString strTemp = "";
		strTemp.Format( "SAnimation::LoadFromFile Error! Name:%s Version:%d Current Version:%d" ,SFile.GetFileName(),dwVER,VERSION);
...
HRESULT SAnimContainer::LoadAnimFromFile ( const TSTRING &strFilePath )
{
	...
	default:
		CString strTemp = "";
		strTemp.Format( "SAnimContainer::LoadAnimFromFile Error! Name:%s Version:%d Current Version:%d" ,SFile.GetFileName(),dwVersion,VERSION);
		if ( CCrypt::bEngineDebug )  MessageBox (NULL,strTemp,"Need New Version",MB_OK);
		if ( CCrypt::bEngineDebugLog )  CDebugSet::ToFile ( "EngineLoadError.txt",strTemp );
		strTemp.Empty();
