...
DWORD DxFrameMesh::VERSION_SAVE = 0x100;
...
BOOL DxFrameMesh::LoadSet ( CSerialFile &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
			...
			if ( bResult )
			{
				...
				if ( bResult )
				{
					...
					else
					{
						...
						CString strTemp = "";
						strTemp.Format( "DxFrameMesh::LoadSet D3DEXMATERIAL Error! Name:%s Version:%d Size:%d Current Version:%d" ,SFile.GetFileName(),dwVer,NODESIZE,D3DEXMATERIAL::VERSION);
	...
	while ( bResult )
	{
		...
		else
		{
			...
			CString strTemp = "";
			strTemp.Format( "DxFrameMesh::LoadSet DXPIECEOBJ Error! Name:%s Version:%d Size:%d Current Version:%d" ,SFile.GetFileName(),dwVer,NODESIZE,DXPIECEOBJ::m_dwVERSION);
			if ( CCrypt::bEngineDebug )  MessageBox (NULL,strTemp,"Need New Version",MB_OK);
			if ( CCrypt::bEngineDebugLog )  CDebugSet::ToFile ( "EngineLoadError.txt",strTemp );
			strTemp.Empty();
	...
	while ( bResult )
	{
		...
		else
		{
			...
			CString strTemp = "";
			strTemp.Format( "DxFrameMesh::LoadSet DxAnimationMan Error! Name:%s Version:%d Size:%d Current Version:%d" ,SFile.GetFileName(),dwVer,NODESIZE,DxAnimationMan::m_dwVERSION);
			if ( CCrypt::bEngineDebug )  MessageBox (NULL,strTemp,"Need New Version",MB_OK);
			if ( CCrypt::bEngineDebugLog )  CDebugSet::ToFile ( "EngineLoadError.txt",strTemp );
			strTemp.Empty();
			//CDebugSet::ToFile ( "LoadSet.log", "   [%x]...버전이 다릅니다.", dwVer );
...
void DxMeshes::SaveSet ( CSerialFile &SFile )
{
	if ( exMaterials )
	{
		...
		SFile << exMaterials->VERSION_SAVE;
        SFile << NodeSize;
		//	-->	버전과 사이즈, 버전이 다를 경우 노드사이즈만큼 스킵한다.
