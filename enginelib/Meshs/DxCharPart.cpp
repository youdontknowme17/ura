...
HRESULT DxCharPart::Render ( LPDIRECT3DDEVICEQ pd3dDevice, DxSkeleton* pSkeleton, SKINEFFDATA& sSKINEFFDATA, BOOL bShadow, BOOL bEff )
{
	...
	if ( !m_pmcMesh->pSkinInfo )
	{
		...
		case EMPEACE_WEAPON_RGUN_SLOT:
		case EMPEACE_WEAPON_LGUN_SLOT:
		case EMPEACE_WEAPON_GUN_SLOT:
		case EMPEACE_WEAPON_RSPEAR_SLOT:
		case EMPEACE_WEAPON_LSPEAR_SLOT:
...
void DxCharPart::RenderEff( LPDIRECT3DDEVICEQ pd3dDevice, DxSkeleton* pSkeleton, const float fScale )
{
	...
	if( !m_pmcMesh->pSkinInfo ) //add effcharparticle
	{
		pMatrix = &m_matCombine;
	}else{
	pMatrix = &pSkeleton->pBoneRoot->matCombined;
	}
