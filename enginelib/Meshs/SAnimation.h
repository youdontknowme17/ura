...
#include "../../EngineSoundLib/DxSound/CharacterSound.h"
...
struct SAnimation
{
	enum { VERSION = 0x0103,VERSION_BIN_INNER_SAVE = 0x0102, };
...
{
	enum { VERSION=0x0115,VERSION_CFGSAVE=0x0107 };

	typedef std::list<DxEffAniData*>				EFFANILIST;			
	typedef std::list<DxEffAniData*>::iterator		EFFANILIST_ITER;

	TCHAR			m_szName[ACF_SZNAME];		//	이름.
	TCHAR			m_szBin[ACF_SZNAME];	//	뼈다귀 이름.
	TCHAR			m_szSkeletion[ACF_SZNAME];	//	뼈다귀 이름.
	...
	BOOL LoadFileTool ( const TCHAR *szFileName, LPDIRECT3DDEVICEQ pd3dDevice=NULL );
...
{
public:
	enum { VERSION = 0x0104,VERSION_ANISAVE = 0x0102 };
