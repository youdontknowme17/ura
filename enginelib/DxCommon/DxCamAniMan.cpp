...
const DWORD DxCameraAni::VERSION_SAVE	 = 0x0101;
...
HRESULT DxCameraAni::LoadSet ( CSerialFile &SFile, DxFrameMesh *pFrameMesh )
{
	...
	{
		CString strTemp = "";
		strTemp.Format( "DxCameraAni::LoadSet Error! Name:%s Version:%d Size:%d Current Version:%d" ,SFile.GetFileName(),dwVer,dwSize,VERSION);
...
HRESULT DxCameraAni::Load ( CSerialFile &SFile )
{
	...
	else if ( dwVer==VERSION || dwVer==0x0100 )
	{
		DWORD dwLength;
		SFile >> dwLength;
		if ( dwLength )
	...
	{
		CString strTemp = "";
		strTemp.Format( "DxCameraAni::Load Error! Name:%s Version:%d Size:%d Current Version:%d" ,SFile.GetFileName(),dwVer,dwSize,VERSION);
		if ( CCrypt::bEngineDebug )  MessageBox (NULL,strTemp,"Need New Version",MB_OK);
		if ( CCrypt::bEngineDebugLog )  CDebugSet::ToFile ( "EngineLoadError.txt",strTemp );
		strTemp.Empty();
