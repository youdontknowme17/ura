...
DxViewPort::DxViewPort() :
	...
	m_fElapsedTime(0.f),
	m_fDefaultVELOMOVE( m_fVELOMOVE )//lev톝펃
{
...
HRESULT DxViewPort::SetGameCamera ()
{
	...
	m_fZOOMIN = 30.0f;
	//m_fZOOMOUT = 200.0f;
	if (RANPARAM::dwScreenZoom == 0 )
	{
		m_fZOOMOUT = 200.0f;
	}
	else if (RANPARAM::dwScreenZoom == 1 )
	{
		m_fZOOMOUT = 250.0f;
	}
	else if (RANPARAM::dwScreenZoom == 2 )
	{
		m_fZOOMOUT = 300.0f;
	}
	else
...
//add chat view item 
void DxViewPort::GetRctPos( RECT& rect )
{
