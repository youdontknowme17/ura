...
const	DWORD	DxClubMan::VERSION_SAVE		= 103;
...
void DxClubMan::LoadData( LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	{
		/*CString strTemp = "";
		strTemp.Format( "DxClubMan::LoadData Error! Name:%s Version:%d Current Version:%d" ,SFile.GetFileName(),dwVer,VERSION);
...
void DxClubMan::NewData()
{
	...
	// 이 이미지를 사용한다고 알린다.
	FILE* file = fopen ( strPathname.c_str(), "wb" );
	if ( !file )	return;
