...
void	DXLIGHT::LoadSet_EP9( CSerialFile &SFile )
{
	BOOL	bResult = FALSE;
	...
	if ( m_Light.Attenuation0 == 0.0f )
		m_Light.Attenuation0 = 1.0f;

	if ( m_Light.Attenuation1 == 0.0f )
...
void DxLightMan::Load ( CSerialFile &SFile, DWORD dwLandVer, BOOL bSETLAND )
{
	...
	else
	{
		...
		if( 0x00106==dwVersion )
		{
			SFile >> m_bLighting;
			SFile >> m_bNightAndDay;
			SFile >> m_bWhenDayPointOff;

			m_LtDirectNoon.LoadSet_EP9 ( SFile );
			m_LtDirectNight.LoadSet_EP9 ( SFile );

			DWORD dwLIGHTNUM(0);
			SFile >> dwLIGHTNUM;

			for ( DWORD dw=0; dw<dwLIGHTNUM; ++dw )
			{
				DXLIGHT* p = new DXLIGHT();
				p->LoadSet_EP9 ( SFile );

				p->pNext = m_pLtListHead;
				m_pLtListHead = p;
			}
		...
		{
			CString strTemp = "";
			strTemp.Format( "DxLightMan::Load Error! Name:%s Version:%d Current Version:%d" ,SFile.GetFileName(),dwVersion,VERSION);
