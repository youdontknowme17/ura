...
const	DWORD	DxEffectRain::VERSION_SAVE	=	0x103;
...
void	DxEffectRain::LoadSet ( CSerialFile &SFile )
{
	...
	if ( dwVer == 0x104 )
	{
		SFile >> m_Property.m_dwFaces;
		SFile >> m_Property.m_vColor;
		SFile >> m_Property.m_iHeight;
		SFile >> m_Property.m_fLength;
		SFile >> m_Property.m_fSpeed;
		SFile >> m_Property.m_iRadius;
		SFile >> m_Property.m_fLimit;
	}
	...
	{
		CString strTemp = "";
		strTemp.Format( "DxEffectRain::LoadSet Error! Name:%s Version:%d Size:%d Current Version:%d" ,SFile.GetFileName(),dwVer,dwBuffSize,VERSION);
...
const	DWORD	DxEffectCloud::VERSION_SAVE	=	0x100;
...
void	DxEffectCloud::LoadSet ( CSerialFile &SFile )
{
	...
	if ( dwVer == 0x101 ) //official dummy
	{
		SFile >> m_Property.m_bUse;
		...
		SFile >> m_Property.m_fWidth;

		StringCchCopy( m_Property.m_szTexture, MAX_PATH, szTexture );
	}
	else if ( dwVer == VERSION )
	{
	...
	{
		CString strTemp = "";
		strTemp.Format( "DxEffectCloud::LoadSet Error! Name:%s Version:%d Size:%d Current Version:%d" ,SFile.GetFileName(),dwVer,dwBuffSize,VERSION);
		if ( CCrypt::bEngineDebug )  MessageBox (NULL,strTemp,"Need New Version",MB_OK);
		if ( CCrypt::bEngineDebugLog )  CDebugSet::ToFile ( "EngineLoadError.txt",strTemp );
		strTemp.Empty();
