...
BOOL OBJOCTree::DynamicLoad( const LPDIRECT3DDEVICEQ pd3dDevice, CSerialFile& SFile, const BOOL bThread )
{
	...
	if ( bExist )
	{
		...
		{
			CString strTemp = "";
			strTemp.Format( "OBJOCTree::DynamicLoad Error! Name:%s Version:%d Current Version:%d" ,SFile.GetFileName(),dwVer,m_dwVERSION);
...
const DWORD	DxAABBOctree::VERSION_SAVE = 0x0100;
...
void DxAABBOctree::Load ( const LPDIRECT3DDEVICEQ pd3dDevice, CSerialFile& SFile, const BOOL bDynamicLoad, BOOL bPiece )
{
	...
	{
		CString strTemp = "";
		strTemp.Format( "DxAABBOctree::Load Error! Name:%s Version:%d Size:%d Current Version:%d" ,SFile.GetFileName(),dwVer,dwBufferSize,VERSION);
		if ( CCrypt::bEngineDebug )  MessageBox (NULL,strTemp,"Need New Version",MB_OK);
		if ( CCrypt::bEngineDebugLog )  CDebugSet::ToFile ( "EngineLoadError.txt",strTemp );
		strTemp.Empty();
...
const DWORD	DxSingleTexMesh::VERSION_SAVE = 0x0101;
...
void DxSingleTexMesh::Load ( const LPDIRECT3DDEVICEQ pd3dDevice, CSerialFile& SFile, const BOOL bDynamicLoad, BOOL bPiece )
{
	...
	{
		CString strTemp = "";
		strTemp.Format( "DxSingleTexMesh::Load Error! Name:%s Version:%d Size:%d Current Version:%d" ,SFile.GetFileName(),dwVer,dwBufferSize,VERSION);
		if ( CCrypt::bEngineDebug )  MessageBox (NULL,strTemp,"Need New Version",MB_OK);
		if ( CCrypt::bEngineDebugLog )  CDebugSet::ToFile ( "EngineLoadError.txt",strTemp );
		strTemp.Empty();
...
void DxSingleTexMesh::Load_EP9 ( const LPDIRECT3DDEVICEQ pd3dDevice, CSerialFile& SFile, const BOOL bDynamicLoad, BOOL bPiece )
{
	CleanUp();
