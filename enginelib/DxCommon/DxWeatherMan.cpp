...
const	DWORD	DxWeatherMan::VERSION_SAVE = 0x102;
...
DWORD DxWeatherMan::LoadSetProp ( CSerialFile &SFile, WEATHER_PROPERTY &sProperty )
{
	...
	if ( dwVer == 0x103 )
	{
		SFile >> sProperty.m_dwFlag;
		SFile >> sProperty.m_fWindPower;
		SFile >> sProperty.m_vWindDirec;
	}
	else if ( dwVer==VERSION && dwBuffSize==sizeof(WEATHER_PROPERTY) )
	{
...
void DxWeatherMan::LoadSet ( CSerialFile &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	{
		CString strTemp = "";
		strTemp.Format( "DxWeatherMan::LoadSet Error! Name:%s Version:%d Current Version:%d" ,SFile.GetFileName(),dwVer,VERSION);
