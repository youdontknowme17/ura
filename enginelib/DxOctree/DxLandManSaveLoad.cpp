...
#include "../../EngineSoundLib/DxSound/StaticSoundMan.h"
...
const DWORD	DxLandMan::VERSION_SAVE = 0x0114;
...
BOOL DxLandMan::SaveFile ( const char *szFile )
{
	...
	SFile.SetFileType ( (char*)FILEMARK, DxLandMan::VERSION_SAVE );
...
BOOL DxLandMan::LoadFile ( const char *szFile, LPDIRECT3DDEVICEQ pd3dDevice, BOOL bUseDynamicLoad )
{
	...
	BOOL bOPEN = m_SFileMap.OpenFile ( FOT_READ, szPathName );
	...
	{
		CString Str;
		Str.Format ( "[%s]  Invalid File", szFile );
		MessageBox ( NULL, Str.GetString(), "ERROR", MB_OK );
		return FALSE;
	}

	if ( dwVersion<0x0108 || dwVersion == 0x0115 || dwVersion > 0x0120  )
	{
		CString Str;
		Str.Format ( "[%s]  Invalid File Version:%d", szFile,  dwVersion );
		MessageBox ( NULL, Str.GetString(), "ERROR", MB_OK );
		return FALSE;
	}
	...
	else if( (dwVersion==0x0112) || (dwVersion==0x0111) )
	{
		return LoadFile_VER112( szFile, pd3dDevice, bUseDynamicLoad, szPathName );
	}
	else if( (dwVersion==0x0113) || (dwVersion==0x0114) )
	{
		return LoadFile_VER114( szFile, pd3dDevice, bUseDynamicLoad, szPathName );
	}
	else if( dwVersion==0x0116 || dwVersion==0x0117 )
	{
	...
	{
		return LoadFile_VER120( szFile, pd3dDevice, bUseDynamicLoad, szPathName );
	}
...
BOOL DxLandMan::LoadFile_VER120( const char *szFile, LPDIRECT3DDEVICEQ pd3dDevice, BOOL bUseDynamicLoad, const char* szPathName )
{
	...
	if ( bExist )
	{
		...
		m_pStaticMesh->Load( pd3dDevice, szPathName );
