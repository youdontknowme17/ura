...
namespace RANPARAM
{
	...
	BOOL				bSET_DISPLAY		= TRUE;			//	이름 항상 표시
	...
	BOOL				DoneOpen				= FALSE;
	BOOL				Link1Open_Done			= FALSE;
	BOOL				Link2Open_A_Done		= FALSE;
	BOOL				Link2Open_B_Done		= FALSE;
	BOOL				Link3Open				= FALSE;
	BOOL				Link3Open_Done			= FALSE;
	CString				UsedLink				= "Google.com";
	...
	DWORD				dwScreenZoom			= 1;

	BOOL				bShowRankName			= TRUE; //add pkrank
	BOOL				bShowRankMark			= FALSE; //add pkrank

	BOOL				bAllowBet				= TRUE; //add duel bet

	BOOL				bLockInven				= FALSE; //add lock
	BOOL				bLockSkill				= FALSE; //add lock
	BOOL				bLockPutOn				= FALSE; //add lock
	BOOL				bLockQSkill				= FALSE; //add lock
	...
	int					MenuShotcut[QUICK_MENU_NUM]		= { DIK_I, DIK_C, DIK_K, DIK_P, DIK_T, DIK_G, 
															...
															DIK_X, DIK_Z ,DIK_U, DIK_J, DIK_O, DIK_SEMICOLON, DIK_RBRACKET};
	...
	int					DefaultShotcutKey[QUICK_ALL_NUM]	= { DIK_Q, DIK_W, DIK_E, DIK_A, DIK_S, DIK_D,
																...
																DIK_U, DIK_J, DIK_O, DIK_SEMICOLON, DIK_RBRACKET };
	...
	int					KeySettingToRanparam[QUICK_MENU_NUM] = { SHOTCUT_INVEN, SHOTCUT_CHAR, SHOTCUT_SKILL, SHOTCUT_PARTY,
																 ...
																 SHOTCUT_HELP, SHOTCUT_PET, SHOTCUT_ATTACKMODE, SHOTCUT_PKMODE, SHOTCUT_SUMMON, SHOTCUT_COMPOUND, SHOTCUT_POINTSHOP};
	...
	int					BasicMenuToRanparam[BASICMENU_NUM] = { SHOTCUT_INVEN, SHOTCUT_CHAR,	SHOTCUT_SKILL, SHOTCUT_PARTY,
															   SHOTCUT_QUEST, SHOTCUT_CLUB, SHOTCUT_FRIEND, SHOTCUT_MAP,
															   SHOTCUT_ITEMBANK, SHOTCUT_RUN, SHOTCUT_ITEMSHOP, SHOTCUT_CHATMACRO, SHOTCUT_COMPOUND, SHOTCUT_POINTSHOP};
	
	CString				ChatMacro[CHAT_MACRO_NUM];

	BOOL				bCHECKWHQL			= TRUE;
	DWORD				dwScrWidth			= 800; //default resolution
	...
	TCHAR				GameName[STRING_NUM_128] = _T("RAN ONLINE");

	bool				bShadowFont = true;//add fontshadow
	bool				bSmallFont = false;//add fontshadow
	...
	BOOL SAVE ()
	{
...

		{
			...
			strBuffer.AppendFormat( "bHelpPopUp = %d\n", bHelpPopUp );//jdev help
			strBuffer.AppendFormat( "dwScreenZoom = %d\n", dwScreenZoom );//jdev help
			strBuffer.AppendFormat( "bIdleAnim = %d\n", bIdleAnim );

			strBuffer.AppendFormat( "bShowRankName = %d\n", bShowRankName ); //add pkrank
			strBuffer.AppendFormat( "bShowRankMark = %d\n", bShowRankMark ); //add pkrank	
		}
...

		{
			...
			strBuffer.AppendFormat( "bAllowBet = %d\n", bAllowBet ); //add duel bet
		}
...
//			return !strIDPARAM.empty() && !strIDPARAM2.empty() && !strIDPARAM3.empty();
	...
	void INIT_DATA()
	{
		...
		bLockInven			= FALSE; //add lock
		bLockSkill			= FALSE; //add lock
		bLockPutOn			= FALSE; //add lock
...
//		cFILE.reg_sep ( ' ' );
		...
		if( dwVER == EMGAMEOPT_VER ) // 채팅 매크로 버전
		{
			...
			cFILE.getflag ( "bPreviewStay", 1, 1, bPreviewStay );
			cFILE.getflag ( "bLockInven", 1, 1, bLockInven );//add lock
			cFILE.getflag ( "bLockSkill", 1, 1, bLockSkill );//add lock
			cFILE.getflag ( "bLockPutOn", 1, 1, bLockPutOn );//add lock
			cFILE.getflag ( "bLockQSkill", 1, 1, bLockQSkill );//add lock
			cFILE.getflag ( "bLockQTray", 1, 1, bLockQTray );//add lock

			cFILE.getflag ( "bVehicleRenderMove", 1, 1, bVehicleRenderMove );
			...
			cFILE.getflag ("SHOTCUT_COMPOUND", 1,1, MenuShotcut[SHOTCUT_COMPOUND] );
			cFILE.getflag ("SHOTCUT_POINTSHOP", 1,1, MenuShotcut[SHOTCUT_POINTSHOP] );
	...
	BOOL SAVE_GAMEOPT ()
	{
		...
		fprintf ( file, "bPreviewStay\t%d\n", bPreviewStay );
		fprintf ( file, "bLockInven\t%d\n", bLockInven );//add lock
		fprintf ( file, "bLockSkill\t%d\n", bLockSkill );//add lock
		...
		fprintf ( file, "SHOTCUT_COMPOUND\t%d\n", MenuShotcut[SHOTCUT_COMPOUND] );
		fprintf ( file, "SHOTCUT_POINTSHOP\t%d\n", MenuShotcut[SHOTCUT_POINTSHOP] );
	...
	BOOL LOAD_PARAM( const TCHAR *szRootPath )
	{
		...
		cFILE.getflag ( "ETC OPTION", "bEnableExitUrl", 0, 1, bEnableExitUrl );
		cFILE.getflag_sz( "ETC OPTION", "GameName", 0, 1, GameName, STRING_NUM_128 );
		cFILE.getflag ( "ETC OPTION", "bShadowFont", 1, 1, bShadowFont ); //add fontshadow
	...
	BOOL LOAD_OPT( const TCHAR *szRootPath, bool bServer )
	{
		...
		
		{
			...
			cFILE.getflag ( "SCREEN OPTION", "bHelpPopUp", 0, 1, bHelpPopUp );//jdev help
			cFILE.getflag ( "SCREEN OPTION", "dwScreenZoom", 0, 1, dwScreenZoom );
			cFILE.getflag ( "SCREEN OPTION", "bIdleAnim", 0, 1, bIdleAnim );

			cFILE.getflag ( "SCREEN OPTION", "bShowRankName", 0, 1, bShowRankName );  //add pkrank
			cFILE.getflag ( "SCREEN OPTION", "bShowRankMark", 0, 1, bShowRankMark );  //add pkrank
		}
...

		{
			...
			cFILE.getflag ( "GRAPHIC OPTION", "bAllowBet", 0, 1, bAllowBet ); //add duel bet
		}
