...
void MobEditAttack::UpdateCrow (int iIndex)
{
	...
	if (bisOK)
	{
		...
		DWORD DivCount = AnimInfo.m_wStrikeCount;
...
BOOL MobEditAttack::InverseUpdateCrow (int iIndex)
{
	...
	if (bisOK)//only save information if reading animation info is successful
	{
		...
		for ( int i=0; i< DivCount ; ++i )
		{
			...
			m_pDummyCrow->m_sCrowAttack[iIndex].sAniAttack.m_wDivFrame[i] = dwValue ;
		}	
	}
	else
	{
