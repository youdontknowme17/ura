...
const char* GARBAGE_DATA[] = { "ANAHI9K", "LUDI8L", "TESH7M", "TENG6N", "TOESADF5O"/*,
...
void* CRcvMsgBuffer::getMsg(bool bClient)
{
	...
	//		*/
			...
			else
			{
				...
				CDebugSet::ToFile ( "MinLzoErr.txt", "----%s----Packet Not LZO!",
				strTIME.c_str() );
		...
		else
		{
			...
			if ( nMoveSize <= 0 )
			{
				// whole packet didn't arrive yet
				::LeaveCriticalSection(&m_CriticalSection);
				return NULL;
			}
...
void* CRcvMsgBuffer::getOneMsg(bool bClient)
{
	...
	if (nOneMsgSize == 0 || nOneMsgSize > NET_DATA_BUFSIZE || nOneMsgSize < sizeof(NET_MSG_GENERIC))
	{
		resetPosition();
		return NULL;
	}
	...
	if( bClient )
	{
		...
		{		
			...
			if (dwBodySize<0 || dwBodySize > 8000) return NULL;
...
//	if (nMoveSize>=8000) return NULL; //crash fix test jdev
	if (nMoveSize > 0)
	{
