...
HRESULT CRanView::FrameMove()
{
	...
	DxEffMouseTarget_A::GetInstance().FrameMove( m_fTime, m_fElapsedTime );
...
HRESULT CRanView::Render()
{
	...
	if( SUCCEEDED( m_pd3dDevice->BeginScene() ) )
	{
		...
		if ( m_pSkinChar )
		{
			...
			DxEffMouseTarget_A::GetInstance().NewEffGroup ( GLCONST_CHAR::strARROW_EFFECT_NPC.c_str() , matTrans_a, &sTargetID );
