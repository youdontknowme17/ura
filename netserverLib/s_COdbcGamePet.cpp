	...
	DWORD dwClientID)
{
	...
	SQLINTEGER nPetDualSkill = 0, cbPetDualSkill = SQL_NTS;
	...
	while (true)
	{
		...
		if (sReturn == SQL_SUCCESS || sReturn == SQL_SUCCESS_WITH_INFO)
		{
			...
			::SQLGetData(pConn->hStmt, 16, SQL_C_LONG, &nPetDualSkill, 0, &cbPetDualSkill);
			...
			if(nPetDualSkill == 1)
				pPet->m_bDualSkill							= TRUE;
			else
				pPet->m_bDualSkill							= FALSE;
			// 프리미엄 만료기간
			// 1970-02-01 : Default
	...
	DWORD dwPetNum)
{
	...
	TCHAR szTemp[256] = {0};
	_snprintf( szTemp, 256, "SELECT PetInvenMID, PetInvenSID, PetInvenAvailable, PetInvenType "
							" FROM PetInven WHERE PetNum=%u"
							" AND (PetInvenType=%d OR PetInvenType=%d)"
							" AND PetChaNum=%d"
							" ORDER BY PetInvenNum",
							dwPetNum,
							DB_PET_INVEN_TYPE::SKILL,
							DB_PET_INVEN_TYPE::SKILL2,
							nChaNum);
	...
	SQLINTEGER nMainID = 0, cbMainID = SQL_NTS;
	SQLINTEGER nSubID = 0, cbSubID = SQL_NTS;
	SQLINTEGER nInvenType = 3, cbInvenType = SQL_NTS;
	while (true)
	{
		sReturn = ::SQLFetch(pConn->hStmt);
		if (sReturn == SQL_ERROR || sReturn == SQL_SUCCESS_WITH_INFO)
		...
		if (sReturn == SQL_SUCCESS || sReturn == SQL_SUCCESS_WITH_INFO)
		{
			...
			::SQLGetData(pConn->hStmt, 4, SQL_C_SHORT, &nInvenType, 0, &cbInvenType);
			...
			if (nAvailable == 1)
			{
				if (nInvenType == DB_PET_INVEN_TYPE::SKILL)
					pPet->m_sActiveSkillID = PetSkill.sNativeID;
				else if (nInvenType == DB_PET_INVEN_TYPE::SKILL2)
					pPet->m_sActiveSkillID2 = PetSkill.sNativeID;
...
int COdbcManager::SetPetDualSkill(int nChaNum, int nPetNum)
{	
	TCHAR szTemp[128] = {0};

	_snprintf( szTemp, 128, "UPDATE PetInfo SET PetDualSkill = 1 WHERE PetNum=%d AND PetChaNum=%d", nPetNum, nChaNum );
