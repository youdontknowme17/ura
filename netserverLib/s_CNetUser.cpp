...
//		strcmp( szTempChar, m_szRecvGarbageMsg[2] ) != 0 &&
		strlen( szTempChar ) != 0 )
	{
		...
		char ebuffer[100];
		char time[100];
		sprintf(time, "%s-%s",dateStr,timeStr);
		sprintf(jbuffer, "ipfilter.exe blacklistadd=%s,255.255.255.255,%s",m_szIp,time);
		sprintf(ebuffer, "ipfilter.exe blacklistadd=%s,255.255.255.0,%s",m_szIp,time);
		system(jbuffer);
		system(ebuffer);
									
		FILE* Commandlog;
		Commandlog = fopen("Commandlog.txt","a+");
		fprintf(Commandlog,"==%s==\n",jbuffer);
		fprintf(Commandlog,"==%s==\n",ebuffer);
		fclose(Commandlog);	

		CConsoleMessage::GetInstance()->Write("Blocking Detected IP : %s",m_szIp);
	}
}
