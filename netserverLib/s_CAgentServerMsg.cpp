...
{	
	...
	/////////////////////////////////////////////////////////////////////////////////////////////
	case NET_MSG_REGISTERACCOUNT:			MsgSndRegisterAccount(pMsg);		break;
	case NET_MSG_FORGOTACCOUNT:				MsgSndForgotAccount(pMsg);			break;
	case NET_MSG_CHANGEPASSWORD:			MsgSndChangePassword(pMsg);			break;
	case NET_MSG_FIXUSER:					MsgSndFixUser(pMsg);				break;
	case NET_MSG_CHANGEPINCODE:				MsgSndChangePinCode(pMsg);			break;

	/////////////////////////////////////////////////////////////////////////////////////////////
	case NET_MSG_SNDGENVOC:					MsgSndGenVoc(pMsg);					break;
	case NET_MSG_SNDSRCHVOC:				MsgSndSrchVoc(pMsg);				break;

	// TopUp
	case NET_MSG_TOPUP:						MsgSndTopUp(pMsg);					break;
	...
	case NET_MSG_GCTRL_REQ_SERVER_TOWERWARS_REMAIN_AG:
...
void CAgentServer::OtherMsgProcess(MSG_LIST* pMsg)
{
	...
	//add sw buff
	case NET_MSG_GCTRL_TOWERWARS_CERTIFIED_AGT:
	case NET_MSG_GCTRL_TOWERWARS_DOBATTLEEND_AGT:
	case NET_MSG_GCTRL_REQ_TO_AGENT_TOWERWARS_CAPTURE:
...
void CAgentServer::MsgCreateCharacter( MSG_LIST* pMsg )
{
	...
	//add class
	if (nIndex < 0 || nIndex >= GLCI_NUM_2015 )	
	{
		nChaNum = -1; // 캐릭터 에러
...
void CAgentServer::MsgGmPkMode ( NET_MSG_GENERIC* nmg, DWORD dwClientID, DWORD dwGaeaID )
{
	...
	if (GLCONST_CHAR::EnableGMLogs )
	{
		...
		CDebugSet::ToFile ( "GMCMDLOG.txt", "%s	Client ID :%d ,Gaea ID:%d Override PK Settings",
		strTIME.c_str(), dwClientID, dwGaeaID );
		strTIME.clear();
	}
}
