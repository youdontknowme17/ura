...
//add sw buff
int COdbcManager::GetSwRegion(std::vector<GLTWID> &vSWID_DB)
{
	SQLRETURN sReturn = 0;
	ODBC_STMT* pConn = m_pGameDB->GetConnection();
	if (!pConn) return DB_ERROR;
	...
	TCHAR szTemp[128] = {0};
	_snprintf( szTemp, 128, "SELECT SwID, SwNum FROM SwRegion WITH (NOLOCK) ORDER BY SwID" ); 
	...
	SQLINTEGER   nSwID  = 0; SQLINTEGER cbSwID  = SQL_NTS;
	SQLINTEGER   nSwNum = 0; SQLINTEGER cbSwNum = SQL_NTS;
	...
	while (true)
	{
		...
		{	
			...
			// Bind data			
			::SQLGetData(pConn->hStmt, 1, SQL_C_LONG,	&nSwID,  0, &cbSwID);
			::SQLGetData(pConn->hStmt, 2, SQL_C_LONG,	&nSwNum, 0, &cbSwNum);		
			
			sSWID_DB.m_dwID         = (DWORD) nSwID;
			sSWID_DB.m_dwTWID       = (DWORD) nSwNum;

			vSWID_DB.push_back(sSWID_DB);
		}
		else
		{
			break;
...
//add sw buff
int COdbcManager::SetSwRegion(DWORD dwSwID, DWORD dwSwNum)
{
	//std::strstream strTemp;
	//strTemp << "{call sp_add_guild_region(";
	//strTemp << dwRegionID   << ",";
 ...
 //   strTemp << fTax << ", ?)}";	
	...
	TCHAR szTemp[128] = {0};
	_snprintf( szTemp, 128, "{call sp_add_sw_region(%u,%u,?)}", dwSwID, dwSwNum );
...
int COdbcManager::SetClubBattle( DWORD dwClubP, DWORD dwClubS, DWORD dwEndTime, int nAlliance )
{
	...
	endTime += dwEndTime;
	CString strEndTime = endTime.Format( "%Y-%m-%d %H:%M:%S" );
	TCHAR szTemp[256] = {0};
	_snprintf( szTemp, 128, "{call sp_RequestGuBattle(%u,%u,'%s',%u,?)}", dwClubP, dwClubS, (LPCTSTR)strEndTime, nAlliance );
