...
//add Csv by CNDev
void CRanEditLevelView::OnBnClickedBtnCsvLoad()
{
	//
}

//add Csv by CNDev
void CRanEditLevelView::OnBnClickedBtnCsvSave()
{
	//
}
...
{
	if ( pCmdUI )	pCmdUI->SetCheck( ( pEffAdjust && bEditMRS /*&& m_DxEditMRS.SetMode()==OBECTMRS_M*/) );
}

void CRanEditLevelView::OnUpdateMousecontrolEffmatrixrotate(CCmdUI *pCmdUI)
{
	if ( pCmdUI )	pCmdUI->SetCheck( ( pEffAdjust && bEditMRS /*&& m_DxEditMRS.SetMode()==OBECTMRS_R*/) );
}

void CRanEditLevelView::OnUpdateMousecontrolEffmatrixscale(CCmdUI *pCmdUI)
{
	if ( pCmdUI )	pCmdUI->SetCheck( ( pEffAdjust && bEditMRS /*&& m_DxEditMRS.SetMode()==OBECTMRS_S*/) );
}
...
void CRanEditLevelView::OnFileOpenex()
{
	...
	BOOL bOK = m_pGLLandServer->LoadWldFile( strLEV.c_str(), strWLD.c_str() );
	if ( !bOK )	
	{
		SAFE_DELETE( m_pGLLandServer );
		SAFE_DELETE( m_pDxLandMan );
