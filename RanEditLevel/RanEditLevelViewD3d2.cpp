...
HRESULT CRanEditLevelView::Render()
{
	...
	if( SUCCEEDED( m_pd3dDevice->BeginScene() ) )
	{
		...
		//if ( m_pGLLandServer && m_pDxLandMan )
		{
			...
			{
				STARGETID sTargetID (CROW_MOB,0,m_vPos);
				D3DXMATRIX matTrans;
				D3DXMATRIX matScale;
				D3DXMATRIX matOut;
				D3DXMatrixTranslation ( &matTrans, m_vPos.x, m_vPos.y, m_vPos.z );
				D3DXMatrixScaling ( &matScale, 2.5f, 2.5f, 2.5f );
				matOut = matScale*matTrans;
//				DxEffMouseClick::GetInstance().NewEffGroup ( "add_target_blue.egp", matOut, &sTargetID );
			}
...
//			DxEffMouseTarget_A::GetInstance().Render ( m_pd3dDevice );
			...
			if ( bRenderCollisionSphere )
			{
				D3DXMATRIX matTrans;
				D3DXMatrixTranslation ( &matTrans, m_vPos.x, m_vPos.y, m_vPos.z );
//				EDITMESHS::RENDERSPHERE( m_pd3dDevice, &matTrans, 4.0, D3DXCOLOR( 0.0f, 0.5f, 1.0f, 1.0f) );
			}
