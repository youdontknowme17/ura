...
BEGIN_MESSAGE_MAP(CDlgCrowEdit, CPropertyPage)
	...
	ON_EN_CHANGE(IDC_EDIT_CROW_REGENMIN, OnEnChangeEditCrowRegenmin)
END_MESSAGE_MAP()
...
{
	STARGETID sTargetID (CROW_MOB,0,vPos);
	D3DXMATRIX matTrans, matScale, matOut;
	D3DXMatrixTranslation ( &matTrans, vPos.x, vPos.y, vPos.z );
	D3DXMatrixScaling ( &matScale, 4.0f, 4.0f, 4.0f );
	matOut = matScale*matTrans;
//	DxEffMouseTarget_B::GetInstance().NewEffGroup ( "add_target_green.egp", matOut, &sTargetID );
}
...
void CDlgCrowEdit::OnEnChangeEditCrowRegenmin()
{
