...
void GLAnyPet::ASSIGN ( PSDROPPET pDropPet )
{
	...
	m_bDualSkill		= pDropPet->m_bDualSkill;
...
HRESULT GLAnyPet::Create ( PSDROPPET pDropPet, NavigationMesh* pNavi, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	if ( m_sActiveSkillID2 != NATIVEID_NULL() )
	{
		...
		{
			STARGETID sTargetID( CROW_PC, pTarChar->m_dwGaeaID, pTarChar->GetPosition () );
			FACTEFF::NewSkillFactEffect ( sTargetID, pTarChar->GetCharSkin (), m_sActiveSkillID2, pTarChar->GetMatrix (), sTargetID.vPos );
		}
	}
	// SAD 액션중이면
...
HRESULT GLAnyPet::UpdateSuit ()
{
	...
	for ( WORD i = 0; i < PET_ACCETYPE_SIZE; ++i )
	{
		...
		{
			EMPIECECHAR emPiece = GetPieceFromSlot ( (PET_ACCESSORYTYPE)i );
...
void GLAnyPet::MsgSkillChange1 ( NET_MSG_GENERIC* nmg )
{
	GLMSG::SNETPET_REQ_SKILLCHANGE_BRD* pNetMsg = ( GLMSG::SNETPET_REQ_SKILLCHANGE_BRD* ) nmg;
...
void GLAnyPet::MsgSkillChange2 ( NET_MSG_GENERIC* nmg )
{
	GLMSG::SNETPET_REQ_SKILLCHANGE_BRD* pNetMsg = ( GLMSG::SNETPET_REQ_SKILLCHANGE_BRD* ) nmg;
	...
	if ( pTarChar )
	{
		FACTEFF::DeleteSkillFactEffect ( STARGETID(CROW_PC, pTarChar->m_dwGaeaID, pTarChar->GetPosition () ), pTarChar->GetCharSkin (), m_sActiveSkillID2 );
	}
	...
	{
		PGLSKILL pSkill = GLSkillMan::GetInstance().GetData ( m_sActiveSkillID2.wMainID, m_sActiveSkillID2.wSubID );
		if ( !pSkill )								return;
...
void GLAnyPet::MsgDualSkill( NET_MSG_GENERIC* nmg )
{
	GLMSG::SNETPET_REQ_DUAL_SKILL_BRD* pNetMsg = (GLMSG::SNETPET_REQ_DUAL_SKILL_BRD*) nmg;
	STARGETID sTargetID(CROW_PET, m_dwGUID, m_vPos);
	DxEffGroupPlayer::GetInstance().NewEffGroup
		(
		GLCONST_CHAR::strSKILL_LEARN_EFFECT.c_str(),
...
void GLAnyPet::MsgProcess ( NET_MSG_GENERIC* nmg )
{
	...
	case NET_MSG_PET_DUAL_SKILL_BRD:				MsgDualSkill(nmg);			break;
