...
BOOL CNpcDialogueSet::Load ( const char* szFileName )
{
	...
	default:
		{
			CString strTemp = "";
			strTemp.Format( "CNpcDialogueSet::Load Error! Name:%s Version:%d " ,SFile.GetFileName(),wVer );
			if ( CCrypt::bEngineDebug )  MessageBox (NULL,strTemp,"Need New Version",MB_OK);
			if ( CCrypt::bEngineDebugLog )  CDebugSet::ToFile ( "RanClientLoadError.txt",strTemp );
		}
	};
