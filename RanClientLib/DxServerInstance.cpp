...
#include "../EngineLib/Meshs/DxSkinPieceRootData.h"//add abl
#include "../EngineLib/Meshs/DxSkinVehicleData.h"//add vcf
...
namespace DxFieldInstance
{
					 ...
					 DWORD dwLangSet )
	{
		...
		GLTowerWarsFieldMan::GetInstance().Load ( GLCONST_CHAR::vecTW_FILE);
...
namespace DxAgentInstance
{
					 ...
					 DWORD dwLangSet )
	{
		...
		GLTowerWarsAgentMan::GetInstance().Load ( GLCONST_CHAR::vecTW_FILE );
		// Note : 자동 레벨 설정 파일 로드
		//
	...
	HRESULT FrameMove()
	{
		...
		GLTowerWarsAgentMan::GetInstance().FrameMove ( fElapsedAppTime );
		//	Note : 자동레벨 갱신
