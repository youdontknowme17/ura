...
HRESULT DxGlobalStage::ChangeStage ( WORD wWidth, WORD wHeight )
{
	...
	std::string strMapName;//add new loading
	...
	case EM_STAGE_GAME:
		{
			...
			strMapName = pMapNode->strMapName; //add new loading
		}
		break;
	};

	NLOADINGTHREAD::StartThreadLOAD( &m_pd3dDevice, m_hWnd, m_szFullPath, strLoadingImageName.c_str(), strMapName.c_str() );
...
void DxGlobalStage::MsgProcessFrame ( NET_MSG_GENERIC *nmg )
{
	...
	case NET_MSG_GCTRL_REQ_TYRANNY_GETREWARD_FB:
	...
	case NET_MSG_GCTRL_TOWERWARS_WINNER_PLAYER_UPDATE: 
...
//	case NET_MSG_GCTRL_SERVER_WINNER:
...
HRESULT DxGlobalStage::ToRestoreSize ()
{
	...
	{
		wndSizeX = 800; //default resolution
		wndSizeY = 600;
