...
HRESULT GLCharClient::Create ( GLLandManClient* pLandManClient, SDROP_CHAR* pCharData, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	m_sVehicle.m_wColor1 = pCharData->m_sVehicle.m_wColor1;
	m_sVehicle.m_wColor2 = pCharData->m_sVehicle.m_wColor2;
	m_sVehicle.m_wColor3 = pCharData->m_sVehicle.m_wColor3;
	m_sVehicle.m_wColor4 = pCharData->m_sVehicle.m_wColor4;
	m_sVehicle.m_wColor5 = pCharData->m_sVehicle.m_wColor5;
	...
	case GLAT_MOVE:
		{
			//	Note : ÀÌµ¿ ½ÃÅ°±â À§ÇØ ³»ºÎ Àü´Þ ¸Þ½ÃÁö ¹ß»ý.
			//
...
HRESULT GLCharClient::UpdateSuit ()
{
	...
	for ( int i=0; i<SLOT_NSIZE_S_2; i++ )
	{
		...
		if (nidITEM == SNATIVEID(false))
		{
			...
			if ( pSkinPiece ) //add abl
			{
				if( RANPARAM::bSET_DISPLAY )
				{
			...
			if ( pItem ) //add abl
			{
...

				{
					...
					else
					{
						...
						if ( pVehicleData )
						{
							m_pSkinChar->SetVehicleData( pVehicleData, m_pd3dDevice, TRUE );
				...
				{//add itemcolor
					...
					{
						m_pSkinChar->SetColor( SLOT_2_PIECE(EMSLOT(i)) , ItemClient.wColor );
					}
					else if ( EMSLOT(i) == SLOT_RHAND || EMSLOT(i) == SLOT_RHAND_S )
					{
						m_pSkinChar->SetColor( PIECE_RHAND , ItemClient.wColor );
						...
						if ( pRHAND )
						{
							...
							{
								m_pSkinChar->SetColor( PIECE_LHAND , ItemClient.wColor );
							}
						}	
					}
				}
...
void GLCharClient::TurnAction ( EMACTIONTYPE toAction )
{
	...
	default:
		GASSERT("GLCharacter::TurnAction() ÁØºñµÇÁö ¾ÊÀº ACTION ÀÌ µé¾î¿Ô½À´Ï´Ù.");
		break;
	};
...
HRESULT GLCharClient::UpateAnimation ( float fTime, float fElapsedTime )
{
	...
	//	Note : »óÅÂ ÀÌ»ó¿¡ µû¶ó ¸ð¼Ç ¼Óµµ¸¦ Á¶Á¤ÇÑ´Ù.
	//
...
HRESULT GLCharClient::Render ( LPDIRECT3DDEVICEQ pd3dDevice, CLIPVOLUME &cv, BOOL bRendAABB )
{
	...
	//	Note : m_bINVISIBLE °¡ ÂüÀÏ °æ¿ì ¹ÝÅõ¸íÇÏ°Ô Ç¥ÇöÇØ¾ßÇÔ.
	//
	if ( m_bINVISIBLE )
	{
	...
	//	TODO : m_bINVISIBLE °¡ ÂüÀÏ °æ¿ì ¹ÝÅõ¸íÇÏ°Ô Ç¥ÇöÇØ¾ßÇÔ.
...
void GLCharClient::MsgProcess ( NET_MSG_GENERIC* nmg )
{
	...
	case NET_MSG_GCTRL_INVEN_BIKECOLOR_CHANGE_BRD: //add bike color
		{
			...
			m_CharData.m_sVehicle.m_wColor1 = pNetMsg->wBikeColor1;
			m_CharData.m_sVehicle.m_wColor2 = pNetMsg->wBikeColor2;
			m_CharData.m_sVehicle.m_wColor3 = pNetMsg->wBikeColor3;
			m_CharData.m_sVehicle.m_wColor4 = pNetMsg->wBikeColor4;
			m_CharData.m_sVehicle.m_wColor5 = pNetMsg->wBikeColor5;		
			UpdateSuit();
		}
		break;
	...
	case NET_MSG_GCTRL_SLOT_ITEMCOLOR_CHANGE_BRD: //add itemcolor
		{
			GLMSG::SNETPC_SLOT_ITEMCOLOR_CHANGE_BRD *pNetMsg = (GLMSG::SNETPC_SLOT_ITEMCOLOR_CHANGE_BRD*)nmg;

			m_CharData.m_PutOnItems[pNetMsg->emSlot].wColor = pNetMsg->wItemColor;
			UpdateSuit();
		}
		break;
	...
	case NET_MSG_GCTRL_ACTIVE_VEHICLE_BRD:
		{
			...
			if ( pNetMsg->bActive )
			{
				...
				*/
				...
				// Äù¼Ç ¾ÆÀÌÅÛ Á¦°Å
				//m_CharData.sQITEMFACT.RESET ();	/*Disable remove buff and Qitem when use vehicle				
...
void GLCharClient::UPDATE_ITEM ( )
{
	...
	for ( int i=0; i<SLOT_NSIZE_S_2; i++ )
	{
		...
		// ÀÌµ¿¼Óµµ ·£´ý¿É¼Ç Àû¿ë( ÆÄÃ÷´Â À§¿¡¼­ Àû¿ë m_sVehicle.GetSpeedRate(); ) 
...
void GLCharClient::UpdateLandEffect()
{
	...
	for( int i = 0; i < EMLANDEFFECT_MULTI; i++ )
	{
		...
		if( landEffect.emLandEffectType == EMLANDEFFECT_CANCEL_ALLBUFF )
		{
			...
			//added by Jerm | 24-6-2012 | add summon skill
...
void GLCharClient::UPDATE_DATA ( float fTime, float fElapsedTime, BOOL bClient )
{
	...
	for ( int i=0; i<SKILLFACT_SIZE; ++i )
	{
		...
		for(int _i = 0; _i < SKILL::MAX_LEVEL; _i++ )
		{
			...
			case EMSPECA_MOVEVELO:
				m_fSKILL_MOVE += sSKEFF.fSPECVAR1[_i];
				break;
		...
		case EMSPECA_INVISIBLE:
			...
			case EMSPECA_ATTACKVELO:
				//	?? ?? ????? ( ?? ??? -0.1 (-10%) ?? ??? ???? ?? ??? ????. )
				m_fATTVELO += sSKEFF.fSPECVAR1[_i];
				break;

			case EMSPECA_GETEXP:
				m_fEXP_RATE += sSKEFF.fSPECVAR1[_i];
				break;
			...
			case EMSPECA_ATTACK_INT:
				m_gdDAMAGE_SKILL.VAR_PARAM (int (sSKEFF.fSPECVAR1[_i]));
				break;
			...
			case EMSPECA_DEF_INT:
				m_nDEFENSE += sSKEFF.fSPECVAR1[_i];
				break;
			...
			case EMSPECA_ACCURACY_RATE:
				m_nSUM_HIT += sSKEFF.fSPECVAR1[_i];
				break;

			case EMSPECA_EVASION_RATE:
				m_nSUM_AVOID += sSKEFF.fSPECVAR1[_i];
				break;
		...
		if ( !pRightItem || !CHECHSKILL_ITEM(pSkill->m_sBASIC.emUSE_RITEM,pRightItem->sSuitOp.emAttack) && pSkill->m_sBASIC.emIMPACT_REALM == REALM_SELF && pSkill->m_sBASIC.emIMPACT_TAR == TAR_SELF && pSkill->m_sBASIC.emUSE_RITEM != ITEMATT_NOCARE  )		sSKEFF.fAGE -= sSKEFF.fAGE;
	...
	//	°ø°Ý/ÀÌµ¿ ¼Óµµ. ( Áö¼Ó skill ¼Ó¼º °¡»êµÈ ºÎºÐ¿¡ state, passive skill ¼Ó¼º °¡»ê. )
