...
HRESULT GLPetField::UpdateClientState ( float fElapsedTime )
{
	...
	if ( m_fTIMER > 3600.0f/GLPeriod::REALTIME_TO_VBR )
	{
		...
		{
			m_nFull -= (GLCONST_PET::nFullDecrement[m_emTYPE] * 1.8);
		}
		else
		{
...
NET_MSG_GENERIC* GLPetField::ReqNetMsg_Drop ()
{
	...
	NetMsg.Data.m_bDualSkill		= m_bDualSkill;
...
{
	if ( (m_sActiveSkillID == NATIVEID_NULL ()) && (m_sActiveSkillID2 == NATIVEID_NULL())) return;
	...
	if ( m_pOwner->m_sPETSKILLFACT.fAGE < 0.0f )
	{
		...
		NetMsg.sNATIVEID = m_sActiveSkillID;
		NetMsg.dSkillOrder == EMPET_SKILL_ORDER_FIRST;
		GLGaeaServer::GetInstance().SENDTOCLIENT ( m_pOwner->m_dwClientID, &NetMsg );
	...
	if (m_pOwner->m_sPETSKILLFACT2.fAGE < 0.0f)
	{
		if ( m_pOwner->m_sPETSKILLFACT2.emTYPE == SKILL::EMFOR_PET_PROTECTITEMS )
		{
			m_pOwner->m_bProtectPutOnItem = false;
		}
		...
		GLMSG::SNETPET_REMOVE_SKILLFACT NetMsg;
		NetMsg.sNATIVEID = m_sActiveSkillID2;
		NetMsg.dSkillOrder = EMPET_SKILL_ORDER_SECOND;
		GLGaeaServer::GetInstance().SENDTOCLIENT ( m_pOwner->m_dwClientID, &NetMsg );
