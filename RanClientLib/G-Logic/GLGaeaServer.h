...
#include "../../NetServerLib/s_CSMsgList.h"
...
struct SCLASSEVENTDATA
{
	...
	//add class
	float fItemGainRate[GLCI_NUM_2015];
	float fExpGainRate[GLCI_NUM_2015];
	float fMoneyGainRate[GLCI_NUM_2015];

	DWORD dwExpMinLevel[GLCI_NUM_2015];
	DWORD dwExpMaxLevel[GLCI_NUM_2015];
	DWORD dwItemMinLevel[GLCI_NUM_2015];
	DWORD dwItemMaxLevel[GLCI_NUM_2015];
	DWORD dwMoneyMinLevel[GLCI_NUM_2015];
	DWORD dwMoneyMaxLevel[GLCI_NUM_2015];
...
//add sw buff
struct SSCHOOLEVENTDATA
{
	float fItemGainRate[GLSCHOOL_NUM];
	float fExpGainRate[GLSCHOOL_NUM];
	float fMoneyGainRate[GLSCHOOL_NUM];
...
public:
	TOWERWARS_SCHOOL_WINNER             m_sTowerWarsWinner;
	STWP_RANK                           m_sTowerWarRankWinner[RANKING_TWNUM];
	WORD								m_wRankNum;
	STWP_RANK_EX                        m_pMyScoreRank;
	
	WORD	m_fNextBattleStart;
	WORD	m_fNextBattleEnd;
	WORD	m_fPreviousBattleStart;
	WORD	m_fPreviousBattleEnd;
	WORD	m_fProgressTime;

	BOOL	ReqSetNextWarTime ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_SCHOOLWAR_NEXTWAR_REQ* pNetMsg );
...
public:
	bool					m_bParticipate;
	bool					m_bRejoin; //BattleFieldUI Data Rejoin - Eduj
...
protected:
	...
	//add sw buff
	SSCHOOLEVENTDATA		m_SchoolEventData;
...
public:
	...
	//add sw buff
    float					GetExpGainRate( WORD wSchool );
	float					GetItemGainRate( WORD wSchool );
	float					GetMoneyGainRate( WORD wSchool );
...
public:
	...
	bool IsSchoolWarsStarted()			{ return m_bSchoolWarsStarted; }
	bool IsRoyalRumbleStarted()			{ return m_bRoyalRumbleStarted; }
	bool IsTowerWarsStarted()			{ return m_bTowerWarsStarted; }
...
public:
	...
	BOOL		RequestViewProperties ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_VIEW_PROPERTIES *pNetMsg );//촥л콜㈙
	...
	BOOL	ReqTowerWarsCTFRanking ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_TOWERWARS_CTFRANKING_REQ* pNetMsg );
	BOOL	ReqCtfIconID ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_TOWERWARS_CTFICON_REQ* pNetMsg );
	BOOL	ReqTowerWarsWinner ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_TOWERWARS_WINNER_REQ* pNetMsg );

	BOOL ReqCrowList ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNETPC_REQ_CROWLIST_FLD *pNetMsg );
...
public:
	...
	BOOL ServerTowerWarsReady	( GLMSG::SNET_TOWERWARS_READY_FLD *pNetMsg );
	BOOL ServerTowerWarsStart	( GLMSG::SNET_TOWERWARS_START_FLD *pNetMsg );
	BOOL ServerTowerWarsEnd	( GLMSG::SNET_TOWERWARS_END_FLD *pNetMsg );
	BOOL ServerTowerWarsCertify ( GLMSG::SNET_TOWERWARS_CERTIFIED_FLD *pNetMsg );
	BOOL ServerTowerWarsDoBattleEnd ( GLMSG::SNET_TOWERWARS_DOBATTLEEND_FLD *pNetMsg );
	...
	BOOL GlobalUpgrade ( GLMSG::SNET_GLOBAL_SUCCESS_GRIND*pNetMsg );
...
public:
	...
	BOOL CtrolMove2MapPos ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_MOVE2MAPPOS_FLD *pNetMsg );
	...
	BOOL CTFRejoin ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_PVP_REJOIN* pNetMsg ); //BattleFieldUI Data Rejoin - Eduj

	BOOL CTFParticipate ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_TYRANNY_PARTICIPATE *pNetMsg );
	...
	//add sw buff
	BOOL GMCtrolSchoolEvent   ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_GM_SCHOOL_EVENT *pNetMsg );
...
public:
	...
	void SENDTOSCHOOLCLIENT_ONMAP ( DWORD dwMapID, WORD wSchoolID, LPVOID nmg );
	void SENDTOPLAYERCLIENT_ONMAP ( DWORD dwMapID, DWORD m_dwCharID, LPVOID nmg );
...
inline void GLGaeaServer::SENDTOALLCHANNEL ( LPVOID nmg )
{
	if ( m_pMsgServer ) m_pMsgServer->SendAllChannel ( nmg );
}
