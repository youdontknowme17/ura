...
#include "../../RanClientUILib/Interface/PvpWindow/CPvpWindowDisplay.h"
//#include <strstream>
...
HRESULT	GLGaeaClient::CreatePET ( NET_MSG_GENERIC* nmg )
{
	...
	sPet.m_sActiveSkillID2  = pNetMsg->m_sActiveSkillID2;
	...
	if ( m_Pet.m_sActiveSkillID2 != NATIVEID_NULL() )
	{
		m_Pet.ReqChangeActiveSkill2 ( m_Pet.m_sActiveSkillID2 );
	}
...
HRESULT GLGaeaClient::MoveActiveMap ( NET_MSG_GENERIC *nmg )
{
	...
	NLOADINGTHREAD::StartThreadLOAD( &m_pd3dDevice, m_hWnd, m_szAppPath, pMapNode->strLoadingImageName.c_str(), pMapNode->strMapName.c_str() );
...
HRESULT GLGaeaClient::ReCallFB ( NET_MSG_GENERIC* nmg )
{
	...
	if ( pNetMsg->emFB!=EMREQ_RECALL_FB_OK )
	{
		...
		case EMREQ_RECALL_FB_IMMOVABLE:
			...
			return S_OK;
		case EMREQ_RECALL_CARD_ITEM:
			CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_RECALL_FB_NOUSE_CARD") );
			return S_OK;
		};
	}
...
HRESULT GLGaeaClient::FrameMove ( float fTime, float fElapsedTime )
{
	...
	if ( m_bTowerWars )
	{
		...
		m_fTWDurationTime += fElapsedTime;

		if ( m_fAge > 1.0f ) CInnerInterface::GetInstance().UpdateClubBattleTime2( m_fTowerWarsTimer , true );
	}
	else
	{
		if ( m_fAge > 1.0f ) CInnerInterface::GetInstance().UpdateTyrannyWarNextBattle( m_fProgressTime, m_fNextBattleStart, m_fNextBattleEnd, m_fPreviousBattleStart, m_fPreviousBattleEnd );
	}
 	...
 	if ( pLand && pLand->m_bTowerWars )
	{
		if ( m_bBuffPlace )
		{
			m_fBuffPlace += fElapsedTime;

			if ( m_fBuffPlace >= 15.0f )
			{
				CInnerInterface::GetInstance().PrintConsoleTextDlg ( "Thank you for participating!." );
				//Send all participants in practicing yard...
...
void GLGaeaClient::MsgProcess ( NET_MSG_GENERIC* nmg )
{
	...
	case NET_MSG_TYRANNY_PARTICIPATE_BRD:
		...
		break;
	case NET_MSG_GCTRL_TOWERWARS_SERVER_WINNER:
		{
			GLMSG::SNETPC_TOWERWARS_SERVER_WINNER * pNetMsg = (GLMSG::SNETPC_TOWERWARS_SERVER_WINNER*)nmg;
			m_nCTFWinner = pNetMsg->sTwWinner.wSCHOOL;
			m_TowerWarsWinner = pNetMsg->sTwWinner;

			int nRankNum = pNetMsg->wRankNum;

			for ( int i = 0; i < nRankNum; ++i )
			{
				int nIndex = pNetMsg->sPlayerWinner[i].nIndex;
				if ( nIndex < 0 ) continue;

				int nSize = m_TowerWarsWinnerVec.size();				
				if ( nIndex < nSize )
				{
					m_TowerWarsWinnerVec[nIndex] = pNetMsg->sPlayerWinner[i];
				}
				else
				{
					m_TowerWarsWinnerVec.resize( nIndex+1 );
					m_TowerWarsWinnerVec[nIndex] = pNetMsg->sPlayerWinner[i];
				}
			}


			if ( m_Character.GETSCHOOL() == m_nCTFWinner )
			{
				CInnerInterface::GetInstance().SetSwBonus( true, false );
	
		  }else{
			CInnerInterface::GetInstance().SetSwBonus( false, false );
			}
		}
		break;
	...
	case NET_MSG_GCTRL_REQ_DISABLESKILLEFF_BRD:
	case NET_MSG_GCTRL_REQ_LINKSKILLEFF_BRD: //Link Skill Effect by NaJ
	...
	case NET_MSG_GCTRL_SLOT_ITEMCOLOR_CHANGE_BRD: //add itemcolor
	...
	case NET_MSG_GCTRL_INVEN_GMITEM_FB: //add item cmd
	...
	//Sealed Card
	case NET_MSG_REBUILD_SEALED_CARD:
	...
	case NET_MSG_REQ_CLUBWAR_REWARD_CLT:	//add clubwar reward
	...
	case NET_MSG_GCTRL_REQ_LINKSKILLEFF_CROW_BRD:
	...
	case NET_MSG_GM_EVENT_EXP_FB:
		{
			...
			CInnerInterface::GetInstance().PrintConsoleText ( "Experience Event Start, Field Server [%d] Experience scale : %f ",
				pNetMsg->wFIELDSVR,
				fSCALE );
		}
		break;
	...
	case NET_MSG_GM_EVENT_ITEM_GEN_FB:
		{
			...
			CInnerInterface::GetInstance().PrintConsoleText ( "Item Drop Event Start scale : %", fSCALE );
		}
		break;
	...
	case NET_MSG_GM_CLASS_EVENT:
		{
			...
			switch(pNetMsgFB->dwClassIndex)
			{
				...
				//case GLCI_NUM_NEWSEX:  strcpy( szClassChar, "ALL:" ); break;
				//add class
				case GLCI_SCIENCE_M:  strcpy( szClassChar, "GUNNER_M:" ); break;
				case GLCI_SCIENCE_W:  strcpy( szClassChar, "GUNNER_W:" ); break;
				case GLCI_ASSASIN_M:  strcpy( szClassChar, "ASSASIN_M:" ); break;
				case GLCI_ASSASIN_W:  strcpy( szClassChar, "ASSASIN_W:" ); break;
				case GLCI_TESTING_M:  strcpy( szClassChar, "TESTING_M:" ); break;
				case GLCI_TESTING_W:  strcpy( szClassChar, "TESTING_W:" ); break;
				case GLCI_SHAPER_M:  strcpy( szClassChar, "SHAPER_M:" ); break;
				case GLCI_SHAPER_W:  strcpy( szClassChar, "SHAPER_W:" ); break;

				case GLCI_NUM_2015:  strcpy( szClassChar, "ALL:" ); break;
					//
			}
	...
	case NET_MSG_GM_SCHOOL_EVENT:
		{
			...
			char szTypeChar[6] = {0,};
			if  ( pNetMsgFB->m_wSchool == -1 )	{
				CInnerInterface::GetInstance().PrintConsoleTextDlg ( "Reward buff started to none!" );
				break;}
			...
			if( pNetMsgFB->bStart )
			{
				CInnerInterface::GetInstance().PrintConsoleText ( "Reward Buff Start! : Type(%s) School(%s) Rate(%.2f)", 
																	szTypeChar, szSchoolChar, pNetMsgFB->fEventRate );
			}/*(else{
				CInnerInterface::GetInstance().PrintConsoleText ( "Reward Buff End! : Type(%s) School(%s) Rate(%.2f)", 
					szTypeChar, szSchoolChar, pNetMsgFB->fEventRate );
			}*/
	...
	case NET_MSG_PVP_REJOIN_BRD:
		{
			...
			if ( m_bRejoin )
	           ...
	           if ( pLandMClient->GetMapID() != sMarKetID  )
			   {
				...
				CInnerInterface::GetInstance().PrintConsoleText ( "You are not in market area. moving to marketplace" );
				}else{
				GLMSG::SNET_GM_MOVE2MAPPOS NetMsg;
				NetMsg.nidMAP.wMainID = 22;
				NetMsg.nidMAP.wSubID = 0;
	...
	case NET_MSG_GM_VIEWALLPLAYER_FLD_FB:
		...
		case NET_MSG_GCTRL_VIEW_PROPERTIES_FB://촥л콜㈙
		{
			m_Character.MsgProcess ( nmg );
		}
		break;
	...
	case NET_MSG_GCTRL_TOWERWARS_START_BRD:
		{
			GLMSG::SNET_TOWERWARS_START_BRD *pNetMsg = (GLMSG::SNET_TOWERWARS_START_BRD *)nmg;	
         	if ( m_bParticipate )
				...
				m_nCTFWinner = -1;
				m_vecCtfRank.clear();
				m_TowerWarsWinnerVec.clear();
				m_TowerWarsWinner.Init();
				m_sMyCtfRank.Init();
	...
	case NET_MSG_GCTRL_TOWERWARS_END_BRD:
		{
			...
			PLANDMANCLIENT pLand = GLGaeaClient::GetInstance().GetActiveMap();
			ReqSchoolWarWinnerInfo();
			m_bParticipate = false;
			if ( pLand && pLand->m_bTowerWars )
			{
				CInnerInterface::GetInstance().PrintConsoleText( "Wait for 15 seconds to automatically transfer in MarketPlace. Thank you for participating." );
				m_bBuffPlace = true;
				m_fBuffPlace = 0.0f;
			}
			if ( pNetMsg->wSCHOOL != -1 )	
			CInnerInterface::GetInstance().PrintConsoleTextDlg ( "%s War Reward buff is activated to %s", pNetMsg->szName, pNetMsg->szClubName );
			else CInnerInterface::GetInstance().PrintConsoleTextDlg ( "%s War Ended with a draw reward buff is activated to %s", pNetMsg->szName, pNetMsg->szClubName );

			CInnerInterface::GetInstance().PrintConsoleTextDlg ( "Tyranny Wars Ended!" );

            m_Character.ReqSetTyrannyWin(pNetMsg->wSCHOOL);
			m_bTowerWars = false;
			m_fTowerWarsTimer = 0.0f;

			m_vecSwTower.clear();
			//removed, not neede too much windows open when sw ends.
	...
	case NET_MSG_GCTRL_TOWERWARS_CTFRANKING_UPDATE:
		{
			...
			case EMCTF_RANKING_UPDATE_FB_ADD:
				{
					int nIndex = pNetMsg->sSwPRank.nIndex;
					int nSize = m_vecCtfRank.size();

					if ( nIndex < nSize )
					{
						m_vecCtfRank[nIndex] = pNetMsg->sSwPRank;
					}
					else
					{
						m_vecCtfRank.resize( nIndex+1 );
						m_vecCtfRank[nIndex] = pNetMsg->sSwPRank;
					}
	...
	case NET_MSG_GCTRL_TOWERWARS_CAPTURE_BRD:
		{
			...
			CInnerInterface::GetInstance().SET_TYRANNY_NOTICE_ID( EMCTFSTREAK( pNetMsg->emHOLDER ) );
	...
	case NET_MSG_GCTRL_SERVER_TOWERWARS_INFO:
		...
		break;
	case NET_MSG_GCTRL_TOWERWARS_WINNER_PLAYER_UPDATE: 	// generate TowerWars school winner Eduj kun
		{
			GLMSG::SNET_TOWER_WINNER_UPDATE *pNetMsg = (GLMSG::SNET_TOWER_WINNER_UPDATE*)nmg;	

			int nRankNum = pNetMsg->wRankNum;

			for ( int i = 0; i < nRankNum; ++i )
			{
				int nIndex = pNetMsg->sRank[i].nIndex;
				if ( nIndex < 0 ) continue;

				int nSize = m_TowerWarsWinnerVec.size();				
				if ( nIndex < nSize )
				{
					m_TowerWarsWinnerVec[nIndex] = pNetMsg->sRank[i];
				}
				else
				{
					m_TowerWarsWinnerVec.resize( nIndex+1 );
	...
	case NET_MSG_GCTRL_LAND_INFO:
		{
			...
			if ( pLand && pLand->GetMapID()==pNetMsg->nidMAP )
			{
				...
				else if ( !pLand->m_bTowerWars )
				{
					...
					m_vecSwTower.clear();
					m_sMyCtfRank.Init();
                if ( m_TowerWarsWinnerVec.empty() ) ReqSchoolWarWinnerInfo();					
	...
	case NET_MSG_GM_WHERE_NPC_FB:
		{
			...
			CInnerInterface::GetInstance().PrintConsoleText ( "Crow Position At : %d, %d", pNetMsg->nPosX, pNetMsg->nPosY );
		}
		break;
	...
	case NET_MSG_GM_CHAR_INFO_FLD_FB:
		{
			...
			CInnerInterface::GetInstance().PrintConsoleText ( "cp           : %d/%d", pNetMsg->sCP.dwNow, pNetMsg->sCP.dwMax );
	...
	case NET_MSG_GCTRL_SERVERTEST_FB:
		{
			...
			m_Character.m_sPETSKILLFACT2.RESET();
			...
			FACTEFF::DeleteSkillFactEffect ( STARGETID(CROW_PC,m_Character.m_dwGaeaID,m_Character.GetPosition() ), m_Character.GetSkinChar (), m_Pet.m_sActiveSkillID2 );
			m_Pet.DeleteDeviceObjects ();
		}
		break;
	...
	case NET_MSG_AP_DT_BRD: //add autopots detection
		{
			GLMSG::SNET_AP_DT_BRD *pNetMsg = ( GLMSG::SNET_AP_DT_BRD* ) nmg;
			switch ( pNetMsg->emFB )
			{
			 case DETECTED:
			    CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, "'%s' Detected Using Third Party Auto Heal Program. User will be automatically disconnected after 10sec.",  pNetMsg->szName );
             break;
			}
		}
		break;	
...
//----------------------------------------------------------------------------------------------------------------
	...
	case NET_MSG_PET_REQ_PETCARDINFO_FB:
		{
			...
			sPetCardInfo.m_sActiveSkillID2 = pNetMsg->sActiveSkillID2;
	...
	case NET_MSG_PET_DUAL_SKILL_FB:
	case NET_MSG_PET_DUAL_SKILL_UPDATE_FB:
		{
			if ( m_Pet.IsVALID () )
			{
	...
	case NET_MSG_PET_DUAL_SKILL_BRD:
		{
...
DWORD GLGaeaClient::GetMobNameColor ( DWORD dwGlobID )
{
	...
	if ( pCROW->m_emOWNER == GetCharacter()->m_wSchool )
		return dwCOLOR;
	if ( pCROW->m_pCrowData->m_emCrow == CROW_MOB )
	{
		...
		else if ( wMOB_LEVEL-wMY_LEVEL >= GLCONST_CHAR::wMOB_COLOR_8) dwCOLOR = CROWCOLOR::MOB_COLOR_8;
		else if ( wMOB_LEVEL-wMY_LEVEL >= GLCONST_CHAR::wMOB_COLOR_7) dwCOLOR = CROWCOLOR::MOB_COLOR_7;
		else if ( wMOB_LEVEL-wMY_LEVEL >= GLCONST_CHAR::wMOB_COLOR_6) dwCOLOR = CROWCOLOR::MOB_COLOR_6;
		else if ( wMOB_LEVEL-wMY_LEVEL >= GLCONST_CHAR::wMOB_COLOR_5) dwCOLOR = CROWCOLOR::MOB_COLOR_5;
		else if ( wMOB_LEVEL-wMY_LEVEL >= GLCONST_CHAR::wMOB_COLOR_4) dwCOLOR = CROWCOLOR::MOB_COLOR_4;
		else if ( wMOB_LEVEL-wMY_LEVEL >= GLCONST_CHAR::wMOB_COLOR_3) dwCOLOR = CROWCOLOR::MOB_COLOR_3;
		else if ( wMOB_LEVEL-wMY_LEVEL >= GLCONST_CHAR::wMOB_COLOR_2) dwCOLOR = CROWCOLOR::MOB_COLOR_2;
		else if ( wMOB_LEVEL-wMY_LEVEL >= GLCONST_CHAR::wMOB_COLOR_1) dwCOLOR = CROWCOLOR::MOB_COLOR_1;
		else dwCOLOR = CROWCOLOR::MOB_COLOR_0;

	}
	else if (pCROW->m_pCrowData->m_emCrow == CROW_NPC  )
	{
		dwCOLOR = CROWCOLOR::MOB_COLOR_ORANGE;
	}

	//add mobschoolreq
	else if ( !(pCROW->m_pCrowData->m_sAction.dwMobReqSchool & index2school(m_Character.m_wSchool) ) ) 
	{
	...
	else
	{
		...
		if ( fEXP_RATE==0 )		dwCOLOR = CROWCOLOR::MOB_COLOR_WHITE;
			//else					dwCOLOR = CROWCOLOR::MOB_COLOR_RED;
		else if ( wMOB_LEVEL-wMY_LEVEL >= GLCONST_CHAR::wMOB_COLOR_8) dwCOLOR = CROWCOLOR::MOB_COLOR_8;
		else if ( wMOB_LEVEL-wMY_LEVEL >= GLCONST_CHAR::wMOB_COLOR_7) dwCOLOR = CROWCOLOR::MOB_COLOR_7;
		else if ( wMOB_LEVEL-wMY_LEVEL >= GLCONST_CHAR::wMOB_COLOR_6) dwCOLOR = CROWCOLOR::MOB_COLOR_6;
...
HRESULT GLGaeaClient::ReqSchoolWarWinnerInfo()
{
    PLANDMANCLIENT pLand = GetActiveMap();
	if ( !pLand ) return E_FAIL;

	GLMSG::SNET_TOWERWARS_WINNER_REQ NetMsg;
	NetMsg.dwMapID = pLand->GetMapID().dwID;
	NETSENDTOFIELD ( &NetMsg );
