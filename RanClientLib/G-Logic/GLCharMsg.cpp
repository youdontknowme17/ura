...
inline HRESULT GLChar::MsgAttack ( NET_MSG_GENERIC* nmg )
{
	...
	if ( pTARGET->GetCrow()==CROW_PC )
	{
		...
		if ( pCHAR )
		{
			...
			{
				GLTowerWars* pTW = GLTowerWarsFieldMan::GetInstance().Find( m_pLandMan->m_dwTowerWarsMapID );
				if ( pTW && pTW->IsBattle() ) bTowerWars = true;
			}

			if ( !(bGuidBattleMap || bClubDeathMatch || bTowerWars || bSCHOOL_FREEPK || bFreePKMap || bBRIGHTEVENT || IsConflictTarget(pTARGET) || bClubBattle ) )
			{
...
{
	m_bSkill = bSkill;
	m_sACTIVELINKSKILL = sSkillID;
	m_wLinkLevel = wLevel;
}

void GLChar::ResetSkillLink()
{
	m_bSkill = false;
	m_sACTIVELINKSKILL = NATIVEID_NULL();
	m_wLinkLevel = 0;
}

inline HRESULT GLChar::MsgReqLinkSkillEff ( NET_MSG_GENERIC* nmg ) //Link Skill Effect by NaJ
{
	GLMSG::SNETPC_REQ_LINKSKILLEFF *pNetMsg = (GLMSG::SNETPC_REQ_LINKSKILLEFF *) nmg;
	PGLSKILL pSkill = GLSkillMan::GetInstance().GetData ( pNetMsg->sSKILLID.wMainID, pNetMsg->sSKILLID.wSubID );
	if ( !pSkill ) return S_FALSE;

	SetSkill(true, pSkill->m_sBASIC.sNATIVEID, pNetMsg->wSKILLLevel);

	GLMSG::SNETPC_REQ_LINKSKILLEFF_FB NetMsg;
	GLMSG::SNETPC_REQ_LINKSKILLEFF_BRD NetMsgBrd;
	...
	if ( m_wTARNUM > 0 )
	{
		for ( WORD i=0; i<m_wTARNUM; ++i )
		{
			...
			if ( pSkill->m_sBASIC.emIMPACT_SIDE != SIDE_ENERMY )
			{
				...
				NetMsg.sSKILLID = pNetMsg->sSKILLID;
				NetMsg.dwSelect = pNetMsg->dwSelect;
				NetMsg.wSKILLLevel = pNetMsg->wSKILLLevel;
				GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&NetMsg);

				NetMsgBrd.dwGaeaID = m_dwGaeaID;
				NetMsgBrd.sSKILLID = pNetMsg->sSKILLID;
				NetMsgBrd.wSKILLLevel = pNetMsg->wSKILLLevel;
				NetMsgBrd.dwSelect = pNetMsg->dwSelect;
				SendMsgViewAround ((NET_MSG_GENERIC*)&NetMsgBrd);
			...
			else
			{
				...
				if ( !pACTOR )					continue;
				if ( !pACTOR->IsValidBody() )	continue;
				...
				if ( sTargetID.emCrow == CROW_PC )
				{
					...
					if ( pCHAR )
					{
						pACTOR->RECEIVE_SKILLFACT( pSkill->m_sBASIC.sNATIVEID, pNetMsg->wSKILLLevel, pNetMsg->dwSelect );
						...
						NetMsgBrd.dwGaeaID = pCHAR->GetCtrlID();
						NetMsgBrd.sSKILLID = pNetMsg->sSKILLID;
						NetMsgBrd.wSKILLLevel = pNetMsg->wSKILLLevel;
						NetMsgBrd.dwSelect = pNetMsg->dwSelect;
						pCHAR->SendMsgViewAround ( (NET_MSG_GENERIC*) &NetMsgBrd );
				...
				else if ( sTargetID.emCrow == CROW_MOB )
				{
					PGLCROW pCROW = m_pLandMan->GetCrow( sTargetID.dwID );
					if ( pCROW )
					{
						pACTOR->RECEIVE_SKILLFACT( pSkill->m_sBASIC.sNATIVEID, pNetMsg->wSKILLLevel, pNetMsg->dwSelect );

						GLMSG::SNETPC_REQ_LINKSKILLEFF_CROW_BRD NetMsgCrowBrd;
						NetMsgCrowBrd.dwGlobID = pCROW->GetGlobID();
						NetMsgCrowBrd.sSKILLID = pSkill->m_sBASIC.sNATIVEID;
						NetMsgCrowBrd.wSKILLLevel = pNetMsg->wSKILLLevel;
						NetMsgCrowBrd.dwSelect = pNetMsg->dwSelect;
						pCROW->SendMsgViewAround ( (NET_MSG_GENERIC*) &NetMsgCrowBrd );
					}
				}
			}
		}
...
inline HRESULT GLChar::MsgReqTyrannyWin( NET_MSG_GENERIC* nmg )
{
	GLMSG::SNET_CTF_WIN *pNetMsg = (GLMSG::SNET_CTF_WIN *) nmg;
	if ( pNetMsg->nSchool < 0 || pNetMsg->nSchool > 2 ) return S_FALSE;

	GLTowerWars* pSW = GLTowerWarsFieldMan::GetInstance().Find ( m_pLandMan->m_dwTowerWarsMapID );
	if ( pSW ) pSW->SetCTFWinner(pNetMsg->nSchool);
			
	DEBUGMSG_WRITE(_T("Towerwars Winner %d"),
				pNetMsg->nSchool );
...
HRESULT GLChar::MsgProcess ( NET_MSG_GENERIC* nmg )
{
	...
	{
	//////////////////////////////////////////////////////////////////////////////////
	case NET_MSG_GCTRL_REQ_TOPUP:			MsgReqTopUp(nmg);				break;
	////////////////////////////////////////////////////////////////////////////////
	//  AutoPilot
	case NET_MSG_GCTRL_REQ_USE_AUTO_SKILL:	MsgReqUseAutoSkill(nmg);		break;
	////////////////////////////////////////////////////////////////////////////////
	...
	case NET_MSG_GCTRL_REQ_LINKSKILLEFF:	MsgReqLinkSkillEff(nmg);		break; //Link Skill Effect by NaJ
	case NET_MSG_GCTRL_REQ_TYRANNY_GETREWARD:			MsgReqTyrannyReward(nmg);				break;
	...
	case NET_MSG_GCTRL_INVEN_POINT_SHOP:	ReqPointShop( nmg );			break;
...
inline HRESULT GLChar::MsgReqTyrannyReward ( NET_MSG_GENERIC* nmg )
{
	GLMSG::SNETPC_REQ_TYRANNY_GETREWARD *pNetMsg = (GLMSG::SNETPC_REQ_TYRANNY_GETREWARD *) nmg;
	
	GETREWARD(pNetMsg->lnValue);

	GLMSG::SNETPC_REQ_TYRANNY_GETREWARD_FB NetMsg;
	NetMsg.lnValue = pNetMsg->lnValue;
	GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&NetMsg);
