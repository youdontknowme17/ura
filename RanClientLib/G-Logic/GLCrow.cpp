...
{
	if ( m_pCrowData->m_sAction.m_dwActFlag&EMCROWACT_IGNORE_SHOCK )
		{
		}
		else
		{
...
HRESULT GLCrow::FrameMove ( float fTime, float fElapsedTime )
{
	...
	if ( IsValidBody() )
	{
		...
		else if ( !IsACTION(GLAT_SHOCK) && !IsACTION(GLAT_PUSHPULL) )
		{
			...
			if ( !m_bSTATE_STUN && !IsStun() )  //Transform Function 
			{
				//	Note : 스케쥴 실행.
...
void GLCrow::ReceivePushPull ( const D3DXVECTOR3 &vMovePos, const bool bReact , const bool bAdjustSpeed , const float fSpeed, const bool bDash ) //add pushpull
{
...
BOOL GLCrow::IsStun()  //Transform Function 
{
