...
NET_MSG_GENERIC* GLChar::ReqNetMsg_Drop ()
{
	...
	dropChar.dwReborn	= m_dwReborn; //add reborn
...
//	NetMsg.dwWeather = m_pLandMan->IsWeatherActive() ? GLPeriod::GetInstance().GetWeather () : NULL;
	...
	if (m_pGLGaeaServer->m_sTowerWarsWinner.bVALID)
	{
		GLMSG::SNETPC_TOWERWARS_SERVER_WINNER NetMsgWin;
		NetMsgWin.sTwWinner = m_pGLGaeaServer->m_sTowerWarsWinner;
		for(int i = 0; i < m_pGLGaeaServer->m_wRankNum; i++ )
		NetMsgWin.sPlayerWinner[i] = m_pGLGaeaServer->m_sTowerWarRankWinner[i];
		NetMsgWin.wRankNum = m_pGLGaeaServer->m_wRankNum;
		m_pGLGaeaServer->SENDTOCLIENT( m_dwClientID, &NetMsgWin );
...
void GLChar::ReceivePushPull ( const D3DXVECTOR3 &vMovePos, const bool bReact , const bool bAdjustSpeed , const float fSpeed, const bool bDash ) //add pushpull
{
	...
	if ( bSucceed )
	{
		...
		if ( bDash ) SetPosition ( m_TargetID.vPos );

		/*if  ( bAdjustSpeed ) */ m_actorMove.SetMaxSpeed ( fSpeed );
...
void GLChar::ReceivePkLossPoint ( DWORD dwPkLoss , std::string strKilledBy)
{
	GLMSG::SNETPC_UPDATE_PKLOSS NetMsg;
	NetMsg.dwPkLoss = dwPkLoss;
	if(strKilledBy.length() >= 16) strKilledBy.resize(15); //fix pk bug
