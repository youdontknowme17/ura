...
HRESULT GLGaeaServer::Create ( DWORD dwMaxClient, DxMsgServer *pMsgServer, DxConsoleMsg* pConsoleMsg, GLDBMan* pDBMan, int nServiceProvider, const char* szMapList, DWORD dwFieldSID, int nChannel, BOOL bUseIntelTBB )
{
	...
	if ( m_pDBMan )
	{
		...
		//add sw buff
		VECTWID_DB vecSwidDb;
		m_pDBMan->GetSwRegion ( vecSwidDb );
		GLTowerWarsFieldMan::GetInstance().SetState( vecSwidDb );
	}
...
BOOL GLGaeaServer::ReqSetNextWarTime ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_SCHOOLWAR_NEXTWAR_REQ* pNetMsg )
{
...
BOOL GLGaeaServer::ReqTowerWarsCTFRanking ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_TOWERWARS_CTFRANKING_REQ* pNetMsg )
{
	...
	if ( pSW->IsBattle() || pSW->IsBattleEndAward() ) 
	{
		...
		for ( ; pos_begin != pos_end; pos_begin++ )
		{
			...
			if ( nIndex >= 0 && nIndex < RANKING_TWNUM )
			{
...
BOOL GLGaeaServer::ReqTowerWarsWinner ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_TOWERWARS_WINNER_REQ* pNetMsg )
{
	PGLCHAR pChar = GetChar ( dwGaeaID );
	if ( !pChar ) return FALSE;	
    ...
    if ( !pSW ) return FALSE;
		...
		if ( pSW->m_nCTFWinner != -1 )
		{
			bool bWin = pSW->m_nCTFWinner >= 0 && pSW->m_nCTFWinner <= 2;
			{
				if ( bWin )
				{
		           GLMSG::SNET_TOWERWARS_SETWINSCHOOL_BRD NetMsgTowerWarsWinner;

		           TWP_RANK_INFO_MAP_ITER	pos = pSW->m_mapTwWinner.begin();
		           TWP_RANK_INFO_MAP_ITER	end = pSW->m_mapTwWinner.end();

		            for ( ; pos != end; pos++ )
		           	{	
		           	  const STWP_RANK_INFO&	sRankInfo = pos->second;

		           	  NetMsgTowerWarsWinner.sTwWinner.wSCHOOL = pSW->m_nCTFWinner;
           	    	  ...
           	    	  NetMsgTowerWarsWinner.sTwWinner.dwCharID = sRankInfo.dwCharID;
           	 		  NetMsgTowerWarsWinner.sTwWinner.dwGaeaID = sRankInfo.dwGaeaID;
	   	 		   	  ...
	   	 		   	  SENDTOCLIENT( dwClientID, &NetMsgTowerWarsWinner );
					}


				GLMSG::SNET_TOWER_WINNER_UPDATE NetMsg;
			    TWP_RANK_INFO_MAP_ITER pos_begin = pSW->m_mapTwWinner.begin();
				TWP_RANK_INFO_MAP_ITER pos_end = pSW->m_mapTwWinner.end();
                m_wRankNum = 0;
				for ( ; pos_begin != pos_end; pos_begin++ )
				{
					const STWP_RANK_INFO& sRankInfo = pos_begin->second;

					if ( sRankInfo.nIndex >= 0 && sRankInfo.nIndex < RANKING_TWNUM )
					{
						STWP_RANK sRank = sRankInfo;
						NetMsg.ADDRANK( sRank );
						if ( RANKING_TWNUM==m_wRankNum )		return false;
						m_sTowerWarRankWinner[m_wRankNum] = sRank;
						++m_wRankNum;
					}
				}
