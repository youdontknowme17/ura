...
struct GLPET
{
    ...
    EMPIECECHAR GetPieceFromSlot ( PET_ACCESSORYTYPE emType );

	// access
	BOOL LoadFile ( const char* szFileName, PETTYPE emType );

	void		UpdateTimeLmtItem ( GLChar* pChar );
	BOOL ISDUAL_SKILL (){ return m_bDualSkill; };
	void LEARN_DUAL_SKILL ();
};
...
namespace COMMENT
{
	extern std::string PET_TYPE[PETTYPE_NEWSIZE];
};
