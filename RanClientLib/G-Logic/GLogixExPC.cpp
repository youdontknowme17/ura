...
//			for ( i=0; i<SKILLFACT_SIZE; ++i )		DISABLESKEFF ( i );
	...
	if( m_bPotsBlock )
	{
		m_fPotsTimer -= fElapsedTime;
		if( m_fPotsTimer <= 0.0f )	{ m_bPotsBlock = false; m_fPotsTimer = 0.0f; }
	}
	else
	{
		if( m_bPotsUse )
...
void GLCHARLOGIC::GETREWARD( LONGLONG lnValue )
{
	m_lnMoney += lnValue;
	INIT_DATA(FALSE, FALSE);
}