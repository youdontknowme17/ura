...
struct SCROWATTACK
{
	...
	SCROWATTACK& operator= ( const SCROWATTACK_105 &OldData );
	SCROWATTACK& operator= ( const SCROWATTACK_106 &OldData );
	SCROWATTACK& operator= ( const SCROWATTACK_107 &OldData );
	...
	static VOID SaveCsvHead_old ( std::fstream &SFile, int iIndex );
...
enum EMCROW_NPCACT
{
	...
	EMCROWACT_TOWERSG	= 0x02000,
	EMCROWACT_TOWERPHX  = 0x04000,
	EMCROWACT_TOWERMP	= 0x08000,
	EMCROWACT_IGNORE_SHOCK	= 0x10000,

	EMCROWACT_AUTODROP	= 0x20000,
	EMCROWACT_CTFTOWER	= 0x40000,
};
...
struct SCROWACTION
{
	enum { VERSION = 0x0109, SALENUM = 3, PATTERNNUM = 10, ATTACKSETNUM = 4 };
...
struct SCROWDATA
{
	...
	BOOL IsTowerSG()		{ return m_sAction.m_dwActFlag&EMCROWACT_TOWERSG; }
	BOOL IsTowerMP()		{ return m_sAction.m_dwActFlag&EMCROWACT_TOWERMP; }
	BOOL IsTowerPHX()		{ return m_sAction.m_dwActFlag&EMCROWACT_TOWERPHX; }
	...
	BOOL IsCTFTOWER()		{ return m_sAction.m_dwActFlag&EMCROWACT_CTFTOWER; }

	BOOL IsBoss ()			{ return m_sAction.m_dwActFlag&EMCROWACT_BOSS; }
	BOOL IsAutoDrop ()		{ return m_sAction.m_dwActFlag&EMCROWACT_AUTODROP; }
	BOOL IsKnock ()			{ return m_sAction.m_dwActFlag&EMCROWACT_KNOCK; }
...
namespace COMMENT
{
	...
	extern std::string CROWSKTAR_TYPE[CROWSKTAR_SIZE];
};
