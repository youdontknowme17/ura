...
struct DxSkinPiece;
class DxSimMesh;
class DxSkinPieceRootData;//add abl
...
public:
	...
	//add class
	DxSkinPiece*	m_pSkinPiece[GLCI_NUM_2015];
	DxSimMesh*		m_pInvenMesh;	//	Inventory Mesh.
	DxSimMesh*		m_pFieldMesh;	//	Field Mesh.
	DxSkinPieceRootData*	m_pSkinPieceRoot[CPSIND_SIZE];//add abl
...
public:
		...
		m_pFieldMesh(NULL)
	{
		...
		m_pSkinPieceRoot[0] = NULL;//add abl
		m_pSkinPieceRoot[1] = NULL;
		m_pSkinPieceRoot[2] = NULL;
		m_pSkinPieceRoot[3] = NULL;
	}
...
public:
	...
	//add abl
	DxSkinPieceRootData* GetABL ( SNATIVEID sNativeID, EMCPSINDEX emCpsInd )	{ return GetABL ( sNativeID.wMainID, sNativeID.wSubID, emCpsInd ); }
	DxSkinPieceRootData* GetABL ( WORD wMID, WORD wSID, EMCPSINDEX emCpsInd );
