...
namespace GLMSG
{
	...
	struct SNETPC_REQ_CROWLIST
	{
		...
		SNETPC_REQ_CROWLIST ()
		{
			...
			nmg.nType = NET_MSG_REQ_CROWLIST;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNETPC_REQ_CROWLIST_FB
	{
		NET_MSG_GENERIC		nmg;
		SNATIVEID			sCrowID;
		D3DXVECTOR3			vPos;
		...
		SNETPC_REQ_CROWLIST_FB ():
			 sCrowID(NATIVEID_NULL())
			,vPos(0,0,0)
			,bRender(false)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_REQ_CROWLIST_FB;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNETPC_REQ_CROWLIST_FLD
	{
		NET_MSG_GENERIC		nmg;
		DWORD				dwGaeaID;

		SNETPC_REQ_CROWLIST_FLD () :
			dwGaeaID(0)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_REQ_CROWLIST_FLD;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};
	struct SNET_PERIOD
	...
	struct SNET_MOVE2MAPPOS
	{
		...
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_MOVE2MAPPOS;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNET_MOVE2MAPPOS_FLD
	{
		NET_MSG_GENERIC		nmg;
		...
		SNET_MOVE2MAPPOS_FLD () :
			nidMAP(false),
			dwGaeaID(0),
			dwGATE(0)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_MOVE2MAPPOS_FLD;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNET_MOVE2MAPPOS_FB
	{
		NET_MSG_GENERIC		nmg;
		D3DXVECTOR3			vPOS;
		...
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_MOVE2MAPPOS_FB;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};
	...
	struct SNET_CTF_WIN
	{
		...
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_CTF_WIN;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};
	struct SNET_TYRANNY_PARTICIPATE
	{
		NET_MSG_GENERIC		nmg;
		bool		bParticipate;

		SNET_TYRANNY_PARTICIPATE ()
			: bParticipate(false)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_TYRANNY_PARTICIPATE;
		}
	};
	struct SNET_TYRANNY_PARTICIPATE_BRD
	{
		NET_MSG_GENERIC		nmg;
		bool		bParticipate;

		SNET_TYRANNY_PARTICIPATE_BRD()
			: bParticipate(false)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_TYRANNY_PARTICIPATE_BRD;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};
	struct SNET_PVP_REJOIN
	{
		NET_MSG_GENERIC		nmg;
		bool		bRejoin;
		...
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_PVP_REJOIN;
		}
	};
	struct SNET_PVP_REJOIN_BRD
	{
		NET_MSG_GENERIC		nmg;
		bool		bRejoin;
		...
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_PVP_REJOIN_BRD;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};
