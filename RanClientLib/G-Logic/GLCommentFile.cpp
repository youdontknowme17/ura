...
bool CGLCommentFile::LOADFILE( const char * szFileName )
{
	...
	/////////////additional skill data types
	for (int i=0; i<SKILL::EMSTIGMA_SIZE; ++i )
		cFILE.getflag( "Skill", "SPEC_STIGMA_TYPE", i, SKILL::EMSTIGMA_SIZE, COMMENT::EMACTION[i]);

	for (int i=0; i<SKILL::EMHALLUCINATE_SIZE; ++i )
		cFILE.getflag( "Skill", "SPEC_HALLUCINATE_TYPE", i, SKILL::EMHALLUCINATE_SIZE, COMMENT::SPEC_HALLUCINATE_TYPE[i]);

	for (int i=0; i<SKILL::EMDOMINATE_SIZE; ++i )
		cFILE.getflag( "Skill", "SPEC_DOMINATE_TYPE", i, SKILL::EMDOMINATE_SIZE, COMMENT::SPEC_DOMINATE_TYPE[i]);

	for (int i=0; i<SKILL::EMASIGN_SIZE; ++i )
		cFILE.getflag( "Skill", "SPEC_ASIGN_TYPE", i, SKILL::EMASIGN_SIZE, COMMENT::SPEC_ASIGN_TYPE[i]);

	for (int i=0; i<SKILL::EMINC_SIZE; ++i )
		cFILE.getflag( "Skill", "SPEC_INC_TYPE", i, SKILL::EMINC_SIZE, COMMENT::SPEC_INC_TYPE[i]);

	for (int i=0; i<SKILL::EMINVISIBLE_SIZE; ++i )
		cFILE.getflag( "Skill", "SPEC_INVISIBLE_USE_TYPE", i, SKILL::EMINVISIBLE_SIZE, COMMENT::SPEC_INVISIBLE_USE_TYPE[i]);

	for (int i=0; i<SKILL::EMINVISIBLE_ANI_SIZE; ++i )
		cFILE.getflag( "Skill", "SPEC_INVISIBLE_ANI_TYPE", i, SKILL::EMINVISIBLE_ANI_SIZE, COMMENT::SPEC_INVISIBLE_ANI_TYPE[i]);

	for (int i=0; i<SKILL::EMSWAPPOS_DIR_SIZE; ++i )
		cFILE.getflag( "Skill", "SPEC_SWAPPOS_DIR_TYPE", i, SKILL::EMSWAPPOS_DIR_SIZE, COMMENT::SPEC_SWAPPOS_DIR_TYPE[i]);
	...
	for (int i=0; i<CROWSKTAR_SIZE; ++i )
		cFILE.getflag( "Crow", "CROWSKTAR_TYPE", i, CROWSKTAR_SIZE, COMMENT::CROWSKTAR_TYPE[i]);
	...
	//for (int i=0; i<ITEMATT_NSIZE+1; ++i )
		...
		//we changes the loop count since the nocare is not equal to size

	for (int i=0; i<ITEMATT_NSIZE; ++i )
		cFILE.getflag( "ItemDef", "ITEMATTACK", i, ITEMATT_NSIZE, COMMENT::ITEMATTACK[i]);
	...
	for ( i=0; i<SLOT_NSIZE_S_2; ++i )
		cFILE.getflag( "ItemDef", "ITEM_PUTON_SLOT", i, SLOT_NSIZE_S_2, COMMENT::ITEM_PUTON_SLOT[i]);

	for ( i=0; i<VEHICLE_ACCETYPE_SIZE; ++i )
		cFILE.getflag( "ItemDef", "ITEM_VEHICLE_SLOT", i, VEHICLE_ACCETYPE_SIZE, COMMENT::ITEM_VEHICLE_SLOT[i]);

	for ( i=0; i<PET_ACCETYPE_SIZE; ++i )
		cFILE.getflag( "ItemDef", "ITEM_PET_SLOT", i, PET_ACCETYPE_SIZE, COMMENT::ITEM_PET_SLOT[i]);
	...
	for (int i=0; i<GLCI_NUM_2015; ++i )
	{
		...
		//add class
		cFILE.getflag( "CharDefine", "CHARCLASS", i, GLCI_NUM_2015, COMMENT::CHARCLASS[i]);
		cFILE.getflag( "CharDefine", "CHARCOMMENT", i, GLCI_NUM_2015, COMMENT::CHARCOMMENT[i]);
	}
	...
	////////////////////////////////////
	// Duel Bet Type //add duel bet
	for (int i=0; i<EMCONFT_BET_SIZE; ++i )
		cFILE.getflag( "Conft", "CONFT_BET_TYPE", i, EMCONFT_BET_SIZE, COMMENT::CONFT_BET_TYPE[i]);
	...
	{
		cFILE.getflag( "SkinAniMan", "ANI_SUBTYPE", i, AN_SUB_SIZE, COMMENT::ANI_SUBTYPE[i]);
	}

	for (int i=0; i<AN_SUB_01_SIZE; ++i )
	{	
		cFILE.getflag( "SkinAniMan", "ANI_SUBTYPE_01", i, AN_SUB_01_SIZE, COMMENT::ANI_SUBTYPE_01[i]);
	}
	...
	for (int i=0; i<PETTYPE_NEWSIZE; ++i )
		cFILE.getflag( "PetDefine", "PET_TYPE", i, PETTYPE_NEWSIZE, COMMENT::PET_TYPE[i] );
	...
	for (int i=0; i<SUMMON_TYPE_SIZE; ++i )
		cFILE.getflag( "SummonDefine", "SUMMON_TYPE", i, SUMMON_TYPE_SIZE, COMMENT::SUMMONTYPE[i] );
