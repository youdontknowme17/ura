...
void GLChar::AddPlayHostile ( DWORD dwCHARID, BOOL bBAD, BOOL bClubBattle )
{
	...
	if ( !(bBRIGHTEVENT||bSCHOOL_FREEPK||bGuidBattleMap||bClubDeathMatch||bSchoolWars ||bTowerWars ) )
	{
		//	Note : 적대 행위자 등록. ( pk )
...
void GLChar::DamageProc ( int nDAMAGE, const DWORD dwDamageFlag, const BOOL bLowSP )
{
	...
	if ( pRHandItem && pRHandItem->sSuitOp.sBLOW.emTYPE!=EMBLOW_NONE )
	{
		...
		if( nBLOWRESIST>GLCONST_CHAR::iBLOWRESIST_MAX ) nBLOWRESIST = GLCONST_CHAR::iBLOWRESIST_MAX;
...
//		float fPOWER = GLOGICEX::WEATHER_BLOW_POW ( sBlow.emTYPE, GLPeriod::GetInstance().GetWeather(), m_pLandMan->IsWeatherActive() );
	...
	if ( dwDamageFlag & DAMAGE_TYPE_CRUSHING_BLOW ) 
	{
		...
		m_pLandMan->PushPullAct( dwActorID, m_TargetID, GLCONST_CHAR::fCRUSH_BLOW_RANGE, 0.0f, false, false, false ); //add pushpull //modify
	}
}
...
// Desc: 부활스킬 시동
// *****************************************************
DWORD GLChar::DoRevive2Skill ( SNATIVEID skill_id, WORD wSKILL_LVL )
{
	...
	{
		const SKILL::SSPEC &sSPEC = pSkill->m_sAPPLY.sSPECS[a].sSPEC[wSKILL_LVL];
...
//					bCRITICAL = CALCDAMAGE ( nVAR_HP, bShock, sTARID, m_pLandMan, pSkill, wSKILL_LVL, GLPeriod::GetInstance().GetWeather(), sAniAttack.m_wDivCount );
				...
				for(int a = 0; a < SKILL::MAX_LEVEL; a++ )
				{
					const SKILL::SSPEC &sSKILL_SPEC = pSkill->m_sAPPLY.sSPECS[a].sSPEC[wSKILL_LVL];
	...
	//			float fPOWER = GLOGICEX::WEATHER_BLOW_POW ( sBLOW.emTYPE, GLPeriod::GetInstance().GetWeather(), m_pLandMan->IsWeatherActive() );
			...
			for(int a = 0; a < SKILL::MAX_LEVEL; a++)
			{
				...
				case EMSPECA_PUSH: 	//add pushpull
					...
					sSKILLACTEX.fPUSH_PULL_B = sSKILL_SPEC.fVAR2;
				...
				case EMSPECA_RECBLOW:
					//	Note : 상태 이상 치료.
					//
					sSKILLACTEX.dwCUREFLAG |= pSkill->m_sAPPLY.sSPECS[a].sSPEC[wSKILL_LVL].dwFLAG;
					break;
			...
			if ( sTARID.emCrow==CROW_PC )
			{
				...
				if ( pPC )
				{
					DWORD wRECOVE = pPC->DoRevive2Skill(skill_id,wSKILL_LVL);
					if ( 0<wRECOVE)
					{
						++nEXP_NUM;
		...
		case EMCONFT_ONE:
			{
				...
				if ( sOption.emBet != 0 )
				{
					if ( IsCanBet() )
					{
						BOOL bOK = ReceiveDuelLoss ( sOption.llBet ,sOption.emBet );
						if (bOK) pCONFT->ReceiveDuelWin ( sOption.llBet ,sOption.emBet );

						CString strTemp;
						std::string strTIME = CDebugSet::GetCurTime ();

						strTemp.Format ("%s--Duel Type:'%s'---Winner:'%s'--Losser:'%s' --Bet:%I64d",
							strTIME.c_str(),
							COMMENT::CONFT_BET_TYPE[sOption.emBet].c_str(),
							pCONFT->m_szName,
							m_szName,
							sOption.llBet );
						CDebugSet::ToFile ( "DuelBetResult.txt",strTemp);
	...
	if ( m_sAssault.emCrow==CROW_PC )
	{
		...
		if ( pCHAR )
		{
			...
			else if ( bTowerWars )
			{
				...
				if ( pTW && pCHAR )
				{		
					pTW->AddSWPScore( pCHAR->m_dwCharID, pCHAR->m_dwGaeaID, m_dwCharID, m_dwGaeaID );
			...
			//add pk
			if (!( m_dwReborn  < GLCONST_CHAR::PkWinMinRb_A))
			{
				DWORD pCHARpk = pCHAR->GetReborn();
				if (!( pCHARpk  < GLCONST_CHAR::PkWinMinRb_B) )
				{
					pCHAR->ReceivePkWinPoint(1,m_szName);
				}
			}
			//
			if (!( m_dwReborn < GLCONST_CHAR::PkLossMinRb_A))
			{
				DWORD pCHARpk = pCHAR->GetReborn();
				if (!( pCHARpk < GLCONST_CHAR::PkLossMinRb_B ))
				{
