...
std::vector< string > StringExplode(const string& str, const char& ch);
std::vector< string > StringExplode(const string& str, const char& ch) {
    string next;
...
VOID SCROWGEN::SaveCsvHead_old ( std::fstream &SFile )
{
	SFile << "dwGenMoney" << ",";
	SFile << "sGenItemID wMainID" << ",";
	SFile << "sGenItemID wSubID" << ",";
...
VOID SCROWGEN::SaveCsv_old ( std::fstream &SFile )
{
	SFile << m_dwGenMoney << ",";
	SFile << m_sGenItemID.wMainID << ",";
	SFile << m_sGenItemID.wSubID << ",";
...
BOOL SGENITEM::ISGEN (  float fGenRate )	//	발생 확율.
{
	...
	float fGEN_RATE = m_fGenRate*GLCONST_CHAR::GETITEM_SCALE();
	if( fGenRate != 0.0f )
		fGEN_RATE *= fGenRate;
	else 
		return FALSE;
...
SCROWATTACK& SCROWATTACK::operator= ( const SCROWATTACK_100 &OldData )
{
	...
	sDamage.dwLow		= OldData.wLow;
	sDamage.dwHigh		= OldData.wHigh;
...
SCROWATTACK& SCROWATTACK::operator= ( const SCROWATTACK_105 &OldData )
{
	bUsed			= OldData.bUsed;
	emAttRgType		= OldData.emAttRgType;
	wRange			= OldData.wRange;
	...
	strSelfBodyEffect	= OldData.strSelfBodyEffect;
	strTargBodyEffect	= OldData.strTargBodyEffect;
	strTargetEffect		= OldData.strTargetEffect;


	return *this;
}

SCROWATTACK& SCROWATTACK::operator= ( const SCROWATTACK_106 &OldData )
{
	bUsed			= OldData.bUsed;
	emAttRgType		= OldData.emAttRgType;
	wRange			= OldData.wRange;
	...
	dwDamageLow		= OldData.dwDamageLow;
	dwDamageHigh	= OldData.dwDamageHigh;
...
SCROWATTACK& SCROWATTACK::operator= ( const SCROWATTACK_107 &OldData )
{
	bUsed			= OldData.bUsed;
	emAttRgType		= OldData.emAttRgType;
	wRange			= OldData.wRange;
...
SCROWATTACK& SCROWATTACK::operator= ( const SCROWATTACK &value )
{
	...
	dwDamageLow		= value.dwDamageLow;
	dwDamageHigh	= value.dwDamageHigh;
...
bool SCROWATTACK::LOAD ( basestream &SFile )
{
	...
	SFile >> sDamage.dwData; //pending

	if ( sDamage.dwHigh < sDamage.dwLow)
	{
		 sDamage.dwHigh =  sDamage.dwLow;
	}
	//SFile >> sDamageBak.dwData2;
	SFile >> dwDamageLow;
	SFile >> dwDamageHigh;

	if ( dwDamageHigh <  dwDamageLow )
	{
		dwDamageHigh = dwDamageLow;
	}
	SFile >> fDelay;
...
bool SCROWATTACK::LOAD_105 ( basestream &SFile )
{
	DWORD dwDATA;
	WORD	wLow;
	WORD	wHigh;
...
bool SCROWATTACK::LOAD_106 ( basestream &SFile )
{
	DWORD dwDATA;
	WORD	wLow;
	WORD	wHigh;
...
bool SCROWATTACK::LOAD_107 ( basestream &SFile )
{
	DWORD dwDATA;
...
VOID SCROWATTACK::SaveCsvHead ( std::fstream &SFile, int iIndex )
{
	...
	SFile << iIndex << " dwDamageLow" << ",";
	SFile << iIndex << " dwDamageHigh" << ",";
	SFile << iIndex << " fDelay" << ",";
...
VOID SCROWATTACK::SaveCsvHead_old ( std::fstream &SFile, int iIndex )
{
	SFile << iIndex << " bUsed" << ",";
	SFile << iIndex << " emAttRgType" << ",";
	SFile << iIndex << " wRange" << ",";
...
VOID SCROWATTACK::SaveCsv_old ( std::fstream &SFile )
{
	SFile << bUsed << ",";
	SFile << emAttRgType << ",";
	SFile << wRange << ",";
...
VOID SCROWATTACK::LoadCsv ( CStringArray &StrArray )
{
	...
	sDamage.dwLow = (DWORD)atoi( StrArray[ iCsvCur++ ] );
	sDamage.dwHigh = (DWORD)atoi( StrArray[ iCsvCur++ ] );
	dwDamageLow = (DWORD)atoi( StrArray[ iCsvCur++ ] );
	dwDamageHigh = (DWORD)atoi( StrArray[ iCsvCur++ ] );
...
VOID SCROWBASIC::SaveCsv_old ( std::fstream &SFile )
{
	SFile << sNativeID.wMainID << ",";
	SFile << sNativeID.wSubID << ",";
...
VOID SCROWBASIC::SaveCsvHead_old ( std::fstream &SFile )
{
	SFile << "sNativeID wMainID" << ",";
	SFile << "sNativeID wSubID" << ",";
...
SCROWACTION& SCROWACTION::operator= ( const SCROWACTION &value )
{
	...
	dwMobReqSchool		= value.dwMobReqSchool;
...
VOID SCROWACTION::SaveCsvHead_old ( std::fstream &SFile )
{
	SFile << "wBodyRadius" << ",";
...
VOID SCROWACTION::SaveCsv_old ( std::fstream &SFile )
{
	SFile << m_wBodyRadius << ",";
...
VOID SCROWACTION::LoadCsv ( CStringArray &StrArray )
{
	...
	dwMobReqSchool = ( DWORD ) atoi( StrArray[ iCsvCur++ ] );
...
HRESULT SCROWDATA::LoadFile ( basestream &SFile, BOOL bServer, bool bPastLoad )
{
		...
		case FILE_SATTACK:
			{
				...
				{
					/*SCROWATTACK_105 sOLDATTACK[3];
					SFile.ReadBuffer ( &sOLDATTACK[0], sizeof(SCROWATTACK_105) );
					SFile.ReadBuffer ( &sOLDATTACK[1], sizeof(SCROWATTACK_105) );
					SFile.ReadBuffer ( &sOLDATTACK[2], sizeof(SCROWATTACK_105) );

					m_sCrowAttack[0] = sOLDATTACK[0];
					m_sCrowAttack[1] = sOLDATTACK[1];
					m_sCrowAttack[2] = sOLDATTACK[2];*/

					m_sCrowAttack[0].LOAD_105 ( SFile );
					m_sCrowAttack[1].LOAD_105 ( SFile );
					m_sCrowAttack[2].LOAD_105 ( SFile );
				...
				else if ( dwDataVer==0x0106 )
				{
					...
					m_sCrowAttack[0].LOAD_106 ( SFile );
					m_sCrowAttack[1].LOAD_106 ( SFile );
					m_sCrowAttack[2].LOAD_106 ( SFile );

				}
				else if ( dwDataVer==0x0107 )
				{
					m_sCrowAttack[0].LOAD_107 ( SFile );
					m_sCrowAttack[1].LOAD_107 ( SFile );
					m_sCrowAttack[2].LOAD_107 ( SFile );
	...
	//if ( bServer )
	{
		if ( !m_sGenerate.m_strGenItem.empty() )
		{
			...
			results = StringExplode(m_sGenerate.m_strGenItem.c_str(), ';');
			for ( int i=0; i<results.size(); i++ )
			{
			//m_sGenItem.LOADFILE ( m_sGenerate.m_strGenItem.c_str() );
				m_sGenItem[i].LOADFILE ( results[i].c_str() );
			}
		}
...
VOID SCROWDATA::SaveCsvHead_old ( std::fstream &SFile )
{
	SCROWBASIC::SaveCsvHead_old( SFile );
	SCROWACTION::SaveCsvHead_old( SFile );
	SCROWGEN::SaveCsvHead_old( SFile );
	for( int i=0; i<EMMAXATTACK; ++i )
		SCROWATTACK::SaveCsvHead_old( SFile, i+1 );

	SFile << std::endl;
}

VOID SCROWDATA::SaveCsv_old ( std::fstream &SFile )
{
	m_sBasic.SaveCsv_old( SFile );
	m_sAction.SaveCsv_old( SFile );
	m_sGenerate.SaveCsv_old( SFile );
	for( int i=0; i<EMMAXATTACK; ++i )
		m_sCrowAttack[i].SaveCsv_old( SFile );
...
HRESULT GLCrowDataMan::SaveCsvFile_old ( CWnd* pWnd )
{
	CFileDialog dlg( FALSE, ".csv", NULL, OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT,
		"csv file (*.csv)|*.csv|", pWnd );
	...
	// Csv Head
	SCROWDATA::SaveCsvHead_old( streamFILE );
	...
	for( int i=0; i<MAX_CROW_MID; ++i )
	{
		for( int j=0; j<MAX_CROW_SID; ++j )
		{
			...
			// Csv Data
			SCROWDATA *pCROWDATA = m_ppPCROWDATA[i][j];
			pCROWDATA->SaveCsv_old( streamFILE );
		}
	}
...
namespace COMMENT
{
	...
	std::string CROWSKTAR_TYPE[CROWSKTAR_SIZE] =
	{
		"CROWSKTAR_OUR",
		"CROWSKTAR_ENEMY"
	};
