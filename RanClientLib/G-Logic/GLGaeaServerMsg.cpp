...
void GLGaeaServer::SENDTOSCHOOLCLIENT_ONMAP ( DWORD dwMapID, WORD wSchoolID, LPVOID nmg )
{
	GLLandMan* pLandMan = GetByMapID( dwMapID );
	if ( !pLandMan )	return;
	if ( !m_pMsgServer )	return;
	...
	for ( ; pCharNode; pCharNode = pCharNode->pNext )
	{		
		if ( pCharNode->Data->m_wSchool == wSchoolID )
			m_pMsgServer->SendClient ( pCharNode->Data->m_dwClientID, nmg );
	}
...
void GLGaeaServer::SENDTOPLAYERCLIENT_ONMAP ( DWORD dwMapID, DWORD m_dwCharID, LPVOID nmg )
{
	GLLandMan* pLandMan = GetByMapID( dwMapID );
	if ( !pLandMan )	return;
	if ( !m_pMsgServer )	return;
	...
	for ( ; pCharNode; pCharNode = pCharNode->pNext )
	{		
		if ( pCharNode->Data->m_dwCharID == m_dwCharID )
			m_pMsgServer->SendClient ( pCharNode->Data->m_dwClientID, nmg );
	}
...
BOOL GLGaeaServer::EntryLand( DWORD dwGaeaID, DWORD dwGateID, BOOL bInstantMap, SNATIVEID sMapID /*= NATIVEID_NULL()*/ )
{
	...
	DWORD dwTOGATEID = pOutGate->GetToGateID();
	if ( pInLandMan->m_bTowerWarsMap && sCurMapID != sToMapID )
	{
		GLTowerWars* pSW = GLTowerWarsFieldMan::GetInstance().Find( pInLandMan->m_dwTowerWarsMapID );
		if ( pSW && pSW->IsEnterMap()) 
		{
			PDXLANDGATE pTESTGATE = pInLandMan->GetLandGateMan().FindLandGate ( pSW->GetReviveGate( pPC->m_wSchool ) );
			if ( pTESTGATE )
			{
				if( sCurMapID != sToMapID ) dwTOGATEID = pTESTGATE->GetGateID();	
			}
		}
	}

	if ( pInLandMan->m_bTowerWarsMap && pPC->m_dwUserLvl < USER_SPECIAL )
		pInLandMan->UpdateAddHPmultiplier( pPC->m_wSchool );
	
	//add sw landgate
	/*if ( pInLandMan->m_bTowerWarsMap ) pInGate = pInGateMan->FindLandGate ( (DWORD)pPC->m_wSchool );
	else*/ pInGate = pInGateMan->FindLandGate ( dwTOGATEID );
...
//add charinfoview
BOOL GLGaeaServer::RequestViewProperties ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_VIEW_PROPERTIES *pNetMsg )
{
	...
	GLMSG::SNET_VIEW_PROPERTIES_FB NetMsg;

	if ( pTarChar )
	{
		NetMsg.dwMasterID = dwGaeaID;
		NetMsg.Viewpropertise = 1;
		NetMsg.WeizhiTo = 99;
		NetMsg.dwTargetID = pTarChar->GETCLIENTID();
		NetMsg.Data = ((GLMSG::SNETDROP_PC*) pTarChar->ReqNetMsg_Drop ())->Data;
		StringCchCopy ( NetMsg.szName, CHAR_SZNAME, pTarChar->m_szName );
	}	
		SENDTOAGENT ( pMyChar->m_dwClientID, &NetMsg );

	for ( int i = 0; i < SLOT_NSIZE_S_2; ++i )
	{
		GLMSG::SNET_VIEWCHARITEM_SLOT_TAR NetMsgEx;
		NetMsgEx.PutOnItem = pTarChar->m_PutOnItems[i];
		NetMsgEx.nSlot = i;
		SENDTOAGENT ( pMyChar->m_dwClientID, &NetMsgEx );
	}
...
// *****************************************************
BOOL GLGaeaServer::MsgReqHoldArray ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNETPC_REQ_HOLD_ARRAY* pNetMsg )
{
	PGLCHAR pPC = GetChar ( dwGaeaID );
	...
	GLMSG::SNETPC_REQ_HOLD_ARRAY *pNetMsgb = reinterpret_cast<GLMSG::SNETPC_REQ_HOLD_ARRAY*> ( pNetMsg );
	
	for( int i = 0; i < 300; i++ )
	{
		if ( pNetMsgb->wPosX[i] == -1 && pNetMsgb->wPosY[i] == -1 ) continue; 
	
		SINVENITEM *pInvenItem = pPC->GetCharData2().m_cInventory.GetItem ( pNetMsgb->wPosX[i], pNetMsgb->wPosY[i] );
		...
		pPC->GetCharData2().m_cInventory.DeleteItem ( sInven_BackUp.wPosX, sInven_BackUp.wPosY );						
		
		pPC->HOLD_ARRAY ( sInven_BackUp.sItemCustom, i );
		
		GLMSG::SNETPC_INVEN_DELETE NetMsg;
		NetMsg.wPosX = sInven_BackUp.wPosX;
		NetMsg.wPosY = sInven_BackUp.wPosY;
		GLGaeaServer::GetInstance().SENDTOCLIENT(pPC->m_dwClientID,&NetMsg);
	}
...
BOOL GLGaeaServer::MsgReqHoldArrayToInven ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNETPC_REQ_HOLDARRAY_TO_INVEN* pNetMsg )
{
	PGLCHAR pPC = GetChar ( dwGaeaID );
	...
	SITEMCUSTOM sITEMCUSTOM[300];
	for( int a = 0; a < 300; a++ )
	{
		if ( pPC->GET_HOLDARRAY_ITEM(a).sNativeID != NATIVEID_NULL())
			sITEMCUSTOM[a] = pPC->GET_HOLDARRAY_ITEM(a);

		pPC->RELEASE_HOLDARRAY_ITEM(a);
	}

	//Can't Pile Items with PILENUM
	/*for( int _I = 0; _I < 300; _I++ )
	{
		SITEMCUSTOM sITEM_B[300];
		int nINDEX = 0;
		WORD wTurnNum = 0;
		SITEM* pItem = GLItemMan::GetInstance().GetItem ( sITEMCUSTOM[_I].sNativeID );
		if ( pItem && pItem->ISPILE())
		{
			for( int _J = 0; _J < 300; _J++ )
			{
				...
				SITEM* pItem_b = GLItemMan::GetInstance().GetItem ( sITEMCUSTOM[_J].sNativeID );
				if ( pItem_b && pItem_b->ISPILE())
				{
					if ( sITEMCUSTOM[_J].sNativeID == sITEMCUSTOM[_I].sNativeID )
					{
						if ( pItem->sDrugOp.wPileNum == pItem_b->sDrugOp.wPileNum )
						{
							if ( nINDEX == 0 )
							{
								sITEM_B[nINDEX] = sITEMCUSTOM[_I];
								wTurnNum += sITEMCUSTOM[_I].wTurnNum;
								sITEMCUSTOM[_I] = SITEMCUSTOM();
							}
							else
							{
								sITEM_B[nINDEX] = sITEMCUSTOM[_J];
								wTurnNum += sITEMCUSTOM[_J].wTurnNum;
								sITEMCUSTOM[_J] = SITEMCUSTOM();
							}
							nINDEX++;
						}
					}
			...
			if ( nINDEX > 0 )
			{
				SITEMCUSTOM sITEM = sITEM_B[0];
				if ( wTurnNum > 0 )
				{
					while ( wTurnNum != 0 )
					{
						for( int a = 0; a < 300; a++ ) //Search for SITEMCUSTOM with NATIVEID_NULL()
						{
							if ( sITEMCUSTOM[a].sNativeID == NATIVEID_NULL()) //Found
							{
								sITEMCUSTOM[a] = sITEM; //Set Found in sITEM which is the first index of specific item
								sITEMCUSTOM[a].wTurnNum = 0; //Set wTurnNum to 0 to avoid bug
								SITEM* pItem = GLItemMan::GetInstance().GetItem ( sITEMCUSTOM[a].sNativeID );
								if ( pItem )
								{
									if ( wTurnNum > pItem->sDrugOp.wPileNum ) //wturnnum is greater than the max pilenum
									{
										sITEMCUSTOM[a].wTurnNum = pItem->sDrugOp.wPileNum; //Set turnnum of new SITEMCUSTOM into max
										wTurnNum -= pItem->sDrugOp.wPileNum; //decrease count of wturnnum by the max pilenum
									}
									else	
									{
										sITEMCUSTOM[a].wTurnNum = wTurnNum;
										wTurnNum = 0;
									}
								}
								break;
	...
	for(int b=1; b<300; b++)
	{
		for(int c=0; c<300-1; c++)
		{
			SITEM* pHoldData = GLItemMan::GetInstance().GetItem ( sITEMCUSTOM[c].sNativeID );
			if ( !pHoldData ) continue;

			SITEM* pHoldData2 = GLItemMan::GetInstance().GetItem ( sITEMCUSTOM[c+1].sNativeID );
			...
			if ( bCOMPARE ) 
			{
				SITEMCUSTOM sCUSTOM = sITEMCUSTOM[c];
				sITEMCUSTOM[c] = sITEMCUSTOM[c+1];
				sITEMCUSTOM[c+1] = sCUSTOM;
			}
		}
	}
	...
	for( int i = 0; i < 300; i++ )
	{
		if ( sITEMCUSTOM[i].sNativeID == NATIVEID_NULL()) continue; //if ( !VALID_HOLDARRAY_ITEM(i) ) continue;
			
		SITEM* pHoldData = GLItemMan::GetInstance().GetItem ( sITEMCUSTOM[i].sNativeID ); //GET_HOLDARRAY_ITEM(i).sNativeID );
		if ( !pHoldData ) return E_FAIL;

		BOOL bOk = pPC->GetCharData2().m_cInventory.IsInsertable ( pHoldData->sBasicOp.wInvenSizeX, pHoldData->sBasicOp.wInvenSizeY, _X, _Y );
		if ( !bOk )		return E_FAIL;

		bOk = pPC->GetCharData2().m_cInventory.InsertItem ( sITEMCUSTOM[i]/*GET_HOLDARRAY_ITEM(i)*/, _X, _Y );
		if ( !bOk )		return E_FAIL;
		
		SINVENITEM sINVENITEM = *pPC->GetCharData2().m_cInventory.FindPosItem ( _X, _Y );
				
		GLMSG::SNETPC_INVEN_INSERT NetMsg_Inven_Insert;
		NetMsg_Inven_Insert.Data = sINVENITEM;
		GLGaeaServer::GetInstance().SENDTOCLIENT(pPC->m_dwClientID,&NetMsg_Inven_Insert);
		...
		if ( pITEM && pITEM->IsTIMELMT() )
		{
			if ( pITEM->sDrugOp.tTIME_LMT != 0 )
			{
				...
				if ( cTIME_CUR > cTIME_LMT )
				{
					...
					{
						CDeletePet* pDbAction = new CDeletePet ( pPC->m_dwCharID, sINVENITEM.sItemCustom.dwPetID );
						GLDBMan* pDBMan = GLGaeaServer::GetInstance().GetDBMan ();
						if ( pDBMan ) pDBMan->AddJob ( pDbAction );

						PGLPETFIELD pMyPet = GLGaeaServer::GetInstance().GetPET ( pPC->m_dwPetGUID );
						if ( pMyPet && sINVENITEM.sItemCustom.dwPetID == pMyPet->m_dwPetID )
						{
							pMyPet->UpdateTimeLmtItem ( pPC );

							for ( WORD i = 0; i < PET_ACCETYPE_SIZE; ++i )
							{
								CItemDrop cDropItem;
								cDropItem.sItemCustom = pMyPet->m_PutOnItems[i];
								if ( pPC->IsInsertToInven ( &cDropItem ) ) pPC->InsertToInven ( &cDropItem );
								else
								{
									if ( m_pLandMan )
									{
										pPC->m_pLandMan->DropItem ( pPC->m_vPos, 
							...
							CGetRestorePetList *pDbAction = new CGetRestorePetList ( pPC->m_dwCharID, pPC->m_dwClientID );
							if ( pDBMan ) pDBMan->AddJob ( pDbAction );
						}
						else
						{
					...
					GLGaeaServer::GetInstance().SENDTOCLIENT(pPC->m_dwClientID,&NetMsgInvenDel);

					GLMSG::SNET_INVEN_DEL_ITEM_TIMELMT NetMsgInvenDelTimeLmt;
					NetMsgInvenDelTimeLmt.nidITEM = sINVENITEM.sItemCustom.sNativeID;
					GLGaeaServer::GetInstance().SENDTOCLIENT(pPC->m_dwClientID,&NetMsgInvenDelTimeLmt);

					pPC->GetCharData2().m_cInventory.DeleteItem ( sINVENITEM.wPosX, sINVENITEM.wPosY );
					...
					if ( sINVENITEM.sItemCustom.nidDISGUISE!=SNATIVEID(false) )
					{
						...
						BOOL bOk = pPC->GetCharData2().m_cInventory.IsInsertable ( pONE->sBasicOp.wInvenSizeX, pONE->sBasicOp.wInvenSizeY, _X, _Y );
						if ( !bOk )			return E_FAIL;

						pPC->GetCharData2().m_cInventory.InsertItem ( sITEM_NEW, _X, _Y );
						SINVENITEM *pINSERT_ITEM = pPC->GetCharData2().m_cInventory.GetItem ( _X, _Y );

						GLMSG::SNETPC_INVEN_INSERT NetItemInsert;
						NetItemInsert.Data = *pINSERT_ITEM;
						GLGaeaServer::GetInstance().SENDTOCLIENT(pPC->m_dwClientID,&NetItemInsert);

						GLITEMLMT::GetInstance().ReqItemRoute ( pINSERT_ITEM->sItemCustom, ID_CHAR, 0, ID_CHAR, pPC->m_dwCharID, EMITEM_ROUTE_CHAR, pINSERT_ITEM->sItemCustom.wTurnNum );
					}
				}
			}
		}
...
BOOL GLGaeaServer::RequestGateOutReq ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNETREQ_GATEOUT_REQ* pNetMsg )
{
	...
	if ( pLandMan )
	{
		...
		if ( pMapNode->bTWZone )
		{
			...
			GLTowerWars *pTW = GLTowerWarsFieldMan::GetInstance().Find ( pLandMan->m_dwTowerWarsMapID );
			if ( pTW )
			{
				if ( !pTW->IsEnterMap() )
				{
...
BOOL GLGaeaServer::ServerCtrlWeather ( GLMSG::SNETSERVER_CTRL_WEATHER *pNetMsg )
{
	...
	if (GLCONST_CHAR::EnableGMLogs)
	{
		CString strWeatherName;

		if ( pNetMsg->dwWeather == 1 )
		{
			strWeatherName.Format ("RAIN" );
		}
		else if ( pNetMsg->dwWeather == 2 )	
		{
			strWeatherName.Format ("SNOW");
		}
		else if ( pNetMsg->dwWeather == 4 )	
		{
			strWeatherName.Format ("LEAVES");
		}
		else if ( pNetMsg->dwWeather == 8 )	
		{
			strWeatherName.Format ("SPORE");
		}
		else if (pNetMsg->dwWeather == 0 )	
		{
			strWeatherName.Format ("CLEAR");
		}
		else 
		{
			strWeatherName.Format ("UNKNOWN");
		}

		std::string strTIME = CDebugSet::GetCurTime ();
		CDebugSet::ToFile ( "GMCMDLOG.txt", "%s	GM Command Override Weather to :%s",
		strTIME.c_str(),strWeatherName.GetString() );

		strWeatherName.Empty();
		strTIME.clear();
	}
	return TRUE;
}
...
BOOL GLGaeaServer::ServerCtrlWeather2 ( GLMSG::SNETSERVER_CTRL_WEATHER2 *pNetMsg )
{
	...
	if (GLCONST_CHAR::EnableGMLogs)
	{
		...
		std::string strTIME = CDebugSet::GetCurTime ();
		CDebugSet::ToFile ( "GMCMDLOG.txt", "%s	GM Command Override Weather:%s to MAP MID:%d SID:%d in %d Minutes",
		strTIME.c_str(),strWeatherName.GetString(),pNetMsg->map_mID,pNetMsg->map_sID,pNetMsg->dwApplyTime );
...
BOOL GLGaeaServer::ServerCtrlTime ( GLMSG::SNETSERVER_CTRL_TIME *pNetMsg )
{
	...
	{
		std::string strTIME = CDebugSet::GetCurTime ();
		CDebugSet::ToFile ( "GMCMDLOG.txt", "%s	GM Command Override TIME:%d",
		strTIME.c_str(),pNetMsg->dwTime );
		strTIME.clear();
	}
	return TRUE;
}
...
BOOL GLGaeaServer::ServerCtrlGenItemHold ( GLMSG::SNETSERVER_CTRL_GENITEMHOLD *pNetMsg )
{
	...
	if (GLCONST_CHAR::EnableGMLogs )
	{
		...
		{
			std::string strTIME = CDebugSet::GetCurTime ();
			CDebugSet::ToFile ( "GMCMDLOG.txt", "%s	GM Command Override ITEMHOLD to ON",
			strTIME.c_str() );
			strTIME.clear();
		}
		else
		{
			std::string strTIME = CDebugSet::GetCurTime ();
			CDebugSet::ToFile ( "GMCMDLOG.txt", "%s	GM Command Override ITEMHOLD to OFF",
			strTIME.c_str() );
			strTIME.clear();
		}
	}
...
BOOL GLGaeaServer::ServerCtrlPlayerKillingMode ( GLMSG::SNET_SERVER_PLAYERKILLING_MODE *pNetMsg )
{
	...
	if (GLCONST_CHAR::EnableGMLogs )
	{
		...
		{
			std::string strTIME = CDebugSet::GetCurTime ();
			CDebugSet::ToFile ( "GMCMDLOG.txt", "%s	GM Command Override PK to ON",
			strTIME.c_str() );
			strTIME.clear();
		}
		else
		{
			std::string strTIME = CDebugSet::GetCurTime ();
			CDebugSet::ToFile ( "GMCMDLOG.txt", "%s	GM Command Override PK to OFF",
			strTIME.c_str() );
			strTIME.clear();
		}
	}
...
BOOL GLGaeaServer::CtrolMove2MapPos ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_MOVE2MAPPOS_FLD *pNetMsg )
{
	PGLCHAR pChar = GetChar ( pNetMsg->dwGaeaID );
	if ( !pChar )							return FALSE;
	...
	sMapAxisInfo.MapPos2MiniPos ( UINT_MAX, UINT_MAX, vPOS.x, vPOS.z );
	...
	if ( pMapNode->dwFieldSID!=m_dwFieldSvrID )
	{
		...
		NetMsgAg.dwGATEID = pNetMsg->dwGATE;
		NetMsgAg.vPOS = vPOS;
	...
	else
	{
		...
		BOOL bOK = RequestInvenRecallThisSvr ( pChar, nidMAP, pNetMsg->dwGATE, vPOS );
		if ( !bOK )
		{
...
BOOL GLGaeaServer::CTFParticipate ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_TYRANNY_PARTICIPATE* pNetMsg )
{
	if ( dwClientID != m_dwAgentSlot ) return FALSE;

	m_bParticipate = pNetMsg->bParticipate;

	return TRUE;
}
//BattleFieldUI Data Rejoin - Eduj
BOOL GLGaeaServer::CTFRejoin ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_PVP_REJOIN* pNetMsg )
{
	if ( dwClientID != m_dwAgentSlot ) return FALSE;

	m_bRejoin = pNetMsg->bRejoin;
...
BOOL GLGaeaServer::GMCtrolClassEvent   ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_GM_CLASS_EVENT *pNetMsg )
{
	...
	//add class
	if( pNetMsg->dwClassIndex > GLCC_ALL_2015 ) return FALSE;

	//bool bClassEvent[GLCI_NUM_NEWSEX];
	//for( BYTE i = 0; i < GLCI_NUM_NEWSEX; i++ )
	//add class
	bool bClassEvent[GLCI_NUM_2015];
	for( BYTE i = 0; i < GLCI_NUM_2015; i++ )
	...
	//add class
	if( pNetMsg->dwClassIndex&GLCC_SCIENCE_M )  bClassEvent[10] = TRUE;	
	if( pNetMsg->dwClassIndex&GLCC_SCIENCE_W )  bClassEvent[11] = TRUE;	
	if( pNetMsg->dwClassIndex&GLCC_ASSASIN_M )  bClassEvent[12] = TRUE;	
	if( pNetMsg->dwClassIndex&GLCC_ASSASIN_W )  bClassEvent[13] = TRUE;	
	if( pNetMsg->dwClassIndex&GLCC_TESTING_M )  bClassEvent[14] = TRUE;	
	if( pNetMsg->dwClassIndex&GLCC_TESTING_W )  bClassEvent[15] = TRUE;	
	if( pNetMsg->dwClassIndex&GLCC_SHAPER_M )  bClassEvent[16] = TRUE;	
	if( pNetMsg->dwClassIndex&GLCC_SHAPER_W )  bClassEvent[17] = TRUE;	
	//
...
//add sw buff
BOOL GLGaeaServer::GMCtrolSchoolEvent   ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_GM_SCHOOL_EVENT *pNetMsg )
{
	if( dwClientID != m_dwAgentSlot )			return FALSE;
	if( pNetMsg->m_wSchool > GLSCHOOL_NUM ) return FALSE;

	bool bSchoolEvent[GLSCHOOL_NUM];
	for( BYTE i = 0; i < GLSCHOOL_NUM; i++ )
		bSchoolEvent[i] = FALSE;

	if( pNetMsg->m_wSchool == 0 )	bSchoolEvent[0] = TRUE;
	if( pNetMsg->m_wSchool == 1 )	bSchoolEvent[1] = TRUE;
	if( pNetMsg->m_wSchool == 2 )	bSchoolEvent[2] = TRUE;	

	if( pNetMsg->bStart )
	{		
		for( i = 0; i < GLSCHOOL_NUM; i++ )
		{
			float fEventRate = 1.0f;

			if( bSchoolEvent[i] == TRUE ) fEventRate = pNetMsg->fEventRate;

			if( pNetMsg->dwEventType == 0 )		 
			{
				m_SchoolEventData.fExpGainRate[i]  = fEventRate;
			}else if( pNetMsg->dwEventType == 1 )
			{
				m_SchoolEventData.fItemGainRate[i]  = fEventRate;
			}else if( pNetMsg->dwEventType == 2 )
			{
				m_SchoolEventData.fMoneyGainRate[i]  = fEventRate;
			}	
		}
		...
		for( i = 0; i < GLSCHOOL_NUM; i++ )
		{
			...
			if( bSchoolEvent[i] == FALSE ) continue;

			if( pNetMsg->dwEventType == 0 )		 
			{
				m_SchoolEventData.InitExpRate(i);
			}else if( pNetMsg->dwEventType == 1 )
			{
				m_SchoolEventData.InitItemRate(i);
			}else if( pNetMsg->dwEventType == 2 )
			{
				m_SchoolEventData.InitMoneyRate(i);
			}	
		}
	}
	...
	if ( GLGaeaServer::GetInstance().GetChar(dwGaeaID) )
	{
		GLMSG::SNET_GM_SCHOOL_EVENT NetMsgFB;
		GLGaeaServer::GetInstance().SENDTOCLIENT ( dwClientID, &NetMsgFB );
	}
...
BOOL GLGaeaServer::GMCtrolLimitEventBegin ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_GM_LIMIT_EVENT_BEGIN *pNetMsg )
{
	...
	//add btg attack
	sEventState.fAttackGainRate = pNetMsg->attackGain_Rate;		
...
BOOL GLGaeaServer::GMCtrolMobLevel ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_GM_MOB_LEV *pNetMsg )
{
	...
	{
		std::string strTIME = CDebugSet::GetCurTime ();
		CDebugSet::ToFile ( "GMCMDLOG.txt", "%s	GM Command Call Level File: %s to MAP MID:%d SID:%d",
		strTIME.c_str(),pNetMsg->szLevelFile,pNetMsg->wMAP_MID,pNetMsg->wMAP_SID );
		strTIME.clear();
	}
...
BOOL GLGaeaServer::GMCtrolMobLevelClear ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_GM_MOB_LEV_CLEAR *pNetMsg )
{
	...
	{
		std::string strTIME = CDebugSet::GetCurTime ();
		CDebugSet::ToFile ( "GMCMDLOG.txt", "%s	GM Command Unload Level File from MAP MID:%d SID:%d",
		strTIME.c_str(),pNetMsg->wMAP_MID,pNetMsg->wMAP_SID );
		strTIME.clear();
	}
...
BOOL GLGaeaServer::RequestShopInfo( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_GM_SHOP_INFO_REQ *pNetMsg )
{
	...
	{	
		...
		if( pChar == NULL || pChar->m_sPMarket.IsOpen() == FALSE || pChar->m_sMapID.wMainID != GLCONST_CHAR::WMARKET_MID || pChar->m_sMapID.wSubID != GLCONST_CHAR::WMARKET_SID  ) 
		{
	...
	{
		std::string strTIME = CDebugSet::GetCurTime ();
		CDebugSet::ToFile ( "GMCMDLOG.txt", "%s	charid= %s (%d), CMD=Viewing all Items MID and SID Sold in MARKET",
		strTIME.c_str(),pMyChar->m_szName,pMyChar->m_dwCharID );
		strTIME.clear();
	}
...
BOOL GLGaeaServer::ServerTowerWarsReady ( GLMSG::SNET_TOWERWARS_READY_FLD *pNetMsg )
{
	GLTowerWarsFieldMan::GetInstance().ReadyBattle ( pNetMsg->dwID );

	return TRUE;}

BOOL GLGaeaServer::ServerTowerWarsStart ( GLMSG::SNET_TOWERWARS_START_FLD *pNetMsg )
{
	m_sTowerWarsWinner.Init();
	for(int i = 0;i<RANKING_TWNUM; ++i )
	{
	  m_sTowerWarRankWinner[i].dwCharID = 0;
	  m_sTowerWarRankWinner[i].dwClass = GLCI_FIGHTER_M;
	  m_sTowerWarRankWinner[i].dwDamageNum = 0;
	  m_sTowerWarRankWinner[i].dwHealNum  = 0;
	  m_sTowerWarRankWinner[i].nIndex = -1;
	  m_sTowerWarRankWinner[i].wCharRanking = 0;
	  m_sTowerWarRankWinner[i].wDeathNum = 0;
      m_sTowerWarRankWinner[i].wKillNum = 0;
      m_sTowerWarRankWinner[i].wResuNum = 0;
      m_sTowerWarRankWinner[i].wSchool = -1;
	}
	m_pMyScoreRank.Init();
	m_wRankNum = 0;
	m_bParticipate = false;
	m_bTowerWarsStarted = true;
	GLTowerWarsFieldMan::GetInstance().BeginBattle ( pNetMsg->dwID );

	return TRUE;
}

BOOL GLGaeaServer::ServerTowerWarsEnd ( GLMSG::SNET_TOWERWARS_END_FLD *pNetMsg )
{
    m_bRejoin = false;
	m_bParticipate = false;
	m_bTowerWarsStarted = false;

	GLTowerWarsFieldMan::GetInstance().EndBattleFld ( pNetMsg->dwID, pNetMsg->dwSCHOOL, pNetMsg->bForceExit );

	return TRUE;
}

BOOL GLGaeaServer::ServerTowerWarsDoBattleEnd ( GLMSG::SNET_TOWERWARS_DOBATTLEEND_FLD *pNetMsg )
{
	GLTowerWarsFieldMan::GetInstance().EndBattle( pNetMsg->dwID, pNetMsg->dwSCHOOL, pNetMsg->bEnd );

	return TRUE;
}

BOOL GLGaeaServer::ServerTowerWarsCertify ( GLMSG::SNET_TOWERWARS_CERTIFIED_FLD *pNetMsg )
{
	GLTowerWarsFieldMan::GetInstance().ChangeSWSchoolID ( pNetMsg->dwID, pNetMsg->dwSCHOOL );
...
BOOL GLGaeaServer::GlobalUpgrade ( GLMSG::SNET_GLOBAL_SUCCESS_GRIND *pNetMsg )
{
	SNATIVEID nidMAP( 22 , 0 );
	GLLandMan *pLandMan = GetByMapID( nidMAP );
	//GetConsoleMsg()->Write( LOG_TEXT_CONSOLE, "Received Field %d %d",pNetMsg->dwCharID,pNetMsg->dwGrindUp );
	
	if( pLandMan )
	{
		GLMSG::SNET_GLOBAL_SUCCESS_GRIND *NetMsg = (GLMSG::SNET_GLOBAL_SUCCESS_GRIND *) pNetMsg;
		NetMsg->dwCharID = pNetMsg->dwCharID; 
		NetMsg->dwGrindUp = pNetMsg->dwGrindUp;
		NetMsg->sItemCustom = pNetMsg->sItemCustom;
//		NetMsg->szName = pNetMsg->szName;
		GLCHARNODE* pCharNode = pLandMan->m_GlobPCList.m_pHead;
		for ( ; pCharNode; pCharNode = pCharNode->pNext )
		{
...
BOOL GLGaeaServer::RequestInvenRecall ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNETPC_REQ_INVEN_RECALL *pNetMsg )
{
	...
	if ( !pLandMan )					return FALSE;
	if ( !pLandMan->IsCardActivity () || pItem->sDrugOp.emDrug==ITEM_DRUG_CALL_SCHOOL && pItem->sDrugOp.emDrug==ITEM_DRUG_CALL_REGEN && pItem->sDrugOp.emDrug==ITEM_DRUG_CALL_LASTCALL && pItem->sDrugOp.emDrug==ITEM_DRUG_CALL_TELEPORT ) //added cannot use teleport card by DarkEagle
	{
		NetMsgFB.emFB = EMREQ_RECALL_CARD_ITEM;//Anti SP, BP During War
		SENDTOCLIENT ( dwClientID, &NetMsgFB );
		return FALSE;
	}
...
BOOL GLGaeaServer::RequestInvenRecallThisSvr ( GLChar* pPC, SNATIVEID sMAPID, DWORD dwGATEID, D3DXVECTOR3 vPOS )
{
	...
	//	Note : 종전 맵에 있던 관리 노드 삭제.
	//
	if ( pPC->m_pLandMan->m_bTowerWarsMap && pPC->m_dwUserLvl < USER_SPECIAL )
		pPC->m_pLandMan->UpdateDecHPmultiplier ( pPC->m_wSchool );
	if ( pPC->m_pLandNode )
		pPC->m_pLandMan->m_GlobPCList.DELNODE ( pPC->m_pLandNode );
...
BOOL GLGaeaServer::RequestTaxi ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNETPC_REQ_TAXI *pNetMsg )
{
	...
	//add money 2
	NetMsgMoney.lnVoteP = pChar->m_lnVoteP;
	//add money 3
	NetMsgMoney.lnPremP = pChar->m_lnPremP;
	//add contributionpoints by CNDev
	NetMsgMoney.lnContribP = pChar->m_lnContribP;
	//
	NetMsgMoney.lnSaldo = pChar->m_lnSaldo;
...
BOOL GLGaeaServer::RequestClubRank2Fld ( GLMSG::SNET_CLUB_RANK_2FLD *pNetMsg )
{
	...
	if ( pMASTER )
	{
		...
		//add money 2
		NetMsg.lnVoteP = pMASTER->m_lnVoteP;
		//add money 3
		NetMsg.lnPremP = pMASTER->m_lnPremP;
		//add contributionpoints by CNDev
		NetMsg.lnContribP = pMASTER->m_lnContribP;
		//
		NetMsg.lnSaldo = pMASTER->m_lnSaldo;
...
HRESULT GLGaeaServer::MsgProcess ( NET_MSG_GENERIC* nmg, DWORD dwClientID, DWORD dwGaeaID )
{
	...
	switch ( nmg->nType )
	{
	case NET_MSG_REQ_CROWLIST_FLD:
		ReqCrowList ( dwClientID, dwGaeaID, (GLMSG::SNETPC_REQ_CROWLIST_FLD*) nmg );
		break;
	case NET_MSG_CHAT:
	...
	case NET_MSG_GCTRL_REQ_DISABLESKILLEFF:
	case NET_MSG_GCTRL_REQ_LINKSKILLEFF: //Link Skill Effect by NaJ

	case NET_MSG_GCTRL_REQ_RANKNAME: //add pkrank
	case NET_MSG_GCTRL_REQ_RANKMARK: //add pkrank
	case NET_MSG_GCTRL_REQ_ALLOW_BET: //add duel bet

	case NET_MSG_GCTRL_FIRECRACKER:
	case NET_MSG_GCTRL_REQ_INVENDRUG:
	case NET_MSG_GCTRL_INVEN_BOXOPEN:
	case NET_MSG_GCTRL_INVEN_GMITEM: //add itemcmd
	...
	case NET_MSG_GCTRL_REQ_TYRANNY_GETREWARD: 
	case NET_MSG_GCTRL_REQ_LEARNSKILL_NONINVEN: //Activate Skill w/o Scroll
	...
	case NET_MSG_GCTRL_INVEN_BIKECOLOR_CHANGE: //add bike color
	case NET_MSG_GCTRL_SLOT_ITEMCOLOR_CHANGE: //add itemcolor
	...
	case NET_MSG_AP_DT:	//add autopots detection
	...
	case NET_MSG_GCTRL_TRADE:
		...
		break;
    //add charinfoview
	case NET_MSG_GCTRL_VIEW_PROPERTIES:
		if ( dwGaeaID == GAEAID_NULL )		return E_FAIL;
		RequestViewProperties ( dwClientID, dwGaeaID, (GLMSG::SNET_VIEW_PROPERTIES *) nmg );
		break;
	...
	case NET_MSG_GCTRL_TOWERWARS_READY_FLD:
		ServerTowerWarsReady ( (GLMSG::SNET_TOWERWARS_READY_FLD *)nmg );
		break;
	case NET_MSG_GCTRL_TOWERWARS_START_FLD:
		ServerTowerWarsStart ( (GLMSG::SNET_TOWERWARS_START_FLD *)nmg );
		break;
	case NET_MSG_GCTRL_TOWERWARS_END_FLD:
		ServerTowerWarsEnd ( (GLMSG::SNET_TOWERWARS_END_FLD *)nmg );
		break;

	case NET_MSG_GCTRL_TOWERWARS_CERTIFIED_FLD:
		ServerTowerWarsCertify ( (GLMSG::SNET_TOWERWARS_CERTIFIED_FLD *)nmg );
		break;
	case NET_MSG_GCTRL_TOWERWARS_DOBATTLEEND_FLD:
		ServerTowerWarsDoBattleEnd ( (GLMSG::SNET_TOWERWARS_DOBATTLEEND_FLD *)nmg );
		break;
	...
	case NET_MSG_SERVER_CTRL_MONTH:
		...
		{
			MsgReqHoldArray ( dwClientID , dwGaeaID, (GLMSG::SNETPC_REQ_HOLD_ARRAY *) nmg); 		
		}
		break;

		
		case NET_MSG_GCTRL_REQ_HOLDARRAY_TO_INVEN:
		{
			MsgReqHoldArrayToInven ( dwClientID , dwGaeaID, (GLMSG::SNETPC_REQ_HOLDARRAY_TO_INVEN *) nmg); 	
		}
		break;
	...
	case NET_MSG_MOVE2MAPPOS_FLD:
		CtrolMove2MapPos ( dwClientID, dwGaeaID, (GLMSG::SNET_MOVE2MAPPOS_FLD*) nmg );
		break;
	...
	case NET_MSG_TYRANNY_PARTICIPATE:
		CTFParticipate ( dwClientID, dwGaeaID, (GLMSG::SNET_TYRANNY_PARTICIPATE*) nmg );
		break;
	case NET_MSG_PVP_REJOIN: //BattleFieldUI Data Rejoin - Eduj
		CTFRejoin ( dwClientID, dwGaeaID, (GLMSG::SNET_PVP_REJOIN*) nmg );
		break;
	...
	case NET_MSG_GM_SCHOOL_EVENT:
		GMCtrolSchoolEvent ( dwClientID, dwGaeaID, (GLMSG::SNET_GM_SCHOOL_EVENT*) nmg  );		
		break;
	...
	case NET_MSG_GCTRL_TOWERWARS_WINNER_REQ:
		ReqTowerWarsWinner ( dwClientID, dwGaeaID, (GLMSG::SNET_TOWERWARS_WINNER_REQ*) nmg );
		break;
	case NET_MSG_GCTRL_SCHOOLWAR_NEXTWAR_REQ:
		ReqSetNextWarTime ( dwClientID, dwGaeaID, (GLMSG::SNET_SCHOOLWAR_NEXTWAR_REQ*) nmg );
		break;
	case NET_MSG_GCTRL_TOWERWARS_CTFRANKING_REQ:
		ReqTowerWarsCTFRanking ( dwClientID, dwGaeaID, (GLMSG::SNET_TOWERWARS_CTFRANKING_REQ*) nmg );
		break;
	case NET_MSG_GCTRL_TOWERWARS_CTFICON_REQ:
		ReqCtfIconID ( dwClientID, dwGaeaID, (GLMSG::SNET_TOWERWARS_CTFICON_REQ*) nmg );
		break;
//----------------------------------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------------------------------
	...
	case NET_MSG_PET_DUAL_SKILL://you miss this part, this is entry point for all message from client, devide for every need, pet, char and other
		{
			PGLCHAR pOwner = GetChar ( dwGaeaID );
			if ( pOwner )
			{
...
BOOL GLGaeaServer::ReqCrowList ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNETPC_REQ_CROWLIST_FLD* pNetMsg ) 
{
	PGLCHAR pCHAR = GetChar ( pNetMsg->dwGaeaID );
	...
	for ( ; pQuadNode; pQuadNode = pQuadNode->pFindNext )
	{
		...
		GASSERT(pLandTree->GetCellNum()>pQuadNode->dwCID&&" ");
		...
		for ( ; pCrowCur; pCrowCur = pCrowCur->pNext )
		{
			...
			GLMSG::SNETPC_REQ_CROWLIST_FB NetMsgFB;	
			NetMsgFB.bRender = true;
			NetMsgFB.sCrowID = pGLCrow->m_pCrowData->sNativeID;
			NetMsgFB.vPos = pGLCrow->GetPosition();
			GLGaeaServer::GetInstance().SENDTOCLIENT ( pCHAR->m_dwClientID, &NetMsgFB );
		}
	}
	...
	for ( i = 0; i < MAXCROW; i++ )
	{
		...
		if ( pCrowData->m_emCrow != CROW_MOB ) continue;
	...
	for ( ; iter != iter_end; ++iter )
	{
		GLMSG::SNETPC_REQ_CROWLIST_FB NetMsgFB;
		SNATIVEID sID = iter->first;
		NetMsgFB.sCrowID = sID;		
		NetMsgFB.bRender = false;
		GLGaeaServer::GetInstance().SENDTOCLIENT ( pCHAR->m_dwClientID, &NetMsgFB );
	}
