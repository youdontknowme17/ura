...
HRESULT GLChar::MsgReqInvenDrug ( NET_MSG_GENERIC* nmg )
{
	...
	case ITEM_DRUG_CURE:
	case ITEM_DRUG_EMERGENCY_POTION: //added by CNDev
...
HRESULT GLChar::MsgReqInvenBikeColorChange ( NET_MSG_GENERIC* nmg ) //add bike color
{
	...
	// DB에 저장 //jdev todo save color after change
	...
	case EMREBUILD_RESULT_SUCCESS:	
		{
			...
			else
			{
				...
				if( pInvenItem )
				{
					...
					SITEMSEALDATA sData3 = pNetMsg->Data3;
					if ( sData1.nOption != -1 || sData2.nOption != -1 || sData3.nOption != -1 )
					{
						...
						if( sITEM_NEW.GENERATE_RANDOM_OPT() )
						{
							if ( sData1.nOption != -1 )
								...
								if ( sData3.nOption != -1 )
								{
									sITEM_NEW.cOptTYPE3 	= sData2.emOpt;
									sITEM_NEW.nOptVALUE3 	= sData2.fValue;

									if( sITEM_NEW.cOptTYPE3 == EMR_OPT_DAMAGE_RATE || sITEM_NEW.cOptTYPE3 == EMR_OPT_DEFENSE_RATE || sITEM_NEW.cOptTYPE3 == EMR_OPT_HIT_RATE || sITEM_NEW.cOptTYPE3 == EMR_OPT_AVOID_RATE )
										sITEM_NEW.nOptVALUE3 = sData3.fValue * 100.0f;
									
									if( sITEM_NEW.cOptTYPE3 == sITEM_NEW.cOptTYPE4)
									{
										sITEM_NEW.cOptTYPE4 = 0;
										sITEM_NEW.nOptVALUE4 = 0;
									}
...
HRESULT GLChar::ReqPointShop( NET_MSG_GENERIC* nmg )
{
	GLMSG::SNET_INVEN_POINT_SHOP* pNetMsg = (GLMSG::SNET_INVEN_POINT_SHOP*)nmg;
	GLMSG::SNET_INVEN_POINT_SHOP_FB	NetMsgFB;


	SINVENITEM* pInvenItem[POINTSHOP_ITEMNUM];
	...
	//	조합 확인
	const POINT_SHOP* pPointShop = GLPointShopMan::GetInstance().GetPointShop( pNetMsg->dwKey );
	if ( !pPointShop ) 
	{
		NetMsgFB.emFB = EMPOINT_SHOP_FB_NOMIX;
		GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&NetMsgFB);
		return E_FAIL;
	}

	//	금액 확인 
	if( m_lnSaldo < pPointShop->dwPrice )
	{
		NetMsgFB.emFB = EMPOINT_SHOP_FB_NOMONEY;
		GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&NetMsgFB);
		return E_FAIL;
	}
	...
	for ( int i =0; i < POINTSHOP_ITEMNUM; ++i ) 
	{
		...
		if ( !pInvenItem[i] ) 
		{
			NetMsgFB.emFB = EMPOINT_SHOP_FB_NOITEM;
			GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&NetMsgFB);
			return E_FAIL;
		}
	...
	for ( int i =0; i < POINTSHOP_ITEMNUM; ++i ) 
	{
		if ( pPointShop->sTicketItem[i].sNID == NATIVEID_NULL() ) continue;

		if ( pPointShop->sTicketItem[i].nNum != pInvenItem[i]->sItemCustom.wTurnNum )
		{
			NetMsgFB.emFB = EMPOINT_SHOP_FB_NOMIXNUM;
			GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&NetMsgFB);
			return E_FAIL;
		}	
	}
	...
	//	결과 아이템 체크
	SITEM* pItemResult = GLItemMan::GetInstance().GetItem( pPointShop->sResultItem.sNID );
	if ( !pItemResult ) 
	{
		NetMsgFB.emFB = EMPOINT_SHOP_FB_NOITEM;
		GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&NetMsgFB);
	...
	{
		//	불가능한 경우 입니다.
		NetMsgFB.emFB = EMPOINT_SHOP_FB_NOINVEN;
		GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&NetMsgFB);
		return E_FAIL;	//	인벤 공간 부족으로 되돌림 실패.
	}
	...
	for ( int i =0; i < POINTSHOP_ITEMNUM; ++i )
	{
		...
		GLITEMLMT::GetInstance().ReqItemRoute( pInvenItem[i]->sItemCustom, ID_CHAR, m_dwCharID, ID_CHAR, 0, 
			EMITEM_ROUTE_POINTSHOP, pInvenItem[i]->sItemCustom.wTurnNum );
	...
	//	소지금 
	if ( pPointShop->dwPrice > 0 )
	{
		CheckMoneyUpdate( m_lnSaldo, pPointShop->dwPrice, FALSE, "POINT_SHOP" );
		m_bMoneyUpdate = TRUE;

		m_lnSaldo -= pPointShop->dwPrice;
	...
	//	실패했을 경우
	float fRate = (float)pPointShop->dwRate;
	if ( !RANDOM_GEN( fRate ) )
	{
		NetMsgFB.emFB = EMPOINT_SHOP_FB_FAIL;
		GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&NetMsgFB);
		return S_OK;	
	}
	...
	//	결과 아이템 생성
	SITEMCUSTOM sITEM_NEW;
	sITEM_NEW.sNativeID = pPointShop->sResultItem.sNID;
	sITEM_NEW.tBORNTIME = CTime::GetCurrentTime().GetTime();
	sITEM_NEW.wTurnNum = pPointShop->sResultItem.nNum;
	sITEM_NEW.cGenType = EMGEN_POINTSHOP;
	sITEM_NEW.cChnID = (BYTE)m_pGLGaeaServer->GetServerChannel();
	sITEM_NEW.cFieldID = (BYTE)m_pGLGaeaServer->GetFieldSvrID();
	sITEM_NEW.lnGenNum = GLITEMLMT::GetInstance().RegItemGen ( sITEM_NEW.sNativeID, (EMITEMGEN)sITEM_NEW.cGenType );
	...
	GLITEMLMT::GetInstance().ReqItemRoute ( pINSERT_ITEM->sItemCustom, ID_CHAR, 0, ID_CHAR, m_dwCharID, 
		EMITEM_ROUTE_POINTSHOP, pINSERT_ITEM->sItemCustom.wTurnNum );
	...
	NetMsgFB.emFB = EMPOINT_SHOP_FB_OK;
	NetMsgFB.sNativeID	= pINSERT_ITEM->sItemCustom.sNativeID;
	NetMsgFB.wTurnNum = pINSERT_ITEM->sItemCustom.wTurnNum;
	GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&NetMsgFB);
...
HRESULT GLChar::MsgReqTopUp(NET_MSG_GENERIC* nmg)
{
	GLMSG::SNETPC_REQ_TOPUP *pNetMsg = (GLMSG::SNETPC_REQ_TOPUP *)nmg;

	if (pNetMsg->lnSaldo == 0)				return S_FALSE;

	m_lnSaldo += pNetMsg->lnSaldo;

	// CNDev will add this later ^^
	/*if (m_bTracingUser)
	{
		NET_LOG_UPDATE_TRACINGCHAR TracingMsg;
		TracingMsg.nUserNum = GetUserID();
	...
	GLMSG::SNETPC_UPDATE_EPOINT NetMsg;
	NetMsg.lnSaldo = m_lnSaldo;
