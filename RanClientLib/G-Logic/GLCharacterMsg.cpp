...
#include "../RanClientUILib/Interface/NameDisplayMan.h"
...
// #include "../RanClientUILib/Interface/CharacterWindow.h"
#include "../RanClientUILib/Interface/CharacterWindow/CharacterWindow.h"
#include "../RanClientUILib/Interface/QBoxButton.h"
#include "../RanClientUILib/Interface/ItemSearchResultWindow.h"
#include "../../EngineSoundLib/DxSound/DxSoundLib.h"
#include "../../enginelib/DxEffect/DxEffectMan.h"
#include "../../enginelib/DxEffect/Single/DxEffGroupPlayer.h"
#include "../RanClientUILib/Interface/WaitServerDialogue.h"
#include "../RanClientUILib/Interface/MapWindow/LargeMapWindow.h"
#include "../RanClientUILib/Interface/ItemRebuild/ItemRebuild.h" //sealed card 
#include "GLItemMan.h"
...
void GLCharacter::ReceiveDamage ( DWORD wDamage, DWORD dwDamageFlag, STARGETID sACTOR )
{
...
void GLCharacter::MsgProcess ( NET_MSG_GENERIC* nmg )
{
	...
	{
	////////////////////////////////////////////////////////////////////////
	case NET_MSG_GCTRL_UPDATE_EPOINT:
	{
		GLMSG::SNETPC_UPDATE_EPOINT *pNetMsg = (GLMSG::SNETPC_UPDATE_EPOINT *)nmg;

		m_lnSaldo = pNetMsg->lnSaldo;
		CInnerInterface::GetInstance().PrintConsoleText ( "Your Total Premium Points is now %d Premium Points.", pNetMsg->lnSaldo);
	}
	break;
	//////////////////////////////////////////
	//  AutoPilot
	case NET_MSG_GCTRL_UPDATE_USE_AUTO_SKILL:
	{
		GLMSG::SNETPC_UPDATE_USE_AUTO_SKILL *pNetMsg = (GLMSG::SNETPC_UPDATE_USE_AUTO_SKILL *) nmg;
		...
		case TRUE:
		{
			...
			CInnerInterface::GetInstance().SetIsUsingAutoSkillOn();
			CInnerInterface::GetInstance().PrintMsgText( NS_UITEXTCOLOR::GREEN, "AutoSkill has been activated ");
		}
		break;
		...
		case FALSE:
		{
			...
			CInnerInterface::GetInstance().SetIsUsingAutoSkillOff();
			CInnerInterface::GetInstance().PrintMsgText( NS_UITEXTCOLOR::ADMIN_COLOR, "AutoSkill has been deactivated");
		}
		break;
		}
	}
	...
	case NET_MSG_GCTRL_REQ_TYRANNY_GETREWARD_FB: //convert
		{
			GLMSG::SNETPC_REQ_TYRANNY_GETREWARD_FB *pNetMsg = (GLMSG::SNETPC_REQ_TYRANNY_GETREWARD_FB *)nmg;
			GETREWARD(pNetMsg->lnValue);
			CInnerInterface::GetInstance().PrintConsoleText( "You have received: %d Gold.", pNetMsg->lnValue );
		}
		break;
	...
	case NET_MSG_REQ_CROWLIST_FB:
		{
			...
			CInnerInterface::GetInstance().GetLargeMapWindow()->LoadMob(nmg, this);
		}
		break;
	...
	case NET_MSG_GCTRL_UPDATE_MONEY:
		{
			...
			//add money 2
			m_lnVoteP = pNetMsg->lnVoteP;
			//add money 3
			m_lnPremP = pNetMsg->lnPremP;
			//add contributionpoints by CNDev
			m_lnContribP = pNetMsg->lnContribP;
			//
			m_lnSaldo = pNetMsg->lnSaldo;
		}
		break;
	...
	case NET_MSG_GCTRL_UPDATE_PKWIN:
		{
			GLMSG::SNETPC_UPDATE_PKWIN *pNetMsg = (GLMSG::SNETPC_UPDATE_PKWIN *)nmg;
			PKWINUP();
			CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("PK_KILLED"), pNetMsg->strKilled.c_str() );	
			//add pk streak
			...
			else if( m_dwPkCounter >= 14) CInnerInterface::GetInstance().SET_PLAYERKILL_STREAK_ID ( PK_OWNAGE );
		}	
		break;

	case NET_MSG_GCTRL_UPDATE_PKLOSS:
		{
			GLMSG::SNETPC_UPDATE_PKLOSS *pNetMsg = (GLMSG::SNETPC_UPDATE_PKLOSS *)nmg;
			PKLOSSUP();
			CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("PK_KILLEDBY"), pNetMsg->strKilledBy.c_str() );		
		}	
		break;

	case NET_MSG_GCTRL_UPDATE_DUELWIN: //add duel bet
		{
			GLMSG::SNETPC_UPDATE_DUELWIN *pNetMsg = (GLMSG::SNETPC_UPDATE_DUELWIN *)nmg;
			DuelWinReward ( pNetMsg->llWin, pNetMsg->wType );
			CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("WIN_DUEL_AMMOUNT"), pNetMsg->llWin );		
		}	
		break;

	case NET_MSG_GCTRL_UPDATE_DUELLOSS: //add duel bet
		{
			GLMSG::SNETPC_UPDATE_DUELLOSS *pNetMsg = (GLMSG::SNETPC_UPDATE_DUELLOSS *)nmg;
			DuelLossReward ( pNetMsg->llLoss, pNetMsg->wType );
			CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("LOSS_DUEL_AMMOUNT"), pNetMsg->llLoss );		
		}	
		break;
	...
	case NET_MSG_GCTRL_UPDATE_CP: //add cp
		{
			...
			CP_UP(pNetMsg->dwRCLvl,pNetMsg->wRCNum);	
		}	
		break;
	...
	case NET_MSG_GCTRL_PICKUP_MONEY:
		{
			...
			m_lnMoney = pNetMsg->lnMoney;
			if( GLGaeaClient::GetInstance().m_nCTFWinner == m_wSchool)		CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::AQUA, "Pick Up %d Gold(+Tyranny Reward)", pNetMsg->lnPickUp );
			else								CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("PICKUP_MONEY"), pNetMsg->lnPickUp );
...
*/
	...
	case NET_MSG_GCTRL_CANNOT_POT: //add antipot
		{
			GLMSG::SNET_ITEM_CANNOT_POT *pNetMsg = (GLMSG::SNET_ITEM_CANNOT_POT *)nmg;
			
			CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("CANNOT_USE_POT") );
		}
		break;
...
*/
	...
	case NET_MSG_GCTRL_PUTON_UPDATE:
		{
			...
			{
				DWORD dwACP = GLCHARLOGIC::CALC_ACCEPTP ( pNetMsg->sItemCustom.sNativeID );
				if ( dwACP > 0 )		CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("ITEMPUTON_ACCEPT_SP"), dwACP );	
	...
	case NET_MSG_GCTRL_REQ_LEARNSKILL_NONINVEN_FB:
		{
			...
			{
				CInnerInterface::GetInstance().PrintMsgText( NS_UITEXTCOLOR::WHITE, "pfft" );
			}
		}
		break;

	case NET_MSG_GCTRL_UPDATE_MONEYTYPE: //Update Any money type
		{
			GLMSG::SNETPC_UPDATE_MONEYTYPE *pNetMsg = (GLMSG::SNETPC_UPDATE_MONEYTYPE *)nmg;

			int nGold = int(pNetMsg->lnGold) - int(m_lnMoney);
			int nPremP = int(pNetMsg->lnPremP) - int(m_lnPremP);
			int nVoteP = int(pNetMsg->lnVoteP) - int(m_lnVoteP);
			int nContribP = int(pNetMsg->lnContribP) - int(m_lnContribP);
			int nSaldo = int(pNetMsg->lnSaldo) - int(m_lnSaldo);

			m_lnMoney = pNetMsg->lnGold;
			m_lnPremP = pNetMsg->lnPremP;
			m_lnVoteP = pNetMsg->lnVoteP;
			m_lnContribP = pNetMsg->lnContribP;
			m_lnSaldo = pNetMsg->lnSaldo;
	...
	case NET_MSG_GCTRL_REQ_DISABLESKILLEFF_FB:
		...
		break;
	case NET_MSG_GCTRL_REQ_LINKSKILLEFF_FB: //Link Skill Effect by NaJ
		{
			GLMSG::SNETPC_REQ_LINKSKILLEFF_FB *pNetMsg = (GLMSG::SNETPC_REQ_LINKSKILLEFF_FB *)nmg;
			...
			{
				/*SKILL::CDATA_LVL &sSKILL_DATA = pSkillLink->m_sAPPLY.sDATA_LVL[0];
				SSKILLFACT sSKILLEF;
				sSKILLEF.emSPEC		= pSkillLink->m_sAPPLY.emSPEC;
				sSKILLEF.fSPECVAR1	= pSkillLink->m_sAPPLY.sSPEC[0].fVAR1;
				sSKILLEF.fSPECVAR2	= pSkillLink->m_sAPPLY.sSPEC[0].fVAR2;
				sSKILLEF.dwSPECFLAG = pSkillLink->m_sAPPLY.sSPEC[0].dwFLAG;
				sSKILLEF.dwNativeID = pSkillLink->m_sAPPLY.sSPEC[0].dwNativeID;
				sSKILLEF.sNATIVEID	= pSkillLink->m_sBASIC.sNATIVEID;
				sSKILLEF.wLEVEL		= 0;
				sSKILLEF.fAGE		= sSKILL_DATA.fLIFE;
				m_sSKILLFACT[pNetMsg->dwSelect] = sSKILLEF;

				DoTransform(pSkillLink);*/
			}
		}
		break;

	case NET_MSG_GCTRL_REQ_RANKNAME_FB: //add pkrank
		{
	...
	case NET_MSG_GCTRL_REQ_RANKMARK_FB: //add pkrank
		{
	...
	case NET_MSG_GCTRL_REQ_ALLOW_BET_FB: //add duel bet
		{
			GLMSG::SNETPC_REQ_ALLOW_BET_FB *pNetMsg = (GLMSG::SNETPC_REQ_ALLOW_BET_FB *)nmg;
	...
	case NET_MSG_GCTRL_SKILLCONSUME_FB:
		{
			...
			m_sHP.dwNow = pNetMsg->wNowHP;
			m_sMP.dwNow = pNetMsg->wNowMP;
			m_sSP.dwNow = pNetMsg->wNowSP;

			m_sCP.dwNow = pNetMsg->wNowCP; //add cp
		}
		break;
...
*/
	...
	case NET_MSG_GCTRL_INVEN_GRINDING_FB:
		...
		{
			GLMSG::SNET_GLOBAL_SUCCESS_GRIND *pNetMsg = (GLMSG::SNET_GLOBAL_SUCCESS_GRIND *)nmg;
			SITEM* pItem = GLItemMan::GetInstance().GetItem ( pNetMsg->sItemCustom.sNativeID );
			
			if ( pItem )
			{
				if ( pNetMsg->dwGrindUp > 9 )
				{
					CString strTemp = "";
					CString strTemp2 = pItem->GetName();
					SITEMLINK sLINK;
					SITEMCUSTOM sITEMCUSTOM;
					sITEMCUSTOM = pNetMsg->sItemCustom;
					sITEMCUSTOM.Convert( sLINK );
					sLINK.BuildFilter( strTemp2 );
					strTemp.Format("%s has upgrade %s to [+%d]",pNetMsg->szName,sLINK.GetFilter(),pNetMsg->dwGrindUp);
					CInnerInterface::GetInstance().PrintMsgDlg ( NS_UITEXTCOLOR::PALEGREEN, "%s has Upgraded %s to [+%d]!",pNetMsg->szName,pItem->GetName(),pNetMsg->dwGrindUp);
				}
			}
		}
		break;
			...
			case EMREGEN_CHARRESET_SUCCEED:
				{
					...
					m_wStatsPoint = (DWORD) pNetMsg->dwSTATS_P;
...
//			CInnerInterface::GetInstance().SET_VNGAINTYPE_GAUGE( 0, 300 );
	...
	case NET_MSG_MOVE2MAPPOS_FB:
		{
			GLMSG::SNET_MOVE2MAPPOS_FB *pNetMsg = (GLMSG::SNET_MOVE2MAPPOS_FB *) nmg;
			SetPosition ( pNetMsg->vPOS );
			//DoActWait ();
		}
		break;
...
// 경험치회복_정의_Npc
	...
	case NET_MSG_GCTRL_PLAYERKILLING_ADD:
		{
			...
			if ( !( bBRIGHTEVENT||bSCHOOLFREEPK||pLand->m_bClubBattle||pLand->m_bClubDeathMatch||pLand->m_bSchoolWars||pLand->m_bRoyalRumble ) )
			{
	...
	case NET_MSG_GCTRL_INVEN_BIKECOLOR_CHANGE_FB: //add bike color
		{
			GLMSG::SNETPC_INVEN_BIKECOLOR_CHANGE_FB *pNetMsg = (GLMSG::SNETPC_INVEN_BIKECOLOR_CHANGE_FB*)nmg;

			switch ( pNetMsg->emFB )
			{
			case EMINVEN_BIKE_CHANGE_FB_FAIL:
				BikeColorChange( m_sVehicle.m_wColor,m_sVehicle.m_wColor1,m_sVehicle.m_wColor2,m_sVehicle.m_wColor3,m_sVehicle.m_wColor4,m_sVehicle.m_wColor5 ); 

				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("EMINVEN_BIKE_CHANGE_FB_FAIL") );
				break;
			case EMINVEN_BIKE_CHANGE_FB_OK:
				{
					...
					CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EMINVEN_BIKE_CHANGE_FB_OK") );
				}
				break;
			case EMINVEN_BIKE_CHANGE_FB_NOITEM:
				BikeColorChange( m_sVehicle.m_wColor,m_sVehicle.m_wColor1,m_sVehicle.m_wColor2,m_sVehicle.m_wColor3,m_sVehicle.m_wColor4,m_sVehicle.m_wColor5 ); 

				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("EMINVEN_BIKE_CHANGE_FB_NOITEM") );
				break;
			case EMINVEN_BIKE_CHANGE_FB_BADITEM:
				BikeColorChange( m_sVehicle.m_wColor,m_sVehicle.m_wColor1,m_sVehicle.m_wColor2,m_sVehicle.m_wColor3,m_sVehicle.m_wColor4,m_sVehicle.m_wColor5 ); 

				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("EMINVEN_BIKE_CHANGE_FB_BADITEM") );
				break;
			case EMINVEN_BIKE_CHANGE_FB_BADCLASS:
				BikeColorChange( m_sVehicle.m_wColor,m_sVehicle.m_wColor1,m_sVehicle.m_wColor2,m_sVehicle.m_wColor3,m_sVehicle.m_wColor4,m_sVehicle.m_wColor5 ); 

				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("EMINVEN_BIKE_CHANGE_FB_BADCLASS") );
				break;
			case EMINVEN_BIKE_CHANGE_FB_COOLTIME:
				BikeColorChange( m_sVehicle.m_wColor,m_sVehicle.m_wColor1,m_sVehicle.m_wColor2,m_sVehicle.m_wColor3,m_sVehicle.m_wColor4,m_sVehicle.m_wColor5 ); 

				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("EMINVEN_BIKE_CHANGE_FB_COOLDOWN") );
				break;
			};
		}
		break;

	case NET_MSG_GCTRL_SLOT_ITEMCOLOR_CHANGE_FB: //add itemcolor
		{
			GLMSG::SNETPC_SLOT_ITEMCOLOR_CHANGE_FB *pNetMsg = (GLMSG::SNETPC_SLOT_ITEMCOLOR_CHANGE_FB*)nmg;

			switch ( pNetMsg->emFB )
			{
			case EMINVEN_ITEMCOLOR_CHANGE_FB_FAIL:
				m_ColorSlot = pNetMsg->emSlot;
				ItemColorChange( m_PutOnItems[pNetMsg->emSlot].wColor ); 
				m_ColorSlot = SLOT_TSIZE;
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("EMINVEN_ITEMCOLOR_CHANGE_FB_FAIL") );
				break;
			case EMINVEN_ITEMCOLOR_CHANGE_FB_OK:
				{
					m_PutOnItems[pNetMsg->emSlot].wColor = pNetMsg->wItemColor;
					UpdateSuit();
					CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EMINVEN_ITEMCOLOR_CHANGE_FB_OK") );
				}
				break;
			case EMINVEN_ITEMCOLOR_CHANGE_FB_NOITEM:
				m_ColorSlot = pNetMsg->emSlot;
				ItemColorChange( m_PutOnItems[pNetMsg->emSlot].wColor ); 
				m_ColorSlot = SLOT_TSIZE;
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("EMINVEN_ITEMCOLOR_CHANGE_FB_NOITEM") );
				break;
			case EMINVEN_ITEMCOLOR_CHANGE_FB_BADITEM:
				m_ColorSlot = pNetMsg->emSlot;
				ItemColorChange( m_PutOnItems[pNetMsg->emSlot].wColor ); 
				m_ColorSlot = SLOT_TSIZE;
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("EMINVEN_ITEMCOLOR_CHANGE_FB_BADITEM") );
				break;
			case EMINVEN_ITEMCOLOR_CHANGE_FB_BADCLASS:
				m_ColorSlot = pNetMsg->emSlot;
				ItemColorChange( m_PutOnItems[pNetMsg->emSlot].wColor ); 
				m_ColorSlot = SLOT_TSIZE;
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("EMINVEN_ITEMCOLOR_CHANGE_FB_BADCLASS") );
				break;
			case EMINVEN_ITEMCOLOR_CHANGE_FB_COOLTIME:
				m_ColorSlot = pNetMsg->emSlot;
				ItemColorChange( m_PutOnItems[pNetMsg->emSlot].wColor ); 
				m_ColorSlot = SLOT_TSIZE;
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("EMINVEN_ITEMCOLOR_CHANGE_FB_COOLDOWN") );
				break;
			};
		}
		break;
	...
	case NET_MSG_GCTRL_QITEMFACT_BRD:
		{
			...
			//add qbox
			CInnerInterface::GetInstance().SetQBoxTime ( pNetMsg->sFACT.fTime );
	...
	case NET_MSG_GCTRL_QITEMFACT_TIMEUPDATE_BRD: //add qbox
		{
			...
			CInnerInterface::GetInstance().UpdateQBoxTime ( pNetMsg->fQBoxTime );
		}
		break;
	...
	case NET_MSG_REBUILD_RESULT: //sealed card 
	{
	...
	case NET_MSG_GCTRL_SET_SERVER_CRASHTIME_FB:
		...
		case NET_MSG_AP_DT_FB://add autopots detection
				{
				GLMSG::SNET_AP_DT_FB *pNetMsg = (GLMSG::SNET_AP_DT_FB*) nmg;
				if ( pNetMsg->emFB == DETECTED )
				{
						CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::NEGATIVE, ("Illegal Thirt Party Program Detected!") );
				}
		         
				GLMSG::SNET_AP_DT_BRD NetMsgBrd;
				NetMsgBrd.emFB = DETECTED;
	...
	case NET_MSG_GM_SHOWMETHEMONEY:
		...
		//add charinfoview
    case NET_MSG_GCTRL_VIEW_PROPERTIES_FB:
		{
		//GLMSG::SNET_VIEW_PROPERTIES_FB *pNetMsg = (GLMSG::SNET_VIEW_PROPERTIES_FB*) nmg;
		//SDROP_CHAR &pTarCharD = pNetMsg->Data;
          CInnerInterface::GetInstance().ViewpropertiseNetTo( nmg );
...
//				CInnerInterface::GetInstance().WAITSERVER_DIALOGUE_OPEN ( ID2GAMEINTEXT("CHINA_GAIN_CLOSEGAME"), WAITSERVER_CLOSEGAME, 30 );
			...
			case EMVEHICLE_GET_FB_OK:
				{
					...
					m_sVehicle.m_wColor	= pNetMsg->wColor; //add bike color
					m_sVehicle.m_wColor1	= pNetMsg->wColor1;
					m_sVehicle.m_wColor2	= pNetMsg->wColor2;
					m_sVehicle.m_wColor3	= pNetMsg->wColor3;
					m_sVehicle.m_wColor4	= pNetMsg->wColor4;
					m_sVehicle.m_wColor5	= pNetMsg->wColor5;
	...
	case NET_MSG_REQ_ATTEND_REWARD_CLT:
		...
		break;
		//add clubwar reward
	case NET_MSG_REQ_CLUBWAR_REWARD_CLT:
		{
			GLMSG::SNETPC_REQ_CLUBWAR_REWARD_CLT* pNetMsg = ( GLMSG::SNETPC_REQ_CLUBWAR_REWARD_CLT* ) nmg;

			SITEM* pItem = GLItemMan::GetInstance().GetItem( pNetMsg->idClubWarReward );
			
			if ( pItem )
			{
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("CLUBWAR_REWARD_ITEM"), pItem->GetName()  );
			}
			else
			{
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("CLUBWAR_REWARD_NOITEM") );
			}
	...
	case NET_MSG_GCTRL_INVEN_ITEM_MIX_FB:
		...
		break;
	case NET_MSG_GCTRL_INVEN_POINT_SHOP_FB:
		{
			GLMSG::SNET_INVEN_POINT_SHOP_FB* pNetMsg = ( GLMSG::SNET_INVEN_POINT_SHOP_FB* ) nmg;
			CString strMsg;
			...
			switch( pNetMsg->emFB )
			{
			case EMPOINT_SHOP_FB_FAIL:
				{				
					strMsg = ID2GAMEINTEXT("EMPOINT_SHOP_FB_FAIL");
					bFail = true;
				}
				break;
			case EMPOINT_SHOP_FB_OK:
				{				
					SITEM* pItem = GLItemMan::GetInstance().GetItem( pNetMsg->sNativeID );
					if ( pItem ) 
					{
						strMsg = CInnerInterface::GetInstance().MakeString ( ID2GAMEINTEXT("EMPOINT_SHOP_FB_OK") , pItem->GetName(), pNetMsg->wTurnNum );
						dwColor = NS_UITEXTCOLOR::PALEGREEN;
						bSucc = true;
					}					
				}				
				break;
			case EMPOINT_SHOP_FB_ERROR:
				{				
					strMsg = ID2GAMEINTEXT("EMPOINT_SHOP_FB_ERROR");
				}
				break;				
			case EMPOINT_SHOP_FB_NOMIX:
				strMsg = ID2GAMEINTEXT("EMPOINT_SHOP_FB_NOMIX");
				break;
			case EMPOINT_SHOP_FB_NOMIXNUM:
				strMsg = ID2GAMEINTEXT("EMPOINT_SHOP_FB_NOMIXNUM");
				break;
			case EMPOINT_SHOP_FB_NOITEM:
				strMsg = ID2GAMEINTEXT("EMPOINT_SHOP_FB_NOITEM");
				break;
			case EMPOINT_SHOP_FB_NOMONEY:
				strMsg = ID2GAMEINTEXT("EMPOINT_SHOP_FB_NOMONEY");
				break;
			case EMPOINT_SHOP_FB_NOINVEN:
				strMsg = ID2GAMEINTEXT("EMPOINT_SHOP_FB_NOINVEN");
				break;
			
			}
			
			CInnerInterface::GetInstance().PrintMsgText ( dwColor, strMsg );
			CInnerInterface::GetInstance().SetBuyItem( strMsg, bSucc, bFail );
