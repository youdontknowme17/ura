...
#include "../../EngineLib/Meshs/DxSkinPieceContainer.h"
#include "../../EngineLib/Meshs/DxSkinCharData.h"
...
DxSkinPiece* GLItemMan::LoadSkinPiece ( WORD wMID, WORD wSID, EMCHARINDEX emIndex )
{
	...
	if ( !pItem->m_pSkinPiece )
	{
		...
		pItem->m_pSkinPiece[emIndex] = DxSkinPieceContainer::GetInstance().LoadPiece( pItem->m_sItem.GetWearingFile2(emIndex), m_pd3dDevice, TRUE );
	}

	return pItem->m_pSkinPiece[emIndex];
}
//add abl
DxSkinPieceRootData* GLItemMan::GetABL ( WORD wMID, WORD wSID, EMCPSINDEX emCpsInd )
{
	GASSERT(wMID<MAX_MID);
	GASSERT(wSID<MAX_SID);
	GASSERT(m_pd3dDevice&&"장치가 초기화되지 않았음!");
	...
	if ( !pItem->m_pSkinPieceRoot )
	{
		pItem->m_pSkinPieceRoot[emCpsInd] = DxSkinPieceRootDataContainer::GetInstance().LoadData( pItem->m_sItem.GetCpsFile(emCpsInd), m_pd3dDevice, TRUE );
	}

	return pItem->m_pSkinPieceRoot[emCpsInd];
}
	...
	for ( WORD wSID=0; wSID<MAX_SID; wSID++ )
	{
		if ( m_ppItem[wMID][wSID] )
		{
			...
			if ( m_ppItem[wMID][wSID]->m_pSkinPiece )
			{
				...
				for ( int i=0; i<GLCI_NUM_2015; i++ )
				{
					DxSkinPieceContainer::GetInstance().ReleasePiece ( m_ppItem[wMID][wSID]->m_sItem.GetWearingFile2((EMCHARINDEX)i) );
					m_ppItem[wMID][wSID]->m_pSkinPiece[i] = NULL;
				}
			}
			//add abl
			if ( m_ppItem[wMID][wSID]->m_pSkinPieceRoot )
			{
				for ( int i=0; i<CPSIND_SIZE; i++ )
				{
					DxSkinPieceRootDataContainer::GetInstance().ReleaseData ( m_ppItem[wMID][wSID]->m_sItem.GetCpsFile((EMCPSINDEX)i) );
					m_ppItem[wMID][wSID]->m_pSkinPieceRoot[i] = NULL;
				}
			}
...
//add abl
static BOOL ISFILE_ABL_EXIST ( CString strFile )
{
	CString strFullName;
	strFullName = DxSkinPieceRootDataContainer::GetInstance().GetPath() + strFile;
...
HRESULT GLItemMan::SyncUpdateData ()
{
	...
	//add class
	char strCHAR_FLAGS[GLCI_NUM_2015][256] =
	{
		"Brawler[M]",	//격투부
		"Swordsman[M]",	//검도부
		"Archer[W]",	//양궁부
	...
	for ( int j = 0; j < MAX_SID; j++ )
	{
		...
		for ( int nClass=0; nClass<GLCI_NUM_2015; ++nClass )
		{
			if ( sITEM.sBasicOp.dwReqCharClass & nCHAR_FLAGS[nClass] )
			{
				...
				else if ( sITEM.GetWearingFile2((EMCHARINDEX)nClass) )
				{	//'착용메시'가 설정되지 않음
					CDebugSet::ToFile ( _LOGFILE, "[%03d][%03d][%s][%s] - not setting wear mesh ", i, j, sITEM.GetName(), strCHAR_FLAGS[nClass] );
				}
				...
				else if ( !ISFILE_CPS_EXIST(sITEM.GetWearingFile2((EMCHARINDEX)nClass)) )
				{	//설정한 '착용메시' 없음
					CDebugSet::ToFile ( _LOGFILE, "[%03d][%03d][%s][%s] - not search wear mesh [%s]", i, j, sITEM.GetName(), strCHAR_FLAGS[nClass], sITEM.GetWearingFile2((EMCHARINDEX)nClass) );
				}
			}
		}
	}
...
HRESULT GLItemMan::SaveCsvFile_old ( CWnd* pWnd )
{
	CFileDialog dlg( FALSE, ".csv", NULL, OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT,
		"csv file (*.csv)|*.csv|", pWnd );
	...
	// Csv Head
	SITEM::SaveCsvHead_old( streamFILE );
