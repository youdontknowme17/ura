...
enum EMPOINT_SHOP_FB
{
	EMPOINT_SHOP_FB_FAIL				= 0,	//	실패
	EMPOINT_SHOP_FB_OK				= 1,	//	성공
	EMPOINT_SHOP_FB_ERROR				= 2,	//	일반적인 오류
	EMPOINT_SHOP_FB_NOMIX				= 3,	//	없는 조합
	EMPOINT_SHOP_FB_NOMIXNUM			= 4,	//	조합에 수량이 맞지 않습니다.
	EMPOINT_SHOP_FB_NOITEM			= 5,	//	아이템 없음	
	EMPOINT_SHOP_FB_NOMONEY			= 6,	//	돈없음
	EMPOINT_SHOP_FB_NOINVEN			= 7,	//	인벤 위치 부족
};
...
namespace GLMSG
{
	...
	struct SNET_GLOBAL_SUCCESS_GRIND
	{
		NET_MSG_GENERIC		nmg;
		DWORD				dwCharID;
		DWORD				dwGrindUp;
		...
		SNET_GLOBAL_SUCCESS_GRIND()
		:dwCharID(0),
		dwGrindUp(0)
		{
			nmg.dwSize = sizeof(SNET_GLOBAL_SUCCESS_GRIND);
			nmg.nType = NET_MSG_GCTRL_GLOBAL_SUCCESS_GRIND;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};
	...
	struct SNET_ITEM_CANNOT_POT //add antipot
	{
		NET_MSG_GENERIC		nmg;

		SNET_ITEM_CANNOT_POT () 
		{
			nmg.dwSize = sizeof(SNET_ITEM_CANNOT_POT);
			nmg.nType = NET_MSG_GCTRL_CANNOT_POT;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};
	...
	// 아이템 조합
	struct SNET_INVEN_POINT_SHOP
	{
		NET_MSG_GENERIC		nmg;

		DWORD				dwKey;
		SINVEN_POS			sInvenPos[POINTSHOP_ITEMNUM];
		DWORD				dwNpcID;

		SNET_INVEN_POINT_SHOP()
			: dwKey( UINT_MAX )
			, dwNpcID( UINT_MAX )
		{
			nmg.dwSize = sizeof( SNET_INVEN_POINT_SHOP );
			nmg.nType = NET_MSG_GCTRL_INVEN_POINT_SHOP;
			GASSERT( nmg.dwSize <= NET_DATA_BUFSIZE );
		}
	};

	// 아이템 조합
	struct SNET_INVEN_POINT_SHOP_FB
	{
		NET_MSG_GENERIC		nmg;

		EMPOINT_SHOP_FB		emFB;
		SNATIVEID			sNativeID;
		WORD				wTurnNum;


		SNET_INVEN_POINT_SHOP_FB()
			: emFB( EMPOINT_SHOP_FB_FAIL )
			, sNativeID(false)
			, wTurnNum( 0 ) 
		{
			nmg.dwSize = sizeof( SNET_INVEN_POINT_SHOP );
			nmg.nType = NET_MSG_GCTRL_INVEN_POINT_SHOP_FB;
			GASSERT( nmg.dwSize <= NET_DATA_BUFSIZE );
		}
	};
