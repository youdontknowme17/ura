...
HRESULT GLPetField::MsgChangeActiveSkill ( NET_MSG_GENERIC* nmg )
{
	...
	EMPET_SKILL_ORDER order = pNetMsg->dSkillOrder;

	if (order == EMPET_SKILL_ORDER_FIRST)
	{
		if (CheckDoubleSkill(pNetMsg))
		{
			pNetMsg->sSkillID = NATIVEID_NULL();
		}
		...
		if ( m_sActiveSkillID != NATIVEID_NULL() )
		{
			...
			CSetPetSkill* pDbAction = new CSetPetSkill ( m_pOwner->m_dwCharID, m_dwPetID, sPetSkill, false, 0 );
		...
		if ( pNetMsg->sSkillID == NATIVEID_NULL() )
		{
			...
			NetMsgRemoveSkill.sNATIVEID = m_sActiveSkillID;
			NetMsgRemoveSkill.dSkillOrder = EMPET_SKILL_ORDER_FIRST;
		...
		NetMsgAddSkill.dwSkillOrder = EMPET_SKILL_ORDER_FIRST;
		...
		// DB에 스킬 변경 저장
		CSetPetSkill* pDbAction = new CSetPetSkill ( m_pOwner->m_dwCharID, m_dwPetID, sPetSkill, true, 0 );
		GLDBMan* pDBMan = GLGaeaServer::GetInstance().GetDBMan ();
		if ( pDBMan ) pDBMan->AddJob ( pDbAction );

		return S_OK;
	}
	else if (order == EMPET_SKILL_ORDER_SECOND)
	{
		if (m_bDualSkill == FALSE) 
		{
			pNetMsg->sSkillID = NATIVEID_NULL();

			GLMSG::SNETPET_REQ_DUAL_SKILL_FB pNetMsgFB;
			pNetMsgFB.emFB = EMPET_REQ_DUAL_SKILL_FB_INVALID_SKILL;
			GLGaeaServer::GetInstance().SENDTOCLIENT(m_pOwner->m_dwClientID,&pNetMsgFB);
		}
		...
		if ( m_sActiveSkillID2 != NATIVEID_NULL() )
		{
			if( m_sActiveSkillID2.wMainID != 26 )
			{
				CDebugSet::ToLogFile( "ERR : GLPetField::MsgChangeActiveSkill_02, MID = %d, SID = %d",  
					m_sActiveSkillID2.wMainID,
					m_sActiveSkillID2.wSubID );
			}

			// DB에 스킬 변경 저장
			PETSKILL sPetSkill( m_sActiveSkillID2, 0 );
			CSetPetSkill* pDbAction = new CSetPetSkill ( m_pOwner->m_dwCharID, m_dwPetID, sPetSkill, false, 1 );
			GLDBMan* pDBMan = GLGaeaServer::GetInstance().GetDBMan ();
			if ( pDBMan ) pDBMan->AddJob ( pDbAction );
		}
		...
		if ( pNetMsg->sSkillID == NATIVEID_NULL() )
		{
			...
			// 보조스킬 팩터에 추가
			m_pOwner->m_sPETSKILLFACT2.RESET ();

			// 스킬제거
			GLMSG::SNETPET_REMOVE_SKILLFACT NetMsgRemoveSkill;
			NetMsgRemoveSkill.sNATIVEID == m_sActiveSkillID2;
			NetMsgRemoveSkill.dSkillOrder = EMPET_SKILL_ORDER_SECOND;
			GLGaeaServer::GetInstance().SENDTOCLIENT( m_pOwner->m_dwClientID,&NetMsgRemoveSkill );
			...
			NetMsgBRD.dwTarID  = m_pOwner->m_dwGaeaID;
			NetMsgBRD.sSkillID = pNetMsg->sSkillID;
			NetMsgBRD.dwSkillOrder = EMPET_SKILL_ORDER_SECOND;
			m_pOwner->SendMsgViewAround ( ( NET_MSG_GENERIC* ) &NetMsgBRD );
			...
			m_sActiveSkillID2 = pNetMsg->sSkillID;
		...
		PGLSKILL pSkill = GLSkillMan::GetInstance().GetData ( m_sActiveSkillID2 );
		if ( !pSkill )	return E_FAIL;

		PETSKILL_MAP_ITER iter = m_ExpSkills.find ( m_sActiveSkillID2.dwID );
		if ( iter == m_ExpSkills.end() ) return E_FAIL;
		PETSKILL sPetSkill = (*iter).second;
		SKILL::CDATA_LVL &sSKILL_DATA = pSkill->m_sAPPLY.sDATA_LVL[sPetSkill.wLevel];
		...
		// 보조스킬 팩터에 추가
		m_pOwner->m_sPETSKILLFACT2.emTYPE = pSkill->m_sAPPLY.emBASIC_TYPE;
		m_pOwner->m_sPETSKILLFACT2.fMVAR = sSKILL_DATA.fBASIC_VAR;
		m_pOwner->m_sPETSKILLFACT2.fAGE = sSKILL_DATA.fLIFE;
		m_pOwner->m_sPETSKILLFACT2.sNATIVEID = m_sActiveSkillID2;

		// 클라이언트에 팩터 추가 알림
		GLMSG::SNETPET_ADD_SKILLFACT NetMsgAddSkill;
		NetMsgAddSkill.sNATIVEID = m_sActiveSkillID2;
		NetMsgAddSkill.emTYPE	 = pSkill->m_sAPPLY.emBASIC_TYPE;
		NetMsgAddSkill.fMVAR	 = sSKILL_DATA.fBASIC_VAR;
		NetMsgAddSkill.fAge		 = sSKILL_DATA.fLIFE;
		NetMsgAddSkill.dwSkillOrder = EMPET_SKILL_ORDER_SECOND;
		GLGaeaServer::GetInstance().SENDTOCLIENT(m_pOwner->m_dwClientID,&NetMsgAddSkill);
		...
		// DB에 스킬 변경 저장
		CSetPetSkill* pDbAction = new CSetPetSkill ( m_pOwner->m_dwCharID, m_dwPetID, sPetSkill, true, 1 );
		GLDBMan* pDBMan = GLGaeaServer::GetInstance().GetDBMan ();
		if ( pDBMan ) pDBMan->AddJob ( pDbAction );
...
bool GLPetField::CheckDoubleSkill(GLMSG::SNETPET_REQ_SKILLCHANGE* pNetMsg)
{
	if(pNetMsg->dSkillOrder == EMPET_SKILL_ORDER_FIRST)
	{
		if ((pNetMsg->sSkillID != NATIVEID_NULL()) && (m_sActiveSkillID != NATIVEID_NULL()))
		{
			if (pNetMsg->sSkillID == m_sActiveSkillID2)
			{
				return true;
			}
			return false;
	...
	else if (pNetMsg->dSkillOrder == EMPET_SKILL_ORDER_SECOND)
	{
		if ((pNetMsg->sSkillID != NATIVEID_NULL()) && (m_sActiveSkillID2 != NATIVEID_NULL()))
		{
			if (pNetMsg->sSkillID == m_sActiveSkillID)
			{
				return true;
			}
			return false;
...
HRESULT GLPetField::MsgDualSkillChange( NET_MSG_GENERIC* nmg )
{
	GLMSG::SNETPET_DUAL_SKILL_UPDATE_FB* pNetMsg = (GLMSG::SNETPET_DUAL_SKILL_UPDATE_FB* ) nmg;

	return S_OK;
}

HRESULT GLPetField::MsgLearnDualSkill( NET_MSG_GENERIC* nmg )
{
	GLMSG::SNETPET_REQ_DUAL_SKILL* pNetMsg = ( GLMSG::SNETPET_REQ_DUAL_SKILL* ) nmg;

	GLMSG::SNETPET_REQ_DUAL_SKILL_FB NetMsgFB;
	SINVENITEM* pInvenItem(NULL);
	...
	if ( pNetMsg->bStorage )
	{
		if ( !m_pOwner->IsKEEP_STORAGE (pNetMsg->dwChannel) )
		{
			NetMsgFB.emFB = EMPET_REQ_DUAL_SKILL_FB_INVALID_STORAGE;
			GLGaeaServer::GetInstance().SENDTOCLIENT(m_pOwner->m_dwClientID,&NetMsgFB);
			return E_FAIL;
		}
		...
		if ( !pInvenItem )
		{
			NetMsgFB.emFB = EMPET_REQ_DUAL_SKILL_FB_NOITEM;
			GLGaeaServer::GetInstance().SENDTOCLIENT(m_pOwner->m_dwClientID,&NetMsgFB);
			return E_FAIL;
		}
	}
	...
	// 팻용 스킬인지 유무????
	if ( !pItem || pItem->sBasicOp.emItemType!=ITEM_DUAL_PETCARD )
	{
		NetMsgFB.emFB = EMPET_REQ_DUAL_SKILL_FB_BADITEM;
		GLGaeaServer::GetInstance().SENDTOCLIENT(m_pOwner->m_dwClientID,&NetMsgFB);
		return E_FAIL;
	}
	...
	if (ISDUAL_SKILL())
	{
		NetMsgFB.emFB = EMPET_REQ_DUAL_SKILL_FB_ALREADY;
		GLGaeaServer::GetInstance().SENDTOCLIENT(m_pOwner->m_dwClientID,&NetMsgFB);
		return E_FAIL;
	}
	...
	NetMsgFB.emFB = EMPET_REQ_DUAL_SKILL_FB_OK;
	NetMsgFB.wPosX = pNetMsg->wPosX;
	NetMsgFB.wPosY = pNetMsg->wPosY;
	GLGaeaServer::GetInstance().SENDTOCLIENT(m_pOwner->m_dwClientID, &NetMsgFB);

	GLMSG::SNETPET_REQ_DUAL_SKILL_BRD NetMsgBRD;
	NetMsgBRD.dwGUID = m_dwGUID;
	NetMsgBRD.bDualSkill = TRUE;
	m_pOwner->SendMsgViewAround((NET_MSG_GENERIC*) &NetMsgBRD);
	...
	// 펫 액션 로그
	GLITEMLMT::GetInstance().ReqPetAction( m_dwPetID, pInvenItem->sItemCustom.sNativeID, EMPET_ACTION_DUAL_SKILL, 0 );

	CSetPetDualSkill* pDbAction = new CSetPetDualSkill(m_pOwner->m_dwCharID, m_dwPetID);
	GLDBMan* pDBMan = GLGaeaServer::GetInstance().GetDBMan();
	if (pDBMan) pDBMan->AddJob(pDbAction);
...
void GLPetField::MsgProcess ( NET_MSG_GENERIC* nmg )
{
	...
	case NET_MSG_PET_PETSKINPACKOPEN:
		...
		break;
	case NET_MSG_PET_DUAL_SKILL:
		MsgLearnDualSkill(nmg);
		break;
	default:
