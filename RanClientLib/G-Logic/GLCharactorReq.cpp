...
///////////////////////////////////////////////////////////////////////
//  AutoPilot
HRESULT GLCharacter::ReqUseAutoSkill(int nValue)
{
	switch (nValue)
	{
	case TRUE:
...
HRESULT GLCharacter::ReqInvenTo ( WORD wPosX, WORD wPosY )
{
	...
	if( ValidRebuildOpen() || ValidGarbageOpen() || ValidItemMixOpen() || ValidPointShopOpen() )	// ITEMREBUILD_MARK
	{
...
HRESULT GLCharacter::ReqInvenDrug ( WORD wPosX, WORD wPosY )
{
	...
	if ( pItem->sBasicOp.emItemType == ITEM_CURE )
	{
		...
		if ( pItem->sDrugOp.emDrug!=ITEM_DRUG_NUNE )
		{
			...
			//dmk14 | 10-20-16 | potsdelay
			if( m_bPotsBlock )				return E_FAIL;
			else							m_bPotsUse = true;
	...
	if ( !pHold )
	{
		...
		else if ( pItem->sBasicOp.emItemType==ITEM_PET_CARD )
		{
			...
			if ( m_fPetDelay >= 5.0f )	//add pet delay by DarkEagle
			{
				...
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMPET_USECARD_FB_DELAY") );
			}
...
HRESULT GLCharacter::ReqReCall ( WORD wPosX, WORD wPosY )
{
	...
	if ( pItem->sDrugOp.emDrug==ITEM_DRUG_CALL_SCHOOL && pItem->sDrugOp.emDrug==ITEM_DRUG_CALL_REGEN && pItem->sDrugOp.emDrug==ITEM_DRUG_CALL_LASTCALL )// Added Anti SP, BP During War
	{
		PLANDMANCLIENT pLandManClient = GLGaeaClient::GetInstance().GetActiveMap ();
			...
			{
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("EMREQ_RECALL_FB_NOUSE_CARD") );// Added Anti SP, BP During War
				return S_OK;
			}
	}
...
//////////////////////////////////////////////
HRESULT GLCharacter::ReqTopUp(LONGLONG lnSaldo)
{
	if (lnSaldo == 0)							return E_FAIL;

	GLMSG::SNETPC_REQ_TOPUP NetMsg;
	NetMsg.lnSaldo = lnSaldo;
	NETSENDTOFIELD(&NetMsg);
...
HRESULT GLCharacter::ReqStorageTo ( DWORD dwChannel, WORD wPosX, WORD wPosY )
{
	...
	if ( VALID_HOLD_ITEM() ) 
	{
		...
		}// comment to disable locker control :P
	}
	...
	if ( pItem->sBasicOp.emItemType == ITEM_CURE )
	{
		...
		if ( pItem->sDrugOp.emDrug!=ITEM_DRUG_NUNE )
		{
			...
			case ITEM_DRUG_EMERGENCY_POTION: //added by CNDev
				if ( !ISANTISKILL() && !ISANTISTUN() )				return S_FALSE;
				break;
...
HRESULT GLCharacter::ReqRebuildResult( SITEMSEALDATA pData1, SITEMSEALDATA pData2, SITEMSEALDATA pData3 ) //sealed card 
{
...
HRESULT GLCharacter::ReqPointShop( DWORD dwNpcID )
{
	SINVENITEM	sInvenItem[POINTSHOP_ITEMNUM];
	for ( int i =0; i < POINTSHOP_ITEMNUM; ++i )
	{
		sInvenItem[i].sItemCustom = GET_POINT_SHOP( i );
		sInvenItem[i].wPosX = m_sPointShopPos[i].wPosX;
		sInvenItem[i].wPosY = m_sPointShopPos[i].wPosY;
	}

	GLPointShopMan::GetInstance().SortInvenItem( sInvenItem );	
	...
	if ( !pPointShop ) 
	{
		...
		CInnerInterface::GetInstance().SetBuyItem( ID2GAMEINTEXT("EMPOINT_SHOP_FB_NOMIX") );
		return E_FAIL;
	}

	if ( m_lnSaldo < sPointShop.dwPrice ) 
	{
		//	금액 없음
		CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMPOINT_SHOP_FB_NOMONEY") );
		CInnerInterface::GetInstance().SetBuyItem( ID2GAMEINTEXT("EMPOINT_SHOP_FB_NOMONEY") );
		return E_FAIL;
	}

	GLMSG::SNET_INVEN_POINT_SHOP sNetMsg;

	sNetMsg.dwKey = pPointShop->dwKey;
	for ( int i = 0; i < POINTSHOP_ITEMNUM; ++i )
	{
		sNetMsg.sInvenPos[i].wPosX = sInvenItem[i].wPosX;
		sNetMsg.sInvenPos[i].wPosY = sInvenItem[i].wPosY;
...
HRESULT GLCharacter::ReqEquipItem ( WORD wPosX, WORD wPosY )
{
	...
	if( ValidRebuildOpen() || ValidGarbageOpen() || ValidItemMixOpen() || ValidPointShopOpen() )
	{
