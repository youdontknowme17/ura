...
void GLPetClient::ReqChangeActiveSkill2 ( SNATIVEID sSkillID )
{
	// 스킬 취소를 위한
	if ( sSkillID == NATIVEID_NULL() )
	{
...
void GLPetClient::ReqLearnSkill2 ( WORD wPosX, WORD wPosY )
{
	if ( !IsVALID () )							return;
	if ( GLTradeClient::GetInstance().Valid() )	return;
	//if( m_pOwner->ValidRebuildOpen() )			return;	// ITEMREBUILD_MARK
...
void GLPetClient::ReqLearnSkill2 ( DWORD dwChannel, WORD wPosX, WORD wPosY )
{
	if ( !IsVALID () )								return;
	if ( !m_pOwner->IsVALID_STORAGE(dwChannel) )	return;
	if ( GLTradeClient::GetInstance().Valid() )		return;
...
void GLPetClient::ReqDualSkillPet( WORD wPosX, WORD wPosY )
{
	if( !IsVALID()) return;
	if ( GLTradeClient::GetInstance().Valid() )	return;
	...
	if ( !pItem || pItem->sBasicOp.emItemType!=ITEM_DUAL_PETCARD )	return;

	if (ISDUAL_SKILL())
	{
		CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::NEGATIVE, "Already dual skill pet" );
		return;
	}

	//	Note : 스킬 습득을 요청합니다.
	//
	GLMSG::SNETPET_REQ_DUAL_SKILL NetMsg;
	NetMsg.wPosX = wPosX;
	NetMsg.wPosY = wPosY;
	NETSENDTOFIELD ( &NetMsg );
