...
#include "../../EngineLib/DxCommon/Collision.h"
...
namespace ITEM
{
	...
	VOID SRANDOM_OPT::SaveCsvHead_old ( std::fstream &SFile )
	{
		SFile << "szNAME" << ",";
	}

	VOID SRANDOM_OPT::SaveCsv_old ( std::fstream &SFile )
	{
		if( strlen( szNAME ) < 1 )
			SFile << " " << ",";
		else
	...
	VOID SDRUG::SaveCsvHead_old ( std::fstream &SFile )
	{
		SFile << "tTIME_LMT" << ",";
		SFile << "bInstance" << ",";
		SFile << "emDrug" << ",";
	...
	VOID SDRUG::SaveCsv_old ( std::fstream &SFile )
	{
		SFile << tTIME_LMT << ",";
		SFile << bInstance << ",";
		SFile << emDrug << ",";
	...
	VOID SSKILLBOOK::SaveCsvHead_old ( std::fstream &SFile )
	{
		SFile << "sSkill_ID wMainID" << ",";
		SFile << "sSkill_ID wSubID" << ",";
	}

	VOID SSKILLBOOK::SaveCsv_old ( std::fstream &SFile )
	{
		SFile << sSkill_ID.wMainID << ",";
		SFile << sSkill_ID.wSubID << ",";
	}
	...
	VOID SGRINDING::SaveCsvHead_old ( std::fstream &SFile )
	{
		SFile << "emCLASS" << ",";
		SFile << "emGRINDER_TYPE" << ",";
		SFile << "emTYPE" << ",";
	...
	VOID SGRINDING::SaveCsv_old ( std::fstream &SFile )
	{
		SFile << emCLASS << ",";
		SFile << emGRINDER_TYPE << ",";
		SFile << emTYPE << ",";
	...
	VOID SGENERATE::SaveCsvHead_old ( std::fstream &SFile )
	{
		SFile << "dwSpecID" << ",";
		SFile << "dwLimitTime" << ",";
		SFile << "dwLimitTimeGen" << ",";
	...
	VOID SGENERATE::SaveCsv_old ( std::fstream &SFile )
	{
		SFile << dwSpecID << ",";
		SFile << dwLimitTime << ",";
		SFile << dwLimitTimeGen << ",";
	...
	void SBASIC::Assign ( SBASIC_100 &sOld )
	{
		...
		sReqStats.dwPow		= 	sOld.wPow;
		sReqStats.dwStr		= 	sOld.wStr;
		sReqStats.dwSpi		= 	sOld.wSpi;
		sReqStats.dwDex		= 	sOld.wDex;
		sReqStats.dwInt		= 	sOld.wInt;
		sReqStats.dwSta		= 	sOld.wSta;
	...
	bool SBASIC::LOAD_103 ( basestream &SFile )
	{
		...
		SFile >> wInt;
		SFile >> wSta;
		sReqStats.dwPow	= wPow;
		sReqStats.dwStr	= wStr;
		sReqStats.dwSpi	= wSpi;
		sReqStats.dwDex	= wDex;
		sReqStats.dwInt	= wInt;
		sReqStats.dwSta	= wSta;
	...
	//add class
	bool SBASIC::LOAD_114 ( basestream &SFile )
	{
		DWORD dwData;
	...
	//
	bool SBASIC::LOAD_115 ( basestream &SFile )
	{
		DWORD dwData;
	...
	bool SBASIC::LOAD_116 ( basestream &SFile )
	{
		DWORD dwData;
		...
		SFile >> dwData;
		m_emSumType = (SUMMON_TYPE) dwData;
	...
	bool SBASIC::LOAD_117 ( basestream &SFile )
	{
		DWORD dwData;
...
//		기획팀 요청으로 제거함
		...
		SFile << "sSumID wMainID" << ",";
		SFile << "sSumID wSubID" << ",";
		SFile << "m_emSumType" << ",";
		SFile << "dwSlotNum" << ",";

		for( int i=0; i<CPSIND_SIZE; ++i ) //add abl
			SFile << "strABLFile " << i << ",";
...
//		기획팀 요청으로 제거함
		...
		SFile << wReqPA << ",";
		SFile << wReqSA << ",";
		SFile << sReqStats.dwPow << ",";
		SFile << sReqStats.dwStr << ",";
		SFile << sReqStats.dwSpi << ",";
		SFile << sReqStats.dwDex << ",";
		SFile << sReqStats.dwInt << ",";
		SFile << sReqStats.dwSta << ",";
		...
		for( int i=0; i<GLCI_NUM_2015; ++i )
			STRUTIL::OutputStrCsv( SFile, strWearingFile2[i] );
		...
		for( int i=0; i<CPSIND_SIZE; ++i ) //add abl
			STRUTIL::OutputStrCsv( SFile, strABLFile[i] );
 
		STRUTIL::OutputStrCsv ( SFile, strVPSFile );//add vps
	}
...
//		기획팀 요청으로 제거함
		...
		sReqStats.dwPow = (DWORD)atoi( StrArray[ iCsvCur++ ] );
		sReqStats.dwStr = (DWORD)atoi( StrArray[ iCsvCur++ ] );
		sReqStats.dwSpi = (DWORD)atoi( StrArray[ iCsvCur++ ] );
		sReqStats.dwDex = (DWORD)atoi( StrArray[ iCsvCur++ ] );
		sReqStats.dwInt = (DWORD)atoi( StrArray[ iCsvCur++ ] );
		sReqStats.dwSta = (DWORD)atoi( StrArray[ iCsvCur++ ] );
		...
		for( int i=0; i<GLCI_NUM_2015; ++i )
			STRUTIL::InputStrCsv( StrArray[ iCsvCur++ ], strWearingFile2[i] );
		...
		sSumID.wMainID = (WORD)atoi( StrArray[ iCsvCur++ ] );
		sSumID.wSubID = (WORD)atoi( StrArray[ iCsvCur++ ] );
		m_emSumType = (SUMMON_TYPE)atoi( StrArray[ iCsvCur++ ] );
		dwSlotNum = (DWORD)atoi( StrArray[ iCsvCur++ ] );

		for( int i=0; i<CPSIND_SIZE; ++i )//add abl
			STRUTIL::InputStrCsv( StrArray[ iCsvCur++ ], strABLFile[i] );

		STRUTIL::InputStrCsv( StrArray[ iCsvCur++ ], strVPSFile ); //add vps
	}
	...
	SBASIC& SBASIC::operator = ( const SBASIC& rvalue )
	{
		...
		for ( int i=0; i<GLCI_NUM_2015; ++i )	
			strWearingFile2[i] = rvalue.strWearingFile2[i];	//	?? ??.
		...
		sSumID = rvalue.sSumID;
		m_emSumType = rvalue.m_emSumType;
		dwSlotNum = rvalue.dwSlotNum;

		for ( int i=0; i<CPSIND_SIZE; ++i )//add abl
			strABLFile[i] = rvalue.strABLFile[i];

		strVPSFile = rvalue.strVPSFile; //add vps
	...
	void SSUIT::Assign ( SSUIT_100 &Suit100 )
	{
		...
		gdDamage.dwHigh = Suit100.wHigh;
		gdDamage.dwLow = Suit100.wLow;
	...
	void SSUIT::Assign ( SSUIT_101 &Suit101 )
	{
		...
		gdDamage.dwHigh = Suit101.wHigh;
		gdDamage.dwLow = Suit101.wLow;
	...
	void SSUIT::Assign ( SSUIT_102 &Suit102 )
	{
		...
		gdDamage.dwHigh = Suit102.wHigh;
		gdDamage.dwLow = Suit102.wLow;
	...
	void SSUIT::Assign ( SSUIT_103 &Suit103 )
	{
		...
		gdDamage.dwHigh = Suit103.wHigh;
		gdDamage.dwLow = Suit103.wLow;
	...
	void SSUIT::Assign ( SSUIT_104 &Suit104 )
	{
		...
		gdDamage.dwHigh = Suit104.wHigh;
		gdDamage.dwLow = Suit104.wLow;
	...
	void SSUIT::Assign ( SSUIT_105 &Suit105 )
	{
		...
		gdDamage.dwHigh = Suit105.wHigh;
		gdDamage.dwLow = Suit105.wLow;
	...
	void SSUIT::Assign ( SSUIT_106 &sSuitOld ) // By 경대
	{
		...
		gdDamage.dwHigh = sSuitOld.wHigh;
		gdDamage.dwLow = sSuitOld.wLow;
	...
	//add 8 addon
	void SSUIT::Assign ( SSUIT_107 &sSuitOld ) // By 경대
	{
		emSuit = sSuitOld.emSuit;
		//SETBOTHHAND ( sSuitOld.bBothHand );
		//SETBIG ( sSuitOld.bBig );
	...
	void SSUIT::Assign ( SSUIT_108 &sSuitOld ) // By 경대
	{
		emSuit = sSuitOld.emSuit;
		//SETBOTHHAND ( sSuitOld.bBothHand );
		//SETBIG ( sSuitOld.bBig );
	...
	VOID SSUIT::SaveCsv ( std::fstream &SFile )
	{
		...
		SFile << gdDamage.dwLow << ",";
		SFile << gdDamage.dwHigh << ",";
		SFile << nDefense << ",";
	...
	VOID SSUIT::SaveCsvHead_old ( std::fstream &SFile )
	{
		SFile << "emSuit" << ",";
		SFile << "dwHAND" << ",";
		SFile << "emHand" << ",";
	...
	VOID SSUIT::SaveCsv_old ( std::fstream &SFile )
	{
		SFile << emSuit << ",";
		SFile << dwHAND << ",";
		SFile << emHand << ",";
	...
	VOID SSUIT::LoadCsv ( CStringArray &StrArray )
	{
		...
		gdDamage.dwLow = (DWORD)atoi( StrArray[ iCsvCur++ ] );
		gdDamage.dwHigh = (DWORD)atoi( StrArray[ iCsvCur++ ] );
	...
	VOID SBOX::SaveCsvHead_old ( std::fstream &SFile )
	{
		for( int i=0; i<ITEM_SIZE; ++i )
		{
			SFile << "sITEMS " << i << " nidITEM wMainID" << ",";
	...
	VOID SBOX::SaveCsv_old ( std::fstream &SFile )
	{
		for( int i=0; i<ITEM_SIZE; ++i )
		{
			SFile << sITEMS[i].nidITEM.wMainID << ",";
	...
	VOID SRANDOMBOX::SaveCsvHead_old ( std::fstream &SFile )
	{
		const int iMaxSize = 30;
	...
	VOID SRANDOMBOX::SaveCsv_old ( std::fstream &SFile )
	{
		const int iMaxSize = 30;
		int iSize = (int)vecBOX.size();
...
//				stRandomItem.nidITEM.wSubID != 0 )
	...
	VOID SQUESTIONITEM::SaveCsvHead_old ( std::fstream &SFile )
	{
		SFile << "emType" << ",";
		SFile << "fTime" << ",";
		SFile << "fExp" << ",";
	...
	VOID SQUESTIONITEM::SaveCsv_old ( std::fstream &SFile )
	{
		SFile << emType << ",";
		SFile << fTime << ",";
		SFile << fExp << ",";
		...
		case 0x0101:
			{
				...
				SFile >> dwPetID;
				SFile >> nType;
				emType = static_cast<PET_ACCESSORYTYPE>(nType);
			}
			break;
	...
	VOID SPET::SaveCsvHead_old ( std::fstream &SFile )	// PetData
	{
		SFile << "dwPetID" << ",";
		SFile << "emType" << ",";
		SFile << "emPetType" << ",";
	...
	VOID SPET::SaveCsv_old ( std::fstream &SFile )	// PetData
	{
		SFile << dwPetID << ",";
		SFile << emType << ",";
		SFile << emPetType << ",";
	...
	VOID SPET::LoadCsv ( CStringArray &StrArray )	// PetData
	{
		...
		emType = (PET_ACCESSORYTYPE)atoi( StrArray[ iCsvCur++ ] );
	...
	VOID SVEHICLE::SaveCsvHead_old ( std::fstream &SFile )	// VehicleData
	{
		SFile << "emVehicleType" << ",";
	}

	VOID SVEHICLE::SaveCsv_old ( std::fstream &SFile )	// VehicleData
	{
		SFile << emVehicleType << ",";
	}
...
VOID SITEM::SaveCsvHead_old ( std::fstream &SFile )
{
	ITEM::SBASIC::SaveCsvHead_old( SFile );
	ITEM::SSUIT::SaveCsvHead_old( SFile );
	ITEM::SDRUG::SaveCsvHead_old( SFile );
	ITEM::SSKILLBOOK::SaveCsvHead_old( SFile );
	ITEM::SGRINDING::SaveCsvHead_old( SFile );
	ITEM::SGENERATE::SaveCsvHead_old( SFile );
	ITEM::SBOX::SaveCsvHead_old( SFile );
	ITEM::SRANDOMBOX::SaveCsvHead_old( SFile );
	ITEM::SQUESTIONITEM::SaveCsvHead_old( SFile );
	ITEM::SRANDOM_OPT::SaveCsvHead_old( SFile );
	ITEM::SPET::SaveCsvHead_old( SFile );	// PetData
	ITEM::SVEHICLE::SaveCsvHead_old( SFile );
	//ITEM::SPETSKINPACKITEM::SaveCsvHead( SFile );

	SFile << std::endl;
}

VOID SITEM::SaveCsv_old ( std::fstream &SFile )
{
	sBasicOp.SaveCsv_old( SFile );
	sSuitOp.SaveCsv_old( SFile );
	sDrugOp.SaveCsv_old( SFile );
	sSkillBookOp.SaveCsv_old( SFile );
	sGrindingOp.SaveCsv_old( SFile );
	sGenerateOp.SaveCsv_old( SFile );
	sBox.SaveCsv_old( SFile );
	sRandomBox.SaveCsv_old( SFile );
	sQuestionItem.SaveCsv_old( SFile );
	sRandomOpt.SaveCsv_old( SFile );
	sPet.SaveCsv_old( SFile );				// PetData
	sVehicle.SaveCsv_old( SFile );
//	sPetSkinPack.SaveCsv( SFile );
...
void SITEMCUSTOM::Assign ( const SITEMCLIENT &sClient )
{
	...
	wColor = sClient.wColor;//add itemcolor
...
DWORD SITEMCUSTOM::GETGRADE_DAMAGE () const
{
	...
	{
		DWORD dwDAMAGE = getdamage().dwHigh; // 물리 데미지
		DWORD dwMaDAMAGE = getmadamage();   // 기력 데미지
...
WORD SITEMCUSTOM::GETGRADE_RESIST_FIRE () const
{
	...
	if ( cGRADE > GLCONST_CHAR::wGRADE_ELE_MAX )	cGRADE = GLCONST_CHAR::wGRADE_ELE_MAX;
...
inline float SITEMCUSTOM::GETOptVALUE ( EMRANDOM_OPT emOPT, short nVALUE ) const
{
	...
	case EMR_OPT_DAMAGE_RATE:
	case EMR_OPT_DEFENSE_RATE:
	case EMR_OPT_MOVE_SPEED:
		return nVALUE*0.01f;

	case EMR_OPT_HIT_RATE:
	case EMR_OPT_AVOID_RATE:
		return nVALUE*0.01f;
	...
	case EMR_OPT_HP_INC_RATE:
	case EMR_OPT_MP_INC_RATE:
	case EMR_OPT_SP_INC_RATE:
	case EMR_OPT_HMS_INC_RATE:
	...
	case EMR_OPT_DAMAGE: //add directval
	...
	case EMR_OPT_STM:
	case EMR_OPT_MELEE:
	case EMR_OPT_MESSILE:
	case EMR_OPT_ENERGY:
	case EMR_OPT_HP_POTION:
	case EMR_OPT_MP_POTION:
	case EMR_OPT_SP_POTION:
	case EMR_OPT_CP_GAIN:
	case EMR_OPT_CP:
	case EMR_OPT_CRIT_DMG:
	case EMR_OPT_BLOW_DMG:
	case EMR_OPT_CRIT_RATE:
...
BOOL SITEMCUSTOM::IsPerRandOpt( INT nRandOPT ) const
{
	...
	case EMR_OPT_DAMAGE:
	case EMR_OPT_DEFENSE:
	case EMR_OPT_HIT: //this one is rate
	case EMR_OPT_AVOID: //this one is rate
	case EMR_OPT_POW:
	case EMR_OPT_VIT:
	case EMR_OPT_INT:
	case EMR_OPT_DEX:
	...
	case EMR_OPT_CP_GAIN:
	case EMR_OPT_CP:
	case EMR_OPT_HIT_RATE: //fix official mistake this is not rate but directval
	case EMR_OPT_AVOID_RATE://fix official mistake this is not rate but directval
		return FALSE;
...
GLPADATA SITEMCUSTOM::getdamage () const
{
	...
	// 렌던 수치 합산
	float fRATE = GETOptVALUE(EMR_OPT_DAMAGE_RATE);
	float fRATE2 = GETOptVALUE(EMR_OPT_DAMAGE);//add directval damage
...
WORD SITEMCUSTOM::getmadamage () const
{
	...
	if ( fRATE!=0 )
	{
		if ( fRATE2!=0 )	wMaDAMAGE = WORD( (wMaDAMAGE + fRATE2) *(100.0f+fRATE)*0.01f);
		else	wMaDAMAGE = WORD(wMaDAMAGE*(100.0f+fRATE)*0.01f);
	}
	else	
	{
...
short SITEMCUSTOM::getdefense () const
{
	...
	float fRATE = GETOptVALUE(EMR_OPT_DEFENSE_RATE);
	float fRATE2 = GETOptVALUE(EMR_OPT_DEFENSE); //add directval defense
	if ( fRATE!=0.0f )
	{
...
short SITEMCUSTOM::GETHITRATE () const
{
	...
	float fRATE = GETOptVALUE(EMR_OPT_HIT_RATE);// change for add directval
...
float SITEMCUSTOM::GETHITRATE_PER () const
{
	...
	float fHITRATE = 0;
	float fRATE = GETOptVALUE(EMR_OPT_HIT); // change for add directval	
...
short SITEMCUSTOM::GETAVOIDRATE () const
{
	...
	float fRATE = GETOptVALUE(EMR_OPT_AVOID_RATE);// change for add directval
	if ( fRATE!=0.0f )
	{
...
float SITEMCUSTOM::GETAVOIDRATE_PER () const
{
	...
	float fRATE = GETOptVALUE(EMR_OPT_AVOID);// change for add directval
...
float SITEMCUSTOM::GETINCHP () const
{
	...
	fINC += GETOptVALUE(EMR_OPT_HP_INC_RATE);
...
float SITEMCUSTOM::GETINCMP () const
{
	...
	fINC += GETOptVALUE(EMR_OPT_MP_INC_RATE);
...
float SITEMCUSTOM::GETINCSP () const
{
	...
	fINC += GETOptVALUE(EMR_OPT_SP_INC_RATE);
...
float SITEMCUSTOM::GETINCAP () const
{
	...
	fINC += GETOptVALUE(EMR_OPT_HMS_INC_RATE);
...
WORD SITEMCUSTOM::GET_STAT_POW() const
{
	...
	WORD wPow = 0;
	float fVALUE = GETOptVALUE(EMR_OPT_POW);
...
WORD SITEMCUSTOM::GET_STAT_VIT() const
{
	...
	WORD wVit = 0;
	float fVALUE = GETOptVALUE(EMR_OPT_VIT);
...
WORD SITEMCUSTOM::GET_STAT_INT() const
{
	...
	WORD wInt = 0;
	float fVALUE = GETOptVALUE(EMR_OPT_INT);
...
WORD SITEMCUSTOM::GET_STAT_DEX() const
{
	...
	WORD wDex = 0;
	float fVALUE = GETOptVALUE(EMR_OPT_DEX);
...
WORD SITEMCUSTOM::GET_STAT_STM() const
{
	...
	WORD wStm = 0;
	float fVALUE = GETOptVALUE(EMR_OPT_STM);
	if ( fVALUE!=0.0f )	wStm += WORD(fVALUE);
	return wStm;
}

//add directval melee
int SITEMCUSTOM::GET_MELEE() const
{
	SITEM *pITEM = GLItemMan::GetInstance().GetItem(sNativeID);
	if ( !pITEM )	return 0;
...
//add directval messile
int SITEMCUSTOM::GET_MESSILE() const
{
	SITEM *pITEM = GLItemMan::GetInstance().GetItem(sNativeID);
	if ( !pITEM )	return 0;
	
	int nMessile = 0;
	float fVALUE = GETOptVALUE(EMR_OPT_MESSILE);
	if ( fVALUE!=0.0f )	nMessile += int(fVALUE);
	return nMessile;
}

//add directval energy
int SITEMCUSTOM::GET_ENERGY() const
{
	SITEM *pITEM = GLItemMan::GetInstance().GetItem(sNativeID);
	if ( !pITEM )	return 0;
...
int SITEMCUSTOM::GET_HP_POTION() const
{
	...
	int nHpPotion = 0;
	float fVALUE = GETOptVALUE(EMR_OPT_HP_POTION);
...
int SITEMCUSTOM::GET_MP_POTION() const
{
	...
	int nMpPotion = 0;
	float fVALUE = GETOptVALUE(EMR_OPT_MP_POTION);
...
int SITEMCUSTOM::GET_SP_POTION() const
{
	...
	int nSpPotion = 0.0f;
	float fVALUE = GETOptVALUE(EMR_OPT_SP_POTION);
	if ( fVALUE!=0.0f )	nSpPotion += int(fVALUE);
	return nSpPotion;
}

//add directval CP Gain
float SITEMCUSTOM::GET_CP_GAIN () const
{
	SITEM *pITEM = GLItemMan::GetInstance().GetItem(sNativeID);
	if ( !pITEM )	return 0;
	
	float fCpGain = 0.0f;
	float fVALUE = GETOptVALUE(EMR_OPT_CP_GAIN);
	if ( fVALUE!=0.0f )	fCpGain += fVALUE;
	return fCpGain;
}

//add directval CP
int SITEMCUSTOM::GET_CP () const
{
	SITEM *pITEM = GLItemMan::GetInstance().GetItem(sNativeID);
	if ( !pITEM )	return 0;
	
	int nCPAdd = 0;
	float fVALUE = GETOptVALUE(EMR_OPT_CP);
	if ( fVALUE!=0.0f )	nCPAdd += int(fVALUE);
	return nCPAdd;
}

//add directval Crit Damage
float SITEMCUSTOM::GET_CRIT_DMG () const
{
	SITEM *pITEM = GLItemMan::GetInstance().GetItem(sNativeID);
	if ( !pITEM )	return 0;
	
	float fVal = 0.0f;
	float fVALUE = GETOptVALUE(EMR_OPT_CRIT_DMG);
	if ( fVALUE!=0.0f )	fVal += fVALUE;
	return fVal;
}

//add directval Blow Damage
float SITEMCUSTOM::GET_BLOW_DMG () const
{
	SITEM *pITEM = GLItemMan::GetInstance().GetItem(sNativeID);
	if ( !pITEM )	return 0;
	
	float fVal = 0.0f;
	float fVALUE = GETOptVALUE(EMR_OPT_BLOW_DMG);
	if ( fVALUE!=0.0f )	fVal += fVALUE;
	return fVal;
}

//add directval crit rate
float SITEMCUSTOM::GET_CRIT_RATE () const
{
	SITEM *pITEM = GLItemMan::GetInstance().GetItem(sNativeID);
	if ( !pITEM )	return 0;
	
	float fVal = 0.0f;
	float fVALUE = GETOptVALUE(EMR_OPT_CRIT_RATE);
	if ( fVALUE!=0.0f )	fVal += fVALUE;
	return fVal;
}

//add directval crit rate
float SITEMCUSTOM::GET_BLOW_RATE () const
{
	SITEM *pITEM = GLItemMan::GetInstance().GetItem(sNativeID);
	if ( !pITEM )	return 0;
	
	float fVal = 0.0f;
	float fVALUE = GETOptVALUE(EMR_OPT_BLOW_RATE);
	if ( fVALUE!=0.0f )	fVal += fVALUE;
	return fVal;
}
...
namespace COMMENT
{
	...
	std::string ITEM_PUTON_SLOT[SLOT_NSIZE_S_2] =
	{
		"SLOT_HEADGEAR",
		"SLOT_UPPER	",
		"SLOT_LOWER",
	...
	std::string ITEM_VEHICLE_SLOT[VEHICLE_ACCETYPE_SIZE] =
	{
		"VEHICLE_ACCETYPE_SKIN",
		"VEHICLE_ACCETYPE_PARTS_A",
		"VEHICLE_ACCETYPE_PARTS_B",
		"VEHICLE_ACCETYPE_PARTS_C",
		"VEHICLE_ACCETYPE_PARTS_D",
		"VEHICLE_ACCETYPE_PARTS_E",
		"VEHICLE_ACCETYPE_PARTS_F",
	};

	std::string ITEM_PET_SLOT[PET_ACCETYPE_SIZE] =
	{
		"PET_ACCETYPEA",
		"PET_ACCETYPEB",
	};
