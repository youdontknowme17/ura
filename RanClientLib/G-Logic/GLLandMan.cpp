...
GLLandMan::GLLandMan (void) :
	...
	m_dwClubMapID(0),
	m_dwSchoolWarMapID(0),
	m_dwRoyalRumbleMapID(0),
	m_pGuidance(NULL),
	...
	m_bSchoolWarMap(false),
	m_dwTowerWarsMapID(0),
	m_bTowerWarsMap(false),

	m_bSchoolWarsMap(false),
	m_bSchoolWarsMapHall(false),

	m_bRoyalRumbleMap(false),
	m_bRoyalRumbleMapHall(false),
	m_bServerStop(false),
...
DWORD GLLandMan::DropCrow ( GLMobSchedule *pMobSch )
{
	...
	if ( pNewCrow->m_pCrowData->m_sAction.m_dwActFlag&EMCROWACT_KNOCK )
	{
		...
		CString strTEXT;
		strTEXT.Format ( "'%s' Monster Has been Regenerated at '%s'",
	...
	if ( m_bTowerWarsMap )
	{
		GLTowerWars* pSW = GLTowerWarsFieldMan::GetInstance().Find( m_dwTowerWarsMapID );
		if ( pSW )
		{
			SW_SCHOOL_DATA emOWNER = pSW->GetTowerSchool( pNewCrow->m_pCrowData->sNativeID );
			pNewCrow->m_emOWNER = emOWNER;
		}
	}
...
BOOL GLLandMan::RemoveChar ( PGLCHAR pChar )
{
	...
	if ( pChar->m_pLandNode )
	{
		...
		if( m_bTowerWarsMap && pChar->m_dwUserLvl < USER_SPECIAL ) UpdateDecHPmultiplier( pChar->m_wSchool );
	}
...
void GLLandMan::DoSkillActEx ( const SSKILLACTEX &sACTEX )
{
	...
	if ( sACTEX.bCrushBlow ) PushPullAct( sACTEX.sID, sACTEX.sID_TAR, sACTEX.fPUSH_PULL, sACTEX.fPUSH_PULL_B, false, false, false );  //add pushpull //modify
	else
	{
		if ( sACTEX.bPush || sACTEX.bPull || sACTEX.bDash ) PushPullAct( sACTEX.sID, sACTEX.sID_TAR, sACTEX.fPUSH_PULL, sACTEX.fPUSH_PULL_B, sACTEX.bPush, sACTEX.bPull, sACTEX.bDash );  //add pushpull //modify
		else PushPullAct( sACTEX.sID_TAR, sACTEX.sID_TAR, sACTEX.fPUSH_PULL, sACTEX.fPUSH_PULL_B, false, false, false );  //add pushpull //modify
	}
}

void GLLandMan::PushPullAct( const STARGETID dwActorID, const STARGETID dwReActorID, const float fPUSH_PULL, const float fPUSH_PULL_B, const bool bPush, const bool bPull, const bool bDash ) //add pushpull //modify
{
	if ( fPUSH_PULL!=0.0f )
	{	
		...
		D3DXVECTOR3 vACTOR_POS = pACTOR->GetPosition();
		D3DXVECTOR3 vDirZ = ( vACTOR_POS - pREACTOR->GetPosition() );
		vDir.y = 0.0f;
		...
		if ( pNavi )
		{
			...
			{
				vPosA = vREACTOR_POS;
				vPosB = pACTOR->GetPosition();
			}
			else
			{
				vPosA = vREACTOR_POS;
				vPosB = vREACTOR_POS + vDir * fPushPull;
			}
			...
			if ( bCoA )
			{
				...
				if ( !bCoB )
				{
					...
					{
						if ( bPull ) vPosB = vREACTOR_POS - vDir * fNewPushPull;
						else vPosB = vREACTOR_POS + vDir * fNewPushPull;
				...
				if ( bCoB )
				{
					...
					{
						if ( bDash )  pACTOR->ReceivePushPull ( vCoB, false, true, fPUSH_PULL_B, true );
						else 
						{
							if ( bPush || bPull ) pREACTOR->ReceivePushPull ( vCoB, true, true, fPUSH_PULL_B, false ); // cancel when push /pull
							else pREACTOR->ReceivePushPull ( vCoB, true, false, 0.0f, false );
						}
					}
				}
			}
...
void GLLandMan::ResetCrowOwner ()
{
	...
	if ( pCurCrow )
	{
		for ( ; pCurCrow; pCurCrow = pCurCrow->pNext )
		{
			...
			pCrow->SetOwner( SW_SCHOOL_MAX );
			pCrow->INIT_DATA();
		}
	}
}

void GLLandMan::UpdateAddHPmultiplier( int index )
{
	m_dwPcCount[index]++;

	DWORD dwPcTotal = m_dwPcCount[0] + m_dwPcCount[1] + m_dwPcCount[2];
	for( int i=0; i<SW_SCHOOL_MAX; ++i )
	{
		float fVal = 1.0 - ( (float) m_dwPcCount[i]/dwPcTotal );
		m_fHpMultiplier[i] = fVal;
	}
}

void GLLandMan::UpdateDecHPmultiplier( int index )
{
	if( m_dwPcCount[index] <= 0 )	return;	
