...
#include "GLItemLink/GLItemLink.h"
#include "GLContrlTowerWars.h"
#include "GLContrlAutoPilot.h"			//  AutoPilot
...
namespace GLMSG
{
	...
	struct SNET_AP_DT //add autopots detection
		{
			NET_MSG_GENERIC			nmg;
			BOOL				bDetect;
			DWORD				dwCharID;
			...
			SNET_AP_DT () 
				...
				, dwCharID(0)
			{
				nmg.dwSize = sizeof(SNET_AP_DT);
				nmg.nType = NET_MSG_AP_DT;
				GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
			}
		};
		struct SNET_AP_DT_FB
		{
			...
			SNET_AP_DT_FB () 
				: emFB(DETECTED)
			{
				nmg.dwSize = sizeof(SNET_AP_DT_FB);
				nmg.nType = NET_MSG_AP_DT_FB;
				GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
			}
		};
		struct SNET_AP_DT_BRD
		{
			NET_MSG_GENERIC			nmg;
			APDETECTED				emFB;
			...
			SNET_AP_DT_BRD () 
				: emFB(DETECTED)
			{
				nmg.dwSize = sizeof(SNET_AP_DT_BRD);
				nmg.nType = NET_MSG_AP_DT_BRD;
				GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
