...
GLLandManClient::GLLandManClient(void) 
	...
	, m_bSchoolWars(false)
	, m_bSchoolWarsHall(false)
	, m_bRoyalRumble(false)
	, m_bRoyalRumbleHall(false)
	...
	, m_fLandBonusMSpeed(0.0f)
{
	...
	m_dwCOUNT = 0;
	for(int i=0;i<3;i++)	m_fHpMultiplier[i]=0.0f;
}
...
HRESULT GLLandManClient::Create ( const char* szSetFileName, LPDIRECT3DDEVICEQ pd3dDevice, D3DXVECTOR3 &vBasicPos, SNATIVEID* pMapID, BOOL bPeaceZone )
{
	...
	if ( pMapNode )
	{
		...
		SetClubDeathMatchZone ( pMapNode->bCDMZone );
		SetSchoolWarsZone	( pMapNode->bSWZone );
		SetRoyalRumbleZone	( pMapNode->bRRZone );
		SetRedZone( pMapNode->bRedZone );
	}
...
{
	m_fAge += fElapsedTime;
	if( m_bTowerWars )	m_fLandBonusMSpeed = 0.6f;
	else m_fLandBonusMSpeed = 0.0f;
...
BOOL GLLandManClient::DropItem ( SDROP_CLIENT_ITEM *pItemDrop )
{
	...
	{
		STARGETID sTARID ( CROW_ITEM, pNewItem->dwGlobID, pNewItem->vPos );
		DxEffGroupPlayer::GetInstance().PassiveEffect ( GLCONST_CHAR::strMINE_EFFECT.c_str(), pNewItem->matWld, sTARID );
	}
...
BOOL GLLandManClient::DropMoney ( LONGLONG lnAmount, D3DXVECTOR3 vPos, DWORD dwGlobID, float fAge )
{
	...
	if (RANPARAM::bMineEffect)
	{
		...
		DxEffGroupPlayer::GetInstance().PassiveEffect ( GLCONST_CHAR::strMINE_EFFECT.c_str(), pNewMoney->matWld, sTARID );
	}
