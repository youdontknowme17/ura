...
float GLMaterial::CalculateItemGenRate( float fGenRate, PGLCHAR pChar, bool bParty )
{
	...
	if( pChar ) fEVENT_RATE = GLGaeaServer::GetInstance().GetItemGainRate(pChar->m_CHARINDEX,pChar->m_wLevel) + GLGaeaServer::GetInstance().GetItemGainRate(pChar->m_wSchool) - 1;
