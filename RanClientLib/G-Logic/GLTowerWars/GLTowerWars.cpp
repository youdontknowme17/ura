...
void GLTowerWars::SetCTFWinner(int nSchool)
{
	if( nSchool == -1 ) return;
...
bool GLTowerWars::Load ( std::string strFile )
{
	...
	/*cFILE.getflag( "SCHOOL_WARS", "AWARD_ITEM_WIN_SCHOOL", 0, 2, m_sTwPAwardItem.nAwardItem[0].wMainID );
	cFILE.getflag( "SCHOOL_WARS", "AWARD_ITEM_WIN_SCHOOL", 1, 2, m_sTwPAwardItem.nAwardItem[0].wSubID );
	cFILE.getflag( "SCHOOL_WARS", "AWARD_ITEM_LOSE_SCHOOL", 0, 2, m_sTwPAwardItem.nAwardItem[1].wMainID );
	cFILE.getflag( "SCHOOL_WARS", "AWARD_ITEM_LOSE_SCHOOL", 1, 2, m_sTwPAwardItem.nAwardItem[1].wSubID );*/
...
void GLTowerWars::DoBattleStart ( DWORD dwORDER, int nHour )
{
    ...
    for(int i = 0; i<3;i++)
	{
       ...
       NetSwMsg.bStart = FALSE;
       NetSwMsg.m_wSchool = i;
...
STWP_RANK_INFO* GLTowerWars::GetCTFRankInof( DWORD dwCharID )
{
	...
	if( pos == m_mapCtfScore.end() ) return NULL;
	return &(*pos).second;
...
void GLTowerWars::AddWinner( STWP_RANK_INFO sRANK )
{
	TWP_RANK_INFO_MAP_ITER pos = m_mapTwWinner.find( sRANK.dwCharID );
	if( pos == m_mapTwWinner.end() )
	{
		m_mapTwWinner[sRANK.dwCharID] = sRANK;
	}
}
