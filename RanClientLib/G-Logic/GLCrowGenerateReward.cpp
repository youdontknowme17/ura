		...
		(m_sAssault.dwID != GAEAID_NULL))
	{
		...
		if ( pChar )
		{
			...
			if ( m_pLandMan && m_pLandMan->m_bTowerWarsMap )
			{
				...
				if ( pSW )
				{
					BOOL bCAPTURED = pSW->AddTower( pChar->GetCharID(), m_pCrowData->m_sBasic.sNativeID, SW_SCHOOL_DATA( pChar->m_wSchool ) );
					if ( bCAPTURED )
					{
						GLTowerWarsFieldMan::GetInstance().UpdateTower( pSW->m_dwID );
					}
				}
			}
...
void GLCrow::GenerateRewardItem(DWORD dwOWNER /* GAEAID_NULL */)
{
	...
	if ( sCROWGEN.m_wGenItem_Rate!=0 && sCROWGEN.m_sGenItemID!=NATIVEID_NULL() )
	{
		...
		float fGEN_RATE_R = sCROWGEN.m_wGenItem_Rate * GLCONST_CHAR::GETITEM_SCALE() * fGEN_RATE;
   ...
   { 
	...
	if ( sCROWGEN.m_wGenMoney_Rate!=0 && sCROWGEN.m_dwGenMoney!=0 )
	{
		...
		float fGEN_RATE = sCROWGEN.m_wGenMoney_Rate * GLCONST_CHAR::GETMONEY_SCALE();
		if ( RANDOM_GEN(fGEN_RATE) )
		{
			...
			if ( bCollision )
			{
				...
				DWORD dwGemMoney = DWORD ( GLCONST_CHAR::GETMONEY_SCALE() * sCROWGEN.m_dwGenMoney * ( 1.0f + 0.3f*RANDOM_NUM ) );
	...
	//
	const SCROWACTION &sCROWACTION = m_pCrowData->m_sAction;
