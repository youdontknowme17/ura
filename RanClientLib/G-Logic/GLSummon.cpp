...
{
	int nDamage = ( m_pSummonCrowData->m_sCrowAttack[m_dwAType].sDamage.dwLow );
...
{
	int nDamage = ( m_pSummonCrowData->m_sCrowAttack[m_dwAType].sDamage.dwHigh );
...
DWORD GLSUMMON::RECEIVE_DAMAGE ( const DWORD wDamage )
{
	...
	for ( int i=0; i<SKILLFACT_SIZE; ++i )
	{
		...
		for(int _i = 0; _i < SKILL::MAX_LEVEL; _i++ )
		{
			...
			case EMIMPACTA_HITRATE:
				m_nSUM_HIT += int(sSKEFF.fADDON_VAR[_i]);
				break;

			case EMIMPACTA_AVOIDRATE:
				m_nSUM_AVOID += int(sSKEFF.fADDON_VAR[_i]);
				break;

			case EMIMPACTA_DAMAGE:
				m_nDX_DAMAGE += int(sSKEFF.fADDON_VAR[_i]);
				break;

			case EMIMPACTA_DEFENSE:
				m_nSUM_DEFENSE += int(sSKEFF.fADDON_VAR[_i]);
				break;

			case EMIMPACTA_VARHP:
				m_fIncHP += sSKEFF.fADDON_VAR[_i]*fElapsedTime;
				break;

			case EMIMPACTA_VARMP:
				m_fIncMP += sSKEFF.fADDON_VAR[_i]*fElapsedTime;
				break;

			case EMIMPACTA_VARSP:
				m_fIncSP += sSKEFF.fADDON_VAR[_i]*fElapsedTime;
				break;
			...
			case EMIMPACTA_DAMAGE_RATE:
				m_fDamageRate += sSKEFF.fADDON_VAR[_i];
				break;

			case EMIMPACTA_DEFENSE_RATE:
				m_fDefenseRate += sSKEFF.fADDON_VAR[_i];
				break;

			case EMIMPACTA_RESIST:
				m_sSUMRESIST += int(sSKEFF.fADDON_VAR[_i]);
				break;
			};
			...
			case EMSPECA_NONBLOW:
				m_dwHOLDBLOW = sSKEFF.dwSPECFLAG[_i];
				break;
			...
			case EMSPECA_ATTACK_INT:
				m_nDX_DAMAGE += sSKEFF.fSPECVAR1[_i];
				break;
			...
			case EMSPECA_DEF_INT:
				m_nSUM_DEFENSE += sSKEFF.fSPECVAR1[_i];
				break;
			...
			//case EMSPECA_GETEXP:
				//m_fEXP_RATE += sSKEFF.fSPECVAR1;
			//	break;

			case EMSPECA_IMMUNE:
				//m_sSUMRESIST_SKILL += 999.0f
				//m_nSUM_AVOID += 999.0f
				//m_nDEFENSE_SKILL += 999.0f
				break;
					...
					if ( m_fDamageElapsed > 1.0f ) //Per Second
					{
						if ( m_dwNowHP > 1 )
						{
							if( sSKEFF.fSPECVAR2[_i] > 0 )
								m_dwNowHP -= int(-sSKEFF.fSPECVAR2[_i]);
							else 
								m_dwNowHP += int(sSKEFF.fSPECVAR2[_i]);
...
BOOL GLSUMMON::CheckPosionItem( SNATIVEID sNativeID )
{
	...
	if ( pItem->sDrugOp.emDrug != ITEM_DRUG_HP && 
		...
		pItem->sDrugOp.emDrug != ITEM_DRUG_CP &&  //add cp
