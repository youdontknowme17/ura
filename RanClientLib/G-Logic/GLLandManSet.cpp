...
//
BOOL GLLandMan::LoadFile ( const char *szFile, std::string strWLD /*= ""*/ )//lev톝펃
{
	GASSERT(szFile);
	HRESULT	hr;
	...
	if ( strWLD.size() > 0 )//lev톝펃
	{
		bOk = LoadWldFile ( strWLD.c_str(), GLLevelFile::GetFileVer()<=VERSION_BEFORE_GATE );
		if ( bOk )
		{
			m_sLevelHead.m_strWldFile = strWLD.c_str();
		}
	}
	else
	{
	...
	if ( pMAP_NODE )
	{
		...
		m_bClubDeathMatch = pMAP_NODE->bCDMZone;
		m_bSchoolWars = pMAP_NODE->bSWZone;
		m_bRoyalRumble = pMAP_NODE->bRRZone;
		m_bTowerWars = pMAP_NODE->bTWZone;
		m_bRedZone = pMAP_NODE->bRedZone;//RedZone Integration SeiferXIII 2/11/14
//		m_bDomiWars = pMAP_NODE->bDomiWars;
		m_bCardActivity = pMAP_NODE->bCardActivity;// Added Anti SP, BP During War
	}
