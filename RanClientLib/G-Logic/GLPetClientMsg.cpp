...
#include "../RanClientUILib/Interface/PetWindow.h"
#include "../RanClientUILib/Interface/InventoryWindow.h" //add new pet
...
void GLPetClient::MsgSkillChange ( NET_MSG_GENERIC* nmg )
{
	...
	if(pNetMsg->dwSkillOrder == EMPET_SKILL_ORDER_FIRST)
		MsgSkillChange1(nmg);
	else if(pNetMsg->dwSkillOrder == EMPET_SKILL_ORDER_SECOND)
		MsgSkillChange2(nmg);
}

void GLPetClient::MsgSkillChange1 ( NET_MSG_GENERIC* nmg )
{
	GLMSG::SNETPET_ADD_SKILLFACT* pNetMsg = ( GLMSG::SNETPET_ADD_SKILLFACT* ) nmg;
...
void GLPetClient::MsgSkillChange2 ( NET_MSG_GENERIC* nmg )
{
	GLMSG::SNETPET_ADD_SKILLFACT* pNetMsg = ( GLMSG::SNETPET_ADD_SKILLFACT* ) nmg;

	m_pOwner->m_sPETSKILLFACT2.sNATIVEID = pNetMsg->sNATIVEID;
	m_pOwner->m_sPETSKILLFACT2.emTYPE	= pNetMsg->emTYPE;
	m_pOwner->m_sPETSKILLFACT2.fMVAR		= pNetMsg->fMVAR;
	m_pOwner->m_sPETSKILLFACT2.fAGE		= pNetMsg->fAge;

	// 이전 이펙트 제거
	FACTEFF::DeleteSkillFactEffect ( STARGETID(CROW_PC,m_pOwner->m_dwGaeaID,m_pOwner->GetPosition() ), m_pOwner->GetSkinChar (), m_sActiveSkillID2 );

	m_sActiveSkillID2 = pNetMsg->sNATIVEID;

	//	지속형 스킬의 경우 스킬 팩터 추가됨.
	if ( m_sActiveSkillID2 != SNATIVEID(false) )
	{
		// 주인이 타겟이된다.
		STARGETID sTargetID( CROW_PC, m_pOwner->m_dwGaeaID, m_pOwner->GetPosition () );
		FACTEFF::NewSkillFactEffect ( sTargetID, m_pOwner->GetSkinChar (), m_sActiveSkillID2, m_matTrans, sTargetID.vPos );
	}

	//CInnerInterface::GetInstance().PrintConsoleText("Prosesing skill %d %d", m_sActiveSkillID2.wMainID, m_sActiveSkillID2.wSubID);
	// 스킬구동
	StartSkillProc2();
}

void GLPetClient::MsgDualSkillLearn( NET_MSG_GENERIC* nmg )
{
	GLMSG::SNETPET_REQ_DUAL_SKILL_FB* pNetMsg = (GLMSG::SNETPET_REQ_DUAL_SKILL_FB*)nmg;
	switch (pNetMsg->emFB)
	{
	case EMPET_REQ_DUAL_SKILL_FB_OK:
		{
			...
			CInnerInterface::GetInstance().PrintMsgText ( 
				NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EMPET_REQ_DUAL_SKILL_FB_OK"), m_szName );
		}
		break;
	case EMPET_REQ_DUAL_SKILL_FB_FAIL:
		CInnerInterface::GetInstance().PrintMsgText ( 
			NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("EMPET_REQ_DUAL_SKILL_FB_FAIL") );
		break;
	case EMPET_REQ_DUAL_SKILL_FB_NOITEM:
		CInnerInterface::GetInstance().PrintMsgText ( 
			NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("EMPET_REQ_DUAL_SKILL_FB_NOITEM") );
		break;
	case EMPET_REQ_DUAL_SKILL_FB_BADITEM:
		CInnerInterface::GetInstance().PrintMsgText ( 
			NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("EMPET_REQ_DUAL_SKILL_FB_BADITEM") );
		break;
	case EMPET_REQ_DUAL_SKILL_FB_ALREADY:
		CInnerInterface::GetInstance().PrintMsgText ( 
			NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("EMPET_REQ_DUAL_SKILL_FB_ALREADY") );
		break;
	case EMPET_REQ_DUAL_SKILL_FB_INVALID_STORAGE:
		CInnerInterface::GetInstance().PrintMsgText ( 
			NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("EMPET_REQ_DUAL_SKILL_FB_INVALIDSTORAGE") );
		break;
	case EMPET_REQ_DUAL_SKILL_FB_INVALID_SKILL:
		CInnerInterface::GetInstance().PrintMsgText ( 
			NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("EMPET_REQ_DUAL_SKILL_FB_INVALID_SKILL") );
		break;
	}
}
	...
	case EMPET_REQ_LEARNSKILL_FB_OK:
		{
			...
			// 스킬콤보박스 리프레쉬
			//CInnerInterface::GetInstance().GetInventoryWindow()->GetPetWindow()->SetSkillCombo();//add new pet
			//CInnerInterface::GetInstance().GetCharacterWindow()->SetSkillcombo();
...
void GLPetClient::MsgRemoveSkillFact ( NET_MSG_GENERIC* nmg )
{
	...
	else if (pNetMsg->dSkillOrder == EMPET_SKILL_ORDER_SECOND)
	{
		...
		m_sActiveSkillID2 = NATIVEID_NULL();
	}
	//m_sActiveSkillID = NATIVEID_NULL();

	if( (m_sActiveSkillID == NATIVEID_NULL()) && (m_sActiveSkillID2 == NATIVEID_NULL()))
...
void GLPetClient::MsgProcess ( NET_MSG_GENERIC* nmg )
{
	...
	case NET_MSG_PET_DUAL_SKILL_FB:								MsgDualSkillLearn(nmg);
		break;
