...
#include "../../EngineLib/DxOctree/DxLandEff.h"
...
GLLevelFile::GLLevelFile(void):
	...
	m_bRedZone(false),//RedZone Integration SeiferXIII 2/11/14
	m_bDomiWars(false),

	m_bClubDeathMatch(false),
	m_bSchoolWars(false),
	m_bRoyalRumble(false),
	m_bCardActivity(false),//added - Anti SP, BP During War
...
//	m_bPKZone			= rvalue.m_bPKZone;
	...
	m_bRedZone	= rvalue.m_bRedZone;//RedZone Integration SeiferXIII 2/11/14
	m_bDomiWars = rvalue.m_bDomiWars;
...
//	m_bInstantMap		= rvalue.m_bInstantMap;
	...
	m_bClubDeathMatch	= rvalue.m_bClubDeathMatch;
	m_bSchoolWars		= rvalue.m_bSchoolWars;
	m_bRoyalRumble		= rvalue.m_bRoyalRumble;
	m_bCardActivity		= rvalue.m_bCardActivity;//added - Anti SP, BP During War
...
BOOL SLEVEL_ETC_FUNC::SAVE ( CSerialFile &SFile )
{
	...
	SFile.BeginBlock();
	{
		...
		if( m_bUseFunction[EMETCFUNC_LANDEFFECT] )
		{
			...
			for( int i = 0; i < m_vecLandEffect.size(); i++ )
			{
...
BOOL GLLevelFile::LOAD_106 ( basestream &SFile, BOOL bCLIENT, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	if ( !bCLIENT )
	{
		m_MobSchMan.LoadFile_106 ( SFile );
	}
	else
	{
		SFile.SetOffSet ( SFile.GetfTell() + dwBLOCKSIZE );
...
BOOL GLLevelFile::LOAD_107 ( basestream &SFile, BOOL bCLIENT, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	if ( !bCLIENT )
	{
		m_MobSchMan.LoadFile_107 ( SFile );
	}
	else
	{
		SFile.SetOffSet ( SFile.GetfTell() + dwBLOCKSIZE );
