...
SPETCARDINFO& SPETCARDINFO::operator= ( const SPETCARDINFO& rvalue )
{
	...
	m_sActiveSkillID2	= rvalue.m_sActiveSkillID2;
...
void GLPET::ASSIGN ( const GLPET& sPetData )
{
	...
	m_sActiveSkillID2	= sPetData.m_sActiveSkillID2;
...
EMPIECECHAR GLPET::GetPieceFromSlot ( PET_ACCESSORYTYPE emType )
{
	...
	case PETTYPE_A:
		{
			if ( emType == PET_ACCETYPE_SIZE ) return PIECE_SIZE;
			if ( emType == PET_ACCETYPE_A ) return PIECE_HEAD;
	...
	for ( WORD i = 0; i < PET_ACCETYPE_SIZE; ++i )
	{
		...
		if ( pITEM->IsTIMELMT() )
		{
			...
			if ( cTIME_CUR > cTIME_LMT )
			{
				...
				NetMsg.accetype = (PET_ACCESSORYTYPE)i;
				...
				NetMsgBrd.accetype = (PET_ACCESSORYTYPE)i;
...
namespace COMMENT
{
	std::string PET_TYPE[PETTYPE_NEWSIZE] = 
	{ 
		"PET_TYPE_A", 
		"PET_TYPE_B",
		"PET_TYPE_C",
