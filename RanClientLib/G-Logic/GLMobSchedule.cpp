...
namespace COMMENT
{
	...
	std::string EMCROWMAP[EMCROWMAP_SIZE] =
	{
		"None",
		"Normal",
		"Bus",
...
void GLMobSchedule::operator= ( const GLMobSchedule &MobSch )
{
	...
	m_emCrowMap = MobSch.m_emCrowMap;
...
bool GLMobSchedule::IsRegen()
{
	if( GetUseRegenTime() )
	{
		...
		for( int i = 0; i < 7; i++ )
		{
			if( curTime.GetDayOfWeek() == (i+1) && !m_bDayOfWeek[i] ) 
			{
				m_RegenTime = 0;
				return FALSE;
			}
...
void GLMobSchedule::ModifierSchList( D3DXVECTOR3* pOffset, NavigationMesh* pNaviMesh )
{
	...
	for( ; pCur; pCur = pCur->pNext )
	{
		if ( pCur->Data.emAction == EMACTION_ACTSTAY )
			continue;
