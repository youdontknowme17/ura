...
void GLPetClient::StartSkillProc2 ()
{
	//	스킬 정보 가져옴.
...
void GLPetClient::ProcessAutoUsePotSkill()
{
		...
		case EMACT_SLOT_DRUG:
			{
				...
				//added by CNDev
				if ( pITEM->sDrugOp.emDrug == ITEM_DRUG_EMERGENCY_POTION )
				{
					if ( m_pOwner->ISANTISKILL() && m_pOwner->ISANTISTUN() )
					{
						m_pOwner->ReqInvenDrug ( pInvenItem->wPosX, pInvenItem->wPosY );
						return;
					}
...
void GLPetClient::ProceedSkill2 ()
{
	// 스킬이 구동중인지?
	if ( !m_bSkillProcessing ) return;
...
void GLPetClient::ProcessPickSkill(int skillOrder)
{
	...
	if ( !IsSTATE(EM_PETACT_RUN_PICKUP_ITEM) )
	{
			...
			case EM_FIELDITEM_STATE_INSERTFAIL:
				{
					...
					if(skillOrder == 0)
						...
						ReqChangeActiveSkill2 ( NATIVEID_NULL() ); // 스킬을 취소해버린다.
		...
		else 
		{
				if (skillOrder == 0)
					...
					GetItembySkill ( m_sActiveSkillID2 ); 
		}
	}
}
...
SNATIVEID GLPetClient::GetActiveSkill1()
{
	return m_sActiveSkillID;
}


SNATIVEID GLPetClient::GetActiveSkill2()
{
	return m_sActiveSkillID2;
}