...
enum EMINVEN_BIKE_CHANGE_FB //add bike color
{
	EMINVEN_BIKE_CHANGE_FB_FAIL		= 0,	//	일반오류.
	EMINVEN_BIKE_CHANGE_FB_OK		= 1,	//	성공.
	EMINVEN_BIKE_CHANGE_FB_NOITEM	= 2,	//	아이템이 없음.
	EMINVEN_BIKE_CHANGE_FB_BADITEM	= 3,	//	사용 불가능 아이템.
	EMINVEN_BIKE_CHANGE_FB_BADCLASS	= 4,	//	케릭터 직업이 틀려서 사용 불가능 아이템.
	EMINVEN_BIKE_CHANGE_FB_COOLTIME = 5,	//	쿨타임 실패~
};

enum EMINVEN_ITEMCOLOR_CHANGE_FB //add itemcolor
{
	EMINVEN_ITEMCOLOR_CHANGE_FB_FAIL		= 0,	//	일반오류.
	EMINVEN_ITEMCOLOR_CHANGE_FB_OK		= 1,	//	성공.
	EMINVEN_ITEMCOLOR_CHANGE_FB_NOITEM	= 2,	//	아이템이 없음.
	EMINVEN_ITEMCOLOR_CHANGE_FB_BADITEM	= 3,	//	사용 불가능 아이템.
	EMINVEN_ITEMCOLOR_CHANGE_FB_BADCLASS	= 4,	//	케릭터 직업이 틀려서 사용 불가능 아이템.
	EMINVEN_ITEMCOLOR_CHANGE_FB_COOLTIME = 5,	//	쿨타임 실패~
};
...
enum EMVEHICLE_REQ_GIVE_BATTERY_FB
{
	...
	EMVEHICLE_REQ_GET_COLOR_FROMDB_OK			= 6,	//add bike color
	EMVEHICLE_REQ_GET_COLOR_FROMDB_FAIL			= 7,	//add bike color
};
...
namespace GLMSG
{
	...
	struct SNETPC_REQ_TYRANNY_GETREWARD
	{
		NET_MSG_GENERIC		nmg;
		LONGLONG			lnValue;

		SNETPC_REQ_TYRANNY_GETREWARD ()
			: lnValue(0)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_REQ_TYRANNY_GETREWARD;
		}
	};

	struct SNETPC_REQ_TYRANNY_GETREWARD_FB 
	{
		NET_MSG_GENERIC		nmg;
		LONGLONG			lnValue;

		SNETPC_REQ_TYRANNY_GETREWARD_FB ()
			: lnValue(0)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_REQ_TYRANNY_GETREWARD_FB;
		}
	};
	...
	struct SNETPC_UPDATE_MONEYTYPE
	{
		...
		LONGLONG			lnPremP;
		LONGLONG			lnVoteP;
		LONGLONG			lnContribP; //add contributionpoint by CNDev
		LONGLONG			lnSaldo;
	...
	////add charinfoview
	struct SNET_VIEW_PROPERTIES
	{
		NET_MSG_GENERIC		nmg;

		DWORD				dwTargetID;

		SNET_VIEW_PROPERTIES () :
		dwTargetID(0)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_VIEW_PROPERTIES;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNET_VIEW_PROPERTIES_TAR
	{
		NET_MSG_GENERIC		nmg;
		
		DWORD				dwMasterID;

		SNET_VIEW_PROPERTIES_TAR () :
		dwMasterID(0)
		{
			nmg.dwSize = sizeof(*this);
	...
	struct SNET_VIEW_PROPERTIES_FB
	{
		NET_MSG_GENERIC		nmg;
		DWORD				dwMasterID;
		DWORD				dwTargetID;
		...
		DWORD               Viewpropertise;
		char			szName[CHAR_SZNAME];
		DWORD           WeizhiTo;


		SNET_VIEW_PROPERTIES_FB () 
			: dwMasterID(0)
			, Viewpropertise(0)
			, dwTargetID(0)
			, nSlot(0)
		...
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_VIEW_PROPERTIES_FB;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
	...
	struct SNETPC_REQ_TOPUP
	{
		...
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_REQ_TOPUP;
		}
	};

	struct SNETPC_UPDATE_EPOINT
	{
		NET_MSG_GENERIC		nmg;
		LONGLONG			lnSaldo;

		SNETPC_UPDATE_EPOINT()
			: lnSaldo(0)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_UPDATE_EPOINT;
		}
	};
	...
	struct SNETPC_UPDATE_PKWIN
	{
		...
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_UPDATE_PKWIN;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNETPC_UPDATE_PKLOSS
	{
		NET_MSG_GENERIC		nmg;
		DWORD					dwPkLoss;
		std::string				strKilledBy;

		SNETPC_UPDATE_PKLOSS () :
			dwPkLoss(0)
			,strKilledBy("")
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_UPDATE_PKLOSS;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNETPC_UPDATE_DUELWIN //add duel bet
	{
		NET_MSG_GENERIC		nmg;
		LONGLONG			llWin;
		DWORD				wType;

		SNETPC_UPDATE_DUELWIN () :
			llWin(0)
			,wType(0)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_UPDATE_DUELWIN;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNETPC_UPDATE_DUELLOSS //add duel bet
	{
		NET_MSG_GENERIC		nmg;
		LONGLONG			llLoss;
		DWORD				wType;

		SNETPC_UPDATE_DUELLOSS () :
			llLoss(0)
			,wType(0)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_UPDATE_DUELLOSS;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNETPC_UPDATE_CP //add cp
	{
		NET_MSG_GENERIC		nmg;
		DWORD			dwRCLvl;
		WORD			wRCNum;
	...
	struct SNETPC_UPDATE_PKWIN_BRD : public SNETPC_BROAD //add pk
	{
		SNETPC_UPDATE_PKWIN_BRD ()
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_UPDATE_PKWIN_BRD;
		}
	};

	struct SNETPC_UPDATE_PKLOSS_BRD : public SNETPC_BROAD //add pk
	{
		SNETPC_UPDATE_PKLOSS_BRD ()
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_UPDATE_PKLOSS_BRD;
		}
	};
	...
	struct SNETPC_REQ_DISABLESKILLEFF
	{
		NET_MSG_GENERIC		nmg;
		DWORD				dwSKILL;

		SNETPC_REQ_DISABLESKILLEFF () :
			dwSKILL(0)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_REQ_DISABLESKILLEFF;
		}
	};

	struct SNETPC_REQ_DISABLESKILLEFF_FB
	{
		NET_MSG_GENERIC		nmg;
		DWORD				dwSKILL;

		SNETPC_REQ_DISABLESKILLEFF_FB () :
			dwSKILL(0)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_REQ_DISABLESKILLEFF_FB;
		}
	};

	struct SNETPC_REQ_DISABLESKILLEFF_BRD : public SNETPC_BROAD //add pk
	{
		DWORD dwSKILL;

		SNETPC_REQ_DISABLESKILLEFF_BRD ():
			dwSKILL(0)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_REQ_DISABLESKILLEFF_BRD;
		}
	};

	//Link Skill Effect by NaJ
	struct SNETPC_REQ_LINKSKILLEFF
	{
		NET_MSG_GENERIC		nmg;
		SNATIVEID			sSKILLID;
		WORD				wSKILLLevel;
		...
		SNETPC_REQ_LINKSKILLEFF ():
			sSKILLID(NATIVEID_NULL()),
			wSKILLLevel(0),
			dwSelect(0)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_REQ_LINKSKILLEFF;
		}
	};

	//Link Skill Effect by NaJ
	struct SNETPC_REQ_LINKSKILLEFF_FB
	{
		NET_MSG_GENERIC		nmg;
		SNATIVEID			sSKILLID;
		WORD				wSKILLLevel;
		...
		SNETPC_REQ_LINKSKILLEFF_FB ():
			sSKILLID(NATIVEID_NULL()),
			wSKILLLevel(0),
			dwSelect(0)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_REQ_LINKSKILLEFF_FB;
		}
	};

	//Link Skill Effect by NaJ
	struct SNETPC_REQ_LINKSKILLEFF_BRD : public SNETPC_BROAD
	{
		SNATIVEID			sSKILLID;
		WORD				wSKILLLevel;
		DWORD				dwSelect;

		SNETPC_REQ_LINKSKILLEFF_BRD ():
			sSKILLID(NATIVEID_NULL()),
			wSKILLLevel(0),
			dwSelect(0)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_REQ_LINKSKILLEFF_BRD;
		}
	};
//add addstats cmd
	...
	struct SNETPC_REQ_RANKNAME // add pkrank
	{
		NET_MSG_GENERIC		nmg;
		BOOL				bRankName;
	...
	struct SNETPC_REQ_RANKMARK // add pkrank
	{
		NET_MSG_GENERIC		nmg;
		BOOL				bRankMark;
	...
	struct SNETPC_REQ_ALLOW_BET // add duel bet
	{
		NET_MSG_GENERIC		nmg;
		BOOL				bAllowBet;

		SNETPC_REQ_ALLOW_BET () :
			bAllowBet(FALSE)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_REQ_ALLOW_BET;
		}
	};

	struct SNETPC_REQ_RANKNAME_FB // add pkrank
	{
		NET_MSG_GENERIC		nmg;
		BOOL				bRankName;
	...
	struct SNETPC_REQ_RANKMARK_FB // add pkrank
	{
		NET_MSG_GENERIC		nmg;
		BOOL				bRankMark;
	...
	struct SNETPC_REQ_ALLOW_BET_FB // add duel bet
	{
		NET_MSG_GENERIC		nmg;
		BOOL				bAllowBet;

		SNETPC_REQ_ALLOW_BET_FB () :
			bAllowBet(FALSE)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_REQ_ALLOW_BET_FB;
		}
	};

	struct SNETPC_REQ_RANKNAME_BRD : public SNETPC_BROAD // add pkrank
	{
		BOOL		bRankName;
	...
	struct SNETPC_REQ_RANKMARK_BRD : public SNETPC_BROAD // add pkrank
	{
		BOOL		bRankMark;
	...
	struct SNETPC_INVEN_BIKECOLOR_CHANGE //add bike color
	{
		NET_MSG_GENERIC			nmg;
		...
		{
			nmg.dwSize = (DWORD) sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_INVEN_BIKECOLOR_CHANGE;
		}
	};

	struct SNETPC_INVEN_BIKECOLOR_CHANGE_FB //add bike color
	{
		NET_MSG_GENERIC			nmg;
		
		EMINVEN_BIKE_CHANGE_FB	emFB;
		WORD					wBikeColor;
		WORD					wBikeColor1;
		WORD					wBikeColor2;
		WORD					wBikeColor3;
		...
		SNETPC_INVEN_BIKECOLOR_CHANGE_FB () :
			emFB(EMINVEN_BIKE_CHANGE_FB_FAIL),
			wBikeColor(0),
			wBikeColor1(0),
			wBikeColor2(0),
			wBikeColor3(0),
		...
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_INVEN_BIKECOLOR_CHANGE_FB;
		}
	};

	struct SNETPC_INVEN_BIKECOLOR_CHANGE_BRD : public SNETPC_BROAD //add bike color
	{
		WORD					wBikeColor;
		WORD					wBikeColor1;
		WORD					wBikeColor2;
		...
		SNETPC_INVEN_BIKECOLOR_CHANGE_BRD () :
			wBikeColor(0),
			wBikeColor1(0),
			wBikeColor2(0),
			wBikeColor3(0),
		...
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_INVEN_BIKECOLOR_CHANGE_BRD;
		}
	};

	struct SNETPC_SLOT_ITEMCOLOR_CHANGE //add itemcolor
	{
		NET_MSG_GENERIC			nmg;
		...
		{
			nmg.dwSize = (DWORD) sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_SLOT_ITEMCOLOR_CHANGE;
		}
	};

	struct SNETPC_SLOT_ITEMCOLOR_CHANGE_FB //add itemcolor
	{
		NET_MSG_GENERIC			nmg;
		
		EMINVEN_ITEMCOLOR_CHANGE_FB	emFB;
		WORD					wItemColor;
		EMSLOT					emSlot;

		SNETPC_SLOT_ITEMCOLOR_CHANGE_FB () :
			emFB(EMINVEN_ITEMCOLOR_CHANGE_FB_FAIL)
			, wItemColor(0)
			, emSlot(SLOT_TSIZE)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_SLOT_ITEMCOLOR_CHANGE_FB;
		}
	};

	struct SNETPC_SLOT_ITEMCOLOR_CHANGE_BRD : public SNETPC_BROAD //add itemcolor
	{
		WORD					wItemColor;
		EMSLOT					emSlot;

		SNETPC_SLOT_ITEMCOLOR_CHANGE_BRD () :
			  wItemColor(0)
			, emSlot(SLOT_TSIZE)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_SLOT_ITEMCOLOR_CHANGE_BRD;
		}
	};
	...
	struct SNETPC_QITEMFACT_TIMEUPDATE_BRD : public SNETPC_BROAD //add qbox
	{
		float fQBoxTime;

		SNETPC_QITEMFACT_TIMEUPDATE_BRD ()
			: fQBoxTime(0.0f)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_QITEMFACT_TIMEUPDATE_BRD;
		}
	};
...
//add clubwar reward
	struct SNETPC_REQ_CLUBWAR_REWARD_CLT
	{
		NET_MSG_GENERIC		nmg;
		SNATIVEID			idClubWarReward;

		SNETPC_REQ_CLUBWAR_REWARD_CLT ()
			: idClubWarReward ( NATIVEID_NULL() )
		{
			nmg.dwSize = sizeof(SNETPC_REQ_CLUBWAR_REWARD_CLT);
			nmg.nType = NET_MSG_REQ_CLUBWAR_REWARD_CLT;
		}
	};
